export const supabaseFacadeAllowlist = [
  'src/app/bootstrap.tsx',
  'src/app/hooks/useAuthGuard.ts',
  'src/app/hooks/useOnboardingFlow.ts',
  'src/features/account/hooks/useApiTokens.ts',
  'src/features/billing/hooks/useCredits.ts',
  'src/features/lora/hooks/useHuggingFaceUpload.ts',
  'src/features/resources/hooks/useResourceBrowserData.ts',
  'src/features/resources/hooks/useResources.ts',
  'src/features/settings/hooks/useApiKeys.ts',
  'src/pages/Home/hooks/auth/useAuthReferralFinalize.ts',
  'src/pages/Home/hooks/auth/useHomeAuthSubscription.ts',
  'src/pages/Home/hooks/auth/useOAuthHashSessionRestore.ts',
  'src/pages/Home/hooks/auth/useStandaloneAuthRedirect.ts',
  'src/pages/Home/hooks/useDiscordSignIn.ts',
  'src/pages/share/hooks/useSharePageData.ts',
  'src/shared/components/CreditsManagement/hooks/useTaskLogDownload.ts',
  'src/shared/components/GlobalHeader/useGlobalHeaderAuth.ts',
  'src/shared/components/ImageGenerationForm/hooks/legacyMigrations/useResourceMigration.ts',
  'src/shared/components/ImageGenerationForm/hooks/referenceUpload/referenceDomainService.ts',
  'src/shared/components/ImageGenerationForm/hooks/referenceUpload/useStyleReferenceUploadHandler.ts',
  'src/shared/components/LoraSelectorModal/components/MyLorasTab/hooks/useLoraFormState.ts',
  'src/shared/components/MediaGallery/hooks/useMediaGalleryLightboxControllers.ts',
  'src/shared/components/MediaLightbox/components/submitSegmentTask.ts',
  'src/shared/components/MediaLightbox/hooks/inpainting/useEditModePersistence.ts',
  'src/shared/components/MediaLightbox/hooks/reposition/useRepositionVariantSave.ts',
  'src/shared/components/MediaLightbox/hooks/useAdjustedTaskDetails.ts',
  'src/shared/components/MediaLightbox/hooks/useGenerationEditSettings.ts',
  'src/shared/components/MediaLightbox/hooks/useMakeMainVariant.ts',
  'src/shared/components/MediaLightbox/hooks/useReferences.ts',
  'src/shared/components/MediaLightbox/hooks/useReplaceInShot.ts',
  'src/shared/components/MediaLightbox/hooks/useSourceGeneration.ts',
  'src/shared/components/MediaLightbox/hooks/useVariantPromotion.ts',
  'src/shared/components/MediaLightbox/hooks/useVideoRegenerateMode.ts',
  'src/shared/components/MotionPresetSelector/SelectedPresetCard.tsx',
  'src/shared/components/MotionPresetSelector/useMotionPresets.ts',
  'src/shared/components/modals/ReferralModal.tsx',
  'src/shared/components/ReferralModal.tsx',
  'src/shared/components/SegmentSettingsForm/hooks/useStructureVideoUpload.ts',
  'src/shared/components/SettingsModal/SettingsModal.tsx',
  'src/shared/components/ShotImageManager/hooks/useExternalGenerations.ts',
  'src/shared/components/TaskDetails/VideoTravelDetails.tsx',
  'src/shared/components/TasksPane/hooks/useImageGeneration.ts',
  'src/shared/components/TasksPane/hooks/useTaskErrorDisplay.ts',
  'src/shared/components/TasksPane/hooks/useTaskNavigation.ts',
  'src/shared/components/TasksPane/hooks/useTasksLightbox.ts',
  'src/shared/components/TasksPane/hooks/useVideoGenerations.ts',
  'src/shared/components/TasksPane/utils/task-utils.ts',
  'src/shared/components/VideoTrimEditor/hooks/useTrimSave.ts',
  'src/shared/components/ui/hooks/useAIInputTextPopover.ts',
  'src/shared/contexts/AuthContext.tsx',
  'src/shared/contexts/UserSettingsContext.tsx',
  'src/shared/hooks/billing/useAutoTopup.ts',
  'src/shared/hooks/gallery/useVideoGalleryPreloader.ts',
  'src/shared/hooks/media/useEditToolMediaPersistence.ts',
  'src/shared/hooks/projects/useProjectCRUD.ts',
  'src/shared/hooks/projects/useProjectGenerationModesCache.ts',
  'src/shared/hooks/projects/useProjectGenerations.ts',
  'src/shared/hooks/projects/useProjectVideoCountsCache.ts',
  'src/shared/hooks/projects/services/projectSetupRepository.ts',
  'src/shared/hooks/segments/segmentOutputsQueries.ts',
  'src/shared/hooks/segments/usePairMetadata.ts',
  'src/shared/hooks/segments/useSegmentMutations.ts',
  'src/shared/hooks/segments/useShotVideoSettings.ts',
  'src/shared/hooks/shots/addImageToShotHelpers.ts',
  'src/shared/hooks/shots/externalImageDrop.ts',
  'src/shared/hooks/shots/useDuplicateAsNewGeneration.ts',
  'src/shared/hooks/shots/useDuplicateShotWithVideos.ts',
  'src/shared/hooks/shots/useShotCreation.ts',
  'src/shared/hooks/shots/useShotGenerationMutations.ts',
  'src/shared/hooks/shots/useShotUpdates.ts',
  'src/shared/hooks/shots/useShotsCrud.ts',
  'src/shared/hooks/shots/useShotsQueries.ts',
  'src/shared/hooks/sourceImageChanges/dataAccess.ts',
  'src/shared/hooks/tasks/paginatedTaskRepository.ts',
  'src/shared/hooks/tasks/useTaskCancellation.ts',
  'src/shared/hooks/tasks/useTaskLog.ts',
  'src/shared/hooks/tasks/useTaskPlaceholder.ts',
  'src/shared/hooks/tasks/useTaskPrefetch.ts',
  'src/shared/hooks/tasks/useTaskStatusCounts.ts',
  'src/shared/hooks/tasks/useTaskType.ts',
  'src/shared/hooks/tasks/useTasks.ts',
  'src/shared/hooks/timeline/timelineMutationService.ts',
  'src/shared/hooks/toolSettingsWriteRepository.ts',
  'src/shared/hooks/useLineageChain.ts',
  'src/shared/hooks/useLoadVariantImages.ts',
  'src/shared/hooks/useMarkVariantViewed.ts',
  'src/shared/hooks/useOnboarding.ts',
  'src/shared/hooks/usePendingGenerationTasks.ts',
  'src/shared/hooks/usePendingSegmentTasks.ts',
  'src/shared/hooks/usePromoteVariantToGeneration.ts',
  'src/shared/hooks/useReferralTracking.ts',
  'src/shared/hooks/useResources.ts',
  'src/shared/hooks/useShareGeneration.ts',
  'src/shared/hooks/useShotGenerationMetadata.ts',
  'src/shared/hooks/useShotImages.ts',
  'src/shared/hooks/useSpecificResources.ts',
  'src/shared/hooks/useTimelineCore.enhancedPromptOperations.ts',
  'src/shared/hooks/useTimelineCore.pairOperations.ts',
  'src/shared/hooks/useTimelineCore.ts',
  'src/shared/hooks/useToggleVariantStar.ts',
  'src/shared/hooks/useUserUIState.ts',
  'src/shared/hooks/useVariants.ts',
  'src/shared/hooks/useVoiceRecording.ts',
  'src/shared/lib/debug/debugPolling.ts',
  'src/shared/lib/generationTaskCache.ts',
  'src/shared/lib/tasks/generationTaskRepository.ts',
  'src/shared/lib/generationTransformers.ts',
  'src/shared/lib/logger.ts',
  'src/shared/lib/media/imageUploader.ts',
  'src/shared/lib/media/videoThumbnailGenerator.ts',
  'src/shared/lib/media/videoUploader.ts',
  'src/shared/lib/projectSettingsInheritance.ts',
  'src/shared/lib/shotSettingsInheritance.ts',
  'src/shared/lib/taskCreation/resolution.ts',
  'src/shared/lib/tasks/individualTravelSegment.ts',
  'src/shared/lib/tasks/shotParentGeneration.ts',
  'src/shared/lib/timelineFrameBatchPersist.ts',
  'src/shared/lib/toolSettingsService.ts',
  'src/shared/lib/toolSettingsWriteService.ts',
  'src/shared/realtime/RealtimeConnection.ts',
  'src/shared/services/externalApiKeys/repository.ts',
  'src/shared/services/projects/projectSetupRepository.ts',
  'src/tools/character-animate/pages/uploadMedia.ts',
  'src/tools/edit-images/pages/EditImagesPage.tsx',
  'src/tools/edit-video/pages/EditVideoPage.tsx',
  'src/tools/video-editor/data/SupabaseDataProvider.ts',
  'src/tools/video-editor/adapters/reigh/generationLookup.ts',
  'src/tools/video-editor/adapters/reigh/staleVariantRepository.ts',
  'src/tools/video-editor/adapters/reigh/useReighEffectsCatalog.ts',
  'src/tools/video-editor/adapters/reigh/useReighTimelinesList.ts',
  'src/tools/video-editor/adapters/reigh/variantPromotionLookup.ts',
  'src/tools/video-editor/hooks/useAgentSession.ts',
  'src/tools/training-data-helper/hooks/useTrainingData.ts',
  'src/tools/training-data-helper/hooks/useTrainingDataBatches.ts',
  'src/tools/training-data-helper/hooks/useTrainingDataUpload.ts',
  'src/tools/training-data-helper/hooks/useVideoUrlCache.ts',
  'src/tools/travel-between-images/components/ProjectSelectorModal.tsx',
  'src/tools/travel-between-images/components/SelectedPresetCard.tsx',
  'src/tools/travel-between-images/components/ShotEditor/hooks/actions/useDropActions.ts',
  'src/tools/travel-between-images/components/ShotEditor/hooks/actions/useJoinSegmentsHandler.ts',
  'src/tools/travel-between-images/components/ShotEditor/hooks/editor-state/timelineDropHelpers.ts',
  'src/tools/travel-between-images/components/ShotEditor/hooks/editor-state/useImageManagement.ts',
  'src/tools/travel-between-images/components/ShotEditor/hooks/video/useLastVideoGeneration.ts',
  'src/tools/travel-between-images/components/ShotEditor/services/applySettings/imageService.ts',
  'src/tools/travel-between-images/components/ShotEditor/services/applySettings/taskDataService.ts',
  'src/tools/travel-between-images/components/ShotEditor/services/generateVideoService.ts',
  'src/tools/travel-between-images/components/ShotEditor/ui/Header.tsx',
  'src/tools/travel-between-images/components/ShotImagesEditor/hooks/useFrameCountUpdater.ts',
  'src/tools/travel-between-images/components/ShotImagesEditor/services/segmentDeletionService.ts',
  'src/tools/travel-between-images/components/Timeline/TimelineContainer/components/GuidanceVideoControls.tsx',
  'src/tools/travel-between-images/components/Timeline/hooks/segment/timelineTrailingEndpointPersistence.ts',
  'src/tools/travel-between-images/components/Timeline/hooks/segment/useSegmentDeletion.ts',
  'src/tools/travel-between-images/components/VideoGallery/hooks/useVideoItemJoinClips.ts',
  'src/tools/travel-between-images/components/hooks/useBatchGuidanceVideoController.ts',
  'src/tools/travel-between-images/components/hooks/useFinalVideoSectionController.ts',
  'src/tools/travel-between-images/components/hooks/useMotionControlPresetState.ts',
  'src/tools/travel-between-images/hooks/navigation/useHashDeepLink.ts',
  'src/tools/travel-between-images/hooks/settings/useSegmentPromptMetadata.ts',
  'src/tools/travel-between-images/hooks/settings/useShotSettings.ts',
  'src/tools/travel-between-images/hooks/useShareActions.ts',
  'src/tools/travel-between-images/hooks/video/useShotFinalVideos.ts',
  'src/tools/travel-between-images/hooks/workflow/useDemoteOrphanedVariants.ts',
  'src/domains/lora/components/LoraSelectorModal/components/MyLorasTab/hooks/useLoraFormState.ts',
  'src/domains/media-lightbox/components/submitSegmentTask.ts',
  'src/domains/media-lightbox/hooks/reposition/useRepositionVariantSave.ts',
  'src/domains/media-lightbox/hooks/useMakeMainVariant.ts',
  'src/domains/media-lightbox/hooks/useSourceGeneration.ts',
  'src/domains/media-lightbox/hooks/useVariantPromotion.ts',
  'src/domains/media-lightbox/hooks/useVideoRegenerateMode.ts',
  'src/features/projects/services/projectSetupRepository.ts',
  'src/features/tasks/components/TasksPane/hooks/useImageGeneration.ts',
  'src/features/tasks/components/TasksPane/hooks/useTaskErrorDisplay.ts',
  'src/features/tasks/components/TasksPane/hooks/useTaskNavigation.ts',
  'src/features/tasks/components/TasksPane/hooks/useVideoGenerations.ts',
  'src/features/tasks/components/TasksPane/utils/task-utils.ts',
  'src/shared/components/ImageGenerationForm/hooks/legacyMigrations/useGenerationBackfillMigration.ts',
  'src/shared/components/ImageGenerationForm/hooks/useReferenceResourceMutations.ts',
  'src/shared/components/ai-input/useAIInputTextPopover.ts',
  'src/shared/hooks/account/useApiTokens.ts',
  'src/shared/hooks/ai/useAIInteractionService.ts',
  'src/shared/hooks/billing/useCredits.ts',
  'src/shared/hooks/resources/useResourceBrowserData.ts',
  'src/shared/hooks/settings/useApiKeys.ts',
  'src/shared/hooks/settings/usePrefetchToolSettings.ts',
  'src/shared/hooks/settings/useToolSettings.ts',
  'src/shared/hooks/shots/useShotGenerationMetadata.ts',
  'src/shared/hooks/shots/useShotImages.ts',
  'src/shared/hooks/shots/useUpdateShotAspectRatio.ts',
  'src/shared/hooks/tasks/taskLogPipeline.ts',
  'src/shared/hooks/tasks/usePendingSegmentTasks.ts',
  'src/shared/hooks/timeline/useTimelineCore.enhancedPromptOperations.ts',
  'src/shared/hooks/timeline/useTimelineCore.pairOperations.ts',
  'src/shared/hooks/timeline/useTimelineCore.ts',
  'src/shared/hooks/variants/useLineageChain.ts',
  'src/shared/hooks/variants/useLoadVariantImages.ts',
  'src/shared/hooks/variants/useMarkVariantViewed.ts',
  'src/shared/hooks/variants/useToggleVariantStar.ts',
  'src/shared/hooks/variants/useVariants.ts',
  'src/shared/lib/media/createGenerationFromFile.ts',
  'src/shared/lib/media/materializeLocalGeneration.ts',
  'src/shared/settings/runtime/toolSettingsWriteService.ts',
  'src/tools/travel-between-images/components/ShotEditor/hooks/actions/useBoundarySummary.ts',
  'src/tools/travel-between-images/components/ShotEditor/services/generateVideo/enhancePromptsForBatch.ts',
  'src/tools/travel-between-images/components/hooks/useModalImageHandlers.ts',
  'src/tools/video-editor/data/generationAssetResolver.ts',
];
