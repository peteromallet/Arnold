...
3. Add useful read indexes on `sprints(epic_id, sprint_number)`, `sprints(epic_id, status, queue_position)`, and `sprint_items(sprint_id, position)`.

### Step 2: Extend the store protocol (`agent_kit/ports.py`)
**Scope:** Medium
1. Add sprint methods near the existing epic/checklist methods at `agent_kit/ports.py:249`: `create_sprint`, `load_sprint`, `list_sprints`, `update_sprint`, `delete_sprint`, `replace_sprint_items`, `list_sprint_items`.
...
4. Return structured changes and blockers so `edit_epic` can report exactly what failed.
...
6. On force-through, still compute blockers, apply the transition, and log `forced_handoff` with bypassed conditions in `prior_state` or event details.
...
2. Snapshot enough prior state before each sprint/status write to make revert reliable.

## Phase 4: Prompt And Turn Behavior

### Step 9: Update Arnold instructions (`prompts/system.md`)
**Scope:** Small
...
### Step 10: Make end-of-turn checks phase-aware (`agent_kit/end_of_turn.py`, `agent_kit/loop.py`)
**Scope:** Medium
...
### Step 11: Add focused unit tests (`tests/test_sprints.py`, `tests/test_gating.py`, `tests/test_lockdown.py`)
**Scope:** Medium
...
### Step 12: Add adapter and migration tests (`tests/store_contract.py`, `tests/test_supabase_adapters.py`, `tests/test_sqlite_store.py`)
**Scope:** Medium
...
### Step 13: Add integration tests (`tests/test_sprint_mode_lifecycle.py`)
**Scope:** Large
...
3. Verify post-handoff `queue sprint 2` and `do sprint 3 first` update positions and log `sprint_status_change`.
...
3. Wire `edit_epic` once the validators and sprint operations are tested directly.
4. Update prompts and end-of-turn checks after server behavior exists.
5. Finish with full lifecycle integration tests and the existing suite.
...
3. Run lifecycle tests: `pytest tests/test_sprint_mode_lifecycle.py tests/test_editorial_loop.py tests/test_end_of_turn.py`.
4. Run the full test suite with `pytest` after targeted tests pass.
