...
The relevant spec anchors are `planning-bot-spec.md` Sprint 5, Code Investigation, `codebases`, `code_artifacts`, and code tool sections. The implementation should keep GitHub read-only, require `GITHUB_PAT`, cache API responses and `analyze_code` results in `code_artifacts`, and preserve cached content when a repo later returns 404.
...
### Step 3: Add a GitHub REST client (`agent_kit/github_client.py`)
**Scope:** Medium
...
3. Track `X-RateLimit-*` headers and log a `warn` `external_api` system event when usage reaches 80% of the hourly limit.
...
### Step 4: Add cache helpers (`agent_kit/code_cache.py`)
**Scope:** Medium
...
4. On deleted repo 404, return a failure payload while leaving existing artifacts untouched and available for future reads.

## Phase 3: Tools And Runtime Wiring

### Step 5: Register codebase management tools (`agent_kit/tools/code.py`)
**Scope:** Medium
...
### Step 6: Register investigation tools (`agent_kit/tools/code.py`, `agent_kit/loop.py`)
**Scope:** Large
1. Add `get_codebase_tree(codebase_id, path?)` backed by GitHub tree API and hourly API cache.
2. Add `read_codebase_file(codebase_id, file_path, line_range?)`, with line range parsing and content truncation rules aligned to the 10k excerpt limit.
3. Add `search_code(codebase_id, query, type?)`, mapping the four spec search types to GitHub search queries or local filtering over cached tree/file data where simpler.
4. Add `analyze_code(codebase_ids, scope, question)`, accepting multiple codebase IDs, gathering relevant tree/search/file snippets, returning per-codebase coverage, and caching identical requests for one hour.
5. Import `agent_kit.tools.code` in `agent_kit/loop.py` so the tools appear in model definitions.

### Step 7: Add code artifact tools (`agent_kit/tools/code.py`, `agent_kit/tools/editorial.py`)
**Scope:** Medium
1. Add `save_code_excerpt(epic_id, source, content, summary, codebase_id?, file_path?, line_range?)`, storing `kind='excerpt'`, `source`, `content_summary`, and artifact metadata.
...
### Step 8: Update prompt/hot context behavior (`agent_kit/prompts.py`, `agent_kit/store/*`)
**Scope:** Medium
...
### Step 9: Add org populator (`scripts/populate_codebases.py`, `scripts/codebase_groups.yaml`)
**Scope:** Medium
...
### Step 10: Add cache cleanup entry point (`scripts/cleanup_code_artifacts.py` or store helper)
**Scope:** Small
...
### Step 11: Add targeted unit tests (`tests/test_codebase_store.py`, `tests/test_github_client.py`, `tests/test_code_tools.py`)
**Scope:** Large
...
### Step 12: Add integration tests (`tests/test_code_investigation.py`)
**Scope:** Large
...
2. Add the GitHub client with mocked tests before wiring tools, so provider behavior and rate-limit handling are isolated.
3. Add code tools in thin slices: management tools, then read tools, then `analyze_code`, then artifact writes.
4. Wire prompt/hot context after tool behavior is stable.
5. Add scripts and cleanup last, since they are operational wrappers over already-tested primitives.

## Validation Order
1. Run focused migration/store tests first: `pytest tests/test_codebase_store.py`.
2. Run GitHub/client/tool tests with mocked HTTP: `pytest tests/test_github_client.py tests/test_code_tools.py`.
3. Run integration chain tests: `pytest tests/test_code_investigation.py`.
4. Run affected existing suites around loop, prompts, tool registry, and Supabase adapters.
5. Finish with the full suite: `pytest`.
