...
## Handoff / release artifacts

- Rollback procedure, kill switch, and deprecation-window decision.
- Compatibility matrix for graph-born vs native-born resume behavior.
- Final docs/skills update checklist and sign-off report.
- Removal list showing which graph-builder scaffolding was deleted and which fallback code remains on purpose.

## No-go conditions

- M6 handoff does not account for every active graph-backed pipeline.
- Graph-born/native-born resume routing is not covered by tested compatibility rules.
- Rollback procedure and deprecation window are still implicit rather than documented.
- The cleanup list includes load-bearing graph-executor behavior rather than unused scaffolding.
