...
- Representative parity fixture matrix and golden-trace locations for the main `megaplan` pipeline.
- Derived-graph topology-hash baseline and validation notes.
- Explicit graph-born/native-born resume-routing rules and escape-hatch behavior.
- Prioritized M5A/M5B target list plus any remaining runtime gaps for `parallel(...)` / human-gate work.
- Must consume the explicit M3 handoff artifacts rather than reverse-engineering behavior from the donor branch.

## No-go conditions

- M3 handoff does not pin the hook mapping, executor-owned-key rules, composite cursor schema, and toy parity fixtures.
- Representative Megaplan parity fixtures have not been agreed before the rewrite starts.
- Graph-born vs native-born resume routing is still ambiguous.
- Work is implicitly depending on donor-branch behavior that is not captured in the M3 handoff.
...
- Derived graph:
  - `build_pipeline()` continues to return a `Pipeline` object, now derived from the native function via `arnold.pipeline.native.compiler.compile_pipeline(...)` plus `arnold.pipeline.native.graph_projection.project_graph(...)` (or the equivalent wrapper that uses those real surfaces).
...
- Other pipelines (they are handled in Milestones 5A, 5B, and 6).
- `parallel(...)` primitive and panel/human-gate conversions (Milestones 5A and 5B).
...
- Milestone 5A/5B handoff identifies which panel/tournament and human-gate pipelines to convert first and any remaining runtime API gaps.
...
- `arnold/pipeline/native/compiler.py` (native program compilation)
- `arnold/pipeline/native/graph_projection.py` (derived graph projection)
- `arnold/pipeline/native/runtime.py` (native execution parity/resume path)
