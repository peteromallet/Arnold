...
- *Fix:* Downgrade m0's driver to a mid-tier profile (e.g. `directed//medium` or a cheaper vendor for the driver) in `.megaplan/briefs/hardening-epic/chain.yaml`. Reserve premium depth for the semantic-edit milestones (M1/M3*/M5*). The safety-net value comes from the *tests being green on main*, not from premium authorship.
