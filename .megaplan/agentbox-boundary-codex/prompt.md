# Codex Review: Arnold / Megaplan / AgentBox Package Boundaries

Working directory: /Users/peteromalley/Documents/megaplan

Task: Review whether AgentBox, Arnold runtime primitives, and Megaplan-specific functionality are at the right package/abstraction level. Key question: should AgentBox be a package/distribution within Arnold, a package within Megaplan, or a separate package depending on Arnold? The current direction is: Arnold owns generic operation/event/resource/scheduled-task/execution-provider/credential/approval/cleanup contracts; Megaplan owns chain/ticket/plan semantics and provides the first adapter; AgentBox packages a one-machine persistent host/product with Discord-first defaults, SSH/systemd/workspace/bootstrap. Sense-check this. Return: RIGHT LEVEL, WRONG LEVEL, PACKAGE SHAPE RECOMMENDATION, DOC/EPIC CHANGES, and RISKS IF WRONG. Be concrete and skeptical. Under 1200 words.
...
Important architectural boundary: AgentBox is the first persistent-machine distribution of broader Arnold runtime primitives. Megaplan is only the first operation adapter. The operation model, scheduled-task model, execution-provider interface, approval linkage, credential manifest, and cleanup/reconcile contracts should be Arnold-level so future Arnold pipelines do not each invent their own mini-AgentBox.
...
The operation registry should be a new first-class Arnold model in the existing Store, not a renamed `cloud_run` and not a parallel database by default. Existing cloud runs, scheduled jobs, resident conversations, progress events, control messages, confirmations, watchdog observations, and chain state are reuse inputs, but Arnold needs one cross-kind operation record that can represent a chain, Codex run, Claude run, shell/test run, subagent repair, or future pipeline operation.
...
Represent pipeline-specific concrete resources as typed resources instead of baking Megaplan/tmux/worktree fields directly into the core schema:
...
resources:
  - type: git_worktree
    repo: megaplan
    branch: agent/op-123
    path: /workspace/worktrees/op-123/megaplan
  - type: process_session
    provider: tmux
    name: op-123
  - type: log
    path: /workspace/runs/op-123/log.txt
```

This keeps the same registry useful for 100+ future Arnold pipelines, including pipelines that have no git repo, many repos, GPU data volumes, containers, or external services.
...
Implementation note: these check-ins should be Arnold scheduled tasks, not ad hoc sleeps inside Guardian. Guardian is the worker that claims due tasks and runs handlers. A scheduled task should carry:
...
- operation liveness tick;
- operation deep supervision;
- daily briefing;
- credential health check;
- cleanup survey reminder;
- repo sync;
- backup;
- pending approval reminder.

For v0 one Guardian process can run all of these, but the lease/idempotency model should exist immediately so Discord Operator, Guardian, and future pipeline workers do not race.
...
- Arnold owns operation, scheduled-task, execution-provider, credential, approval, and cleanup/reconcile contracts.
...
```bash
python -m arnold_pipelines.megaplan chain start --spec .megaplan/briefs/agentbox-persistent-machine/chain.yaml
```
...
milestones:
  - label: m1-arnold-runtime-core
    idea: .megaplan/briefs/agentbox-persistent-machine/m1-arnold-runtime-core.md
    profile: partnered-5
    robustness: thorough
    depth: high
    vendor: codex
    branch: agentbox-m1-arnold-runtime-core
    notes: "Arnold-level operation/event/state/scheduled-task/resource/approval contracts. Prevents AgentBox from owning primitives every future pipeline will need."

  - label: m2-agentbox-host-provider
    idea: .megaplan/briefs/agentbox-persistent-machine/m2-agentbox-host-provider.md
    profile: partnered-5
    robustness: full
    depth: high
    vendor: codex
    branch: agentbox-m2-host-provider
    notes: "AgentBox distribution layer: repo registry, normal-checkout worktrees, host-local tmux runner, paths, resources, CLI inspection."

  - label: m3-megaplan-chain-adapter
    idea: .megaplan/briefs/agentbox-persistent-machine/m3-megaplan-chain-adapter.md
    profile: partnered-5
    robustness: full
    depth: high
    vendor: codex
    branch: agentbox-m3-megaplan-chain-adapter
    notes: "Megaplan proves the first Arnold operation adapter without making the framework Megaplan-shaped."

  - label: m4-discord-thin-path
    idea: .megaplan/briefs/agentbox-persistent-machine/m4-discord-thin-path.md
    profile: partnered-5
    robustness: full
    depth: medium
    vendor: codex
    branch: agentbox-m4-discord-thin-path
    notes: "Discord-first v0: tickets, chain launch, status, logs, and operation id via existing resident runtime."

  - label: m5-guardian-v0
    idea: .megaplan/briefs/agentbox-persistent-machine/m5-guardian-v0.md
    profile: partnered-5
    robustness: thorough
    depth: high
    vendor: codex
    branch: agentbox-m5-guardian-v0
    notes: "Arnold Guardian worker consumes scheduled tasks and operation handlers; AgentBox packages/configures it."

  - label: m6-credentials-preflight
    idea: .megaplan/briefs/agentbox-persistent-machine/m6-credentials-preflight.md
    profile: partnered-5
    robustness: thorough
    depth: high
    vendor: codex
    branch: agentbox-m6-credentials-preflight
    notes: "Arnold credential manifest/sync plus AgentBox host backend, redaction, health checks, and launch gates."

  - label: m7-completion-github-cleanup
    idea: .megaplan/briefs/agentbox-persistent-machine/m7-completion-github-cleanup.md
    profile: partnered-5
    robustness: full
    depth: medium
    vendor: codex
    branch: agentbox-m7-completion-cleanup
    notes: "Completion DM, PR metadata, Arnold cleanup/reconcile contract, and approval-gated consolidation using cleanup-loose-branches discipline."

  - label: m8-bootstrap-day2
    idea: .megaplan/briefs/agentbox-persistent-machine/m8-bootstrap-day2.md
    profile: partnered-5
    robustness: full
    depth: medium
    vendor: codex
    branch: agentbox-m8-bootstrap-day2
    notes: "AgentBox packaging: bootstrap, SSH host profile, systemd, doctor/reconcile, backup/restore."
