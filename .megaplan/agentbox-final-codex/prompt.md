# Final Codex Sense Check Brief

Review docs/agentbox-persistent-machine-plan.md after three DeepSeek challenge waves. Task: sense-check the current design. Do not praise. Find contradictions, overbuilt parts, or missing build-vs-borrow decisions. Focus on whether it delivers Peter’s desired UX: single-user Discord-first launch of tickets/epics/Megaplans/chains; Guardian fixes/relaunches; completion DMs; cleanup-loose-branches; one persistent machine with worktrees. Return: must-change, should-change, and no-change. Keep under 1000 words.
...
The operation registry should be a new first-class model, not a renamed `cloud_run`. Existing cloud runs, scheduled jobs, resident conversations, watchdog observations, and chain state are reuse inputs, but AgentBox needs one cross-kind operation record that can represent a chain, Codex run, Claude run, shell/test run, or subagent repair.
...
- delete the operation registry or `agentbox.db`;
...
Crash-safety rule: every state transition writes an append-only event to `events.ndjson` before updating the registry view. On startup, Guardian replays event streams newer than `last_event_seq` and repairs stale registry state. The event stream is the durable history; the registry is the queryable current-state view.
...
2. **Discord Operator profile**, using resident runtime:
   ...
   The resident runtime already persists conversations, coalesces message bursts, and loads hot context. AgentBox needs an Operator profile and pending-question store on top of that, plus intent classification on the coalesced message batch before tool dispatch.
...
4. **Cloud supervise:** split provider-independent chain classification from provider-specific SSH/Railway execution, or add a local provider for AgentBox.
