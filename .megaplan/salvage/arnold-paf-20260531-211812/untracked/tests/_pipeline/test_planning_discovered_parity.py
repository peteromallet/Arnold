"""T9 parity test — prove manifest-first discovery finds planning with correct metadata.

_BUILTIN_NAMES / _planning_builder / the builtin collision skip /
the read_skill_md builtin branch have been DELETED.  This test
now asserts that discover_python_pipelines() (flag-ON) returns a
'planning' entry whose manifest-derived metadata matches the expected
contract — i.e. the deletion did not break discovery parity.
"""
...
def test_discover_python_pipelines_flag_on_finds_planning_with_correct_metadata(
    ...
    """Under M6 flag-ON, planning is discovered via manifest and its
    metadata matches the expected contract.
    """
    with patch.dict(
...
def test_scan_python_pipelines_flag_on_planning_disposition_has_manifest(
    ...
    """Under flag-ON, scan_python_pipelines() returns a 'discovered'
    Disposition with a populated Manifest for planning."""
    with patch.dict(
        "os.environ", {"MEGAPLAN_M6_MANIFEST_DISCOVERY": "1"}, clear=False
    ), patch.object(
        registry, "_get_scan_roots",
