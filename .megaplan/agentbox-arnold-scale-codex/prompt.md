# Codex Review: AgentBox as Arnold-Level Machinery

Working directory: /Users/peteromalley/Documents/megaplan

Task: Think deeply about whether AgentBox is lifting the right concepts out of Megaplan into Arnold-level infrastructure. Megaplan is only one pipeline inside Arnold. The current plan is AgentBox as a persistent machine with Discord Operator, Guardian, operations, worktrees, tmux, Store-backed registry, scheduled checks, credentials, cleanup, and first operation kind megaplan_chain. Review what happens if we later have 100+ Arnold pipelines with Megaplan-like long-running work, supervision, Discord interactions, scheduled check-ins, credentials, worktrees, and cleanup. Should operation registry, host-local runner, scheduled checks, Guardian, Discord resident tools, credential sync, and cleanup be AgentBox-specific, Megaplan-specific, or Arnold-level primitives? Is the scheduled check-in just one instance of a general scheduled-task ability? Return: (1) what should be lifted to Arnold-level, (2) what should stay Megaplan-specific, (3) what should stay AgentBox-specific, (4) 100+ pipeline scaling risks, (5) scheduling/check-in design, (6) concrete doc/epic changes. Be direct and skeptical. Under 1400 words.
...
milestones:
  - label: m1-agentbox-core
    idea: .megaplan/briefs/agentbox-persistent-machine/m1-agentbox-core.md
    profile: partnered-5
    robustness: thorough
    depth: high
    vendor: codex
    branch: agentbox-m1-core
    notes: "Central Store/state/locking design. Highest non-local damage risk because every later milestone depends on the operation model and recovery semantics."

  - label: m2-chain-runner
    idea: .megaplan/briefs/agentbox-persistent-machine/m2-chain-runner.md
    profile: partnered-5
    robustness: full
    depth: high
    vendor: codex
    branch: agentbox-m2-chain-runner
    notes: "Normal-checkout worktree path, host-local tmux runner, and first operation kind. Proves one repo/one chain/one operation before Discord and Guardian build on it."

  - label: m3-discord-thin-path
    idea: .megaplan/briefs/agentbox-persistent-machine/m3-discord-thin-path.md
    profile: partnered-5
    robustness: full
    depth: medium
    vendor: codex
    branch: agentbox-m3-discord-thin-path
    notes: "Discord-first v0: tickets, chain launch, status, logs, and operation id via existing resident runtime."

  - label: m4-guardian-v0
    idea: .megaplan/briefs/agentbox-persistent-machine/m4-guardian-v0.md
    profile: partnered-5
    robustness: thorough
    depth: high
    vendor: codex
    branch: agentbox-m4-guardian-v0
    notes: "Autonomous restart/resume/classify/notify loop with safety boundaries. Reuses cloud supervise and watchdog logic without creating a second daemon."

  - label: m5-credentials-preflight
    idea: .megaplan/briefs/agentbox-persistent-machine/m5-credentials-preflight.md
    profile: partnered-5
    robustness: thorough
    depth: high
    vendor: codex
    branch: agentbox-m5-credentials-preflight
    notes: "Credential sync, redaction, health checks, and launch gates. High-stakes because leakage or false-positive auth checks are security/operational failures."

  - label: m6-completion-github-cleanup
    idea: .megaplan/briefs/agentbox-persistent-machine/m6-completion-github-cleanup.md
    profile: partnered-5
    robustness: full
    depth: medium
    vendor: codex
    branch: agentbox-m6-completion-cleanup
    notes: "Completion DM, PR metadata, cleanup survey, and approval-gated consolidation using cleanup-loose-branches discipline."

  - label: m7-bootstrap-day2
    idea: .megaplan/briefs/agentbox-persistent-machine/m7-bootstrap-day2.md
    profile: partnered-5
    robustness: full
    depth: medium
    vendor: codex
    branch: agentbox-m7-bootstrap-day2
    notes: "Bootstrap, SSH host profile, systemd, doctor/reconcile, backup/restore. Mostly packaging around the already-proven local loop."
...
Overall plan difficulty: 5/5; selected profile: partnered-5; because a bad operation/state model can pass local tests while corrupting later Guardian, Discord, cleanup, and recovery behavior.

## Outcome

Add AgentBox's first-class core inside Megaplan: config/repo registry, operation registry, operation state machine, per-operation event history, optimistic update/locking, and minimal CLI read/write inspection paths.

## Scope

In:

- create an AgentBox module/package structure inside this repo;
- define operation, repo, host/config, and event models;
- add Store-backed current state for AgentBox operations, using existing Megaplan Store patterns rather than a parallel database by default;
- define valid operation states and allowed transitions;
- define operation id generation and run/log/event path generation;
- write per-operation event history and minimal manifest/debug files;
- add lock/revision/idempotency handling for concurrent Guardian/Operator updates;
- add minimal `agentbox status`/registry inspection commands for local debugging.

Out:

- Discord runtime tools;
- Guardian loop;
- credential sync;
- GitHub PR automation;
- cleanup/consolidation;
- remote bootstrap;
- operation kinds beyond enough scaffolding for later `megaplan_chain`.

## Locked Decisions

- AgentBox starts inside the Megaplan repo.
- Operation registry is first-class, not a renamed `cloud_run`.
- Prefer extending existing Store abstractions and DB/file backends.
- Store state is the queryable current-state view used by Guardian and Operator.
- Per-operation `events.ndjson` is replay/debug history, not a peer source of truth that competes with Store.
- Normal canonical checkouts are the v0 repo layout; bare repos are deferred.

## Open Questions

- Exact module placement for AgentBox code.
- Exact Store model names and whether file-store parity is required in this milestone.
- Whether migrations are needed immediately or can follow existing Store extension conventions.
- Final event schema field names.

## Constraints

- Do not create a separate long-lived service or database outside existing Store conventions.
- Do not add destructive cleanup behavior.
- Do not introduce a plugin framework.
- Every state transition must include actor, reason, prior state, next state, timestamp, and idempotency key.
...
- Tests cover event append and path generation.
- Tests cover partial/current-state reconstruction expectations at the model level.
- CLI inspection can show a created operation and recent event history.

## Touchpoints

- `arnold_pipelines/megaplan/store/*`
- resident Store patterns for conversations, scheduled jobs, cloud runs, confirmations, progress events
- new AgentBox module and CLI entrypoints
- `docs/agentbox-persistent-machine-plan.md`

## Anti-Scope

- No Discord bot changes.
- No Guardian autonomous behavior.
- No credential implementation.
- No external project adoption.
- No Codex/Claude/shell/test operation adapters.
