...
pre-merge CI and a mechanically independent attestation before automatic
merge readiness, with the old final gate used only as a post-merge backstop.
Native may rely on its completion manifest only when those records, the new
gate schema, and each required readiness artifact occur as an exact
content-addressed proof row in that manifest. The schema supplies
...
accepted/consolidated Custody M11 + bounded-projection handoff
  -> accepted milestone-gate bootstrap + downstream-spec readiness
...
templates. C1 is the inserted execution of Completion M1; C2 is the inserted
execution of Completion M2. Their preserved Completion proposal, briefs, and
crosswalk remain the normative semantic source while Native hosts them in the
same program frontier. C1/C2 own the experimental neutral completion contracts
and remain non-authoritative. Admission and S2R instantiate runtime
occurrences; S2R owns concrete child sets and aggregation instances, and its
GO-0 transition is the only live enablement. Every later Native gate consumes
the exact kernel receipts and current divergence-ledger hash. S5A/S5B own the complete
...
The authoring surface is intentionally small and omission-proof. An author
declares durable intent and only domain obligations on the canonical `.pype`
surface. Deterministic compiler/admission tooling derives mechanical
obligations, generates and pins the immutable binding, and renders completion
worksheets and Markdown reports as disposable non-authoritative projections.
Static lint and admission reject a durable subject whose declaration or
generated contract is missing. Pure helpers require no completion contract;
their transitive behavior remains in the containing executable digest.
