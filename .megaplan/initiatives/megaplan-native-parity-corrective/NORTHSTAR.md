...
independent review, and manual merge precede its post-merge backstop.
...
C1 owns the experimental `arnold/workflow/completion/` boundary, versioned
schemas/serialization, stable obligation identity, durable/helper lint, one
completion candidate-outcome registry, `superseded_by_named_exit`, shadow
generation, the false-done/`REVIEW` fixture, and one content-addressed
append-only divergence ledger. C2 owns immutable binding/evaluation schemas,
exact evidence scope and proof modes, aggregation signatures, verifier
independence, waiver taint, internal persisted-wire compatibility and decoder
behavior, shadow atomic acceptance integration, introduction-time restore, and
projection deletion/forgery invariance. Neither milestone is authoritative.
