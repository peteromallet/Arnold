...
The merge-HEAD S5B readiness proof map must also contain a Native-owned
`s5b-q49-load-gate-receipt.json` from replaying the captured high-volume
corpus through the exact production-consumer path with pinned latency,
peak-memory, cursor-bounded-read, full-rebuild-parity, and no-full-history-
fallback assertions. Custody owns the projector and upstream benchmark; S5B
owns this consumer load gate. The transition consumes it through
`current_validation`, so it is mechanically impossible to enable the live
path before the gate passes.
...
- Execute at least the captured 57,000-event history through Custody's exact
  ...
  projector, checkpoint, snapshot or invalidation scheme. Emit the blocking
  `s5b-q49-load-gate-receipt.json` into the merge-HEAD readiness proof map
