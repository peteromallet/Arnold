...
The external
`.megaplan/initiatives/megaplan-chain-milestone-gates/chain.yaml` bootstrap is
a launch prerequisite. Native Parity assumes its typed `conformance_gate` runs
before merge eligibility, reruns against merge HEAD, and binds accepted
...
- Close the review's remaining intake questions before freezing evidence:
  validate the exact M11 completion manifest and at least one executed
  capability-probe receipt; generate the exhaustive suffix/carrier
  inventory; inventory the existing pack/lock descriptor owner; capture the
  pre-result DX corpus/environment/numeric baseline; determine whether a real
  Megaplan model-directed durable inner-call consumer exists; and obtain the
  source for the oracle's referenced but absent numbered transitions 2, 3, 5,
  and 7. If that source cannot be obtained, record a content-addressed
  `oracle-source-gap` disposition and prohibit any fixture or claim that relies
  on those missing transitions. Never invent them.
- Freeze and stage the source-suffix migration without selecting it for new
  work. Produce the complete candidate rename/map for every live
  `*.pypeline` workflow, including `workflow.pypeline` and named subworkflows,
  to its target `*.pype` path. Keep the exact pre-cutover `.pypeline` source
  and new-authoring/admission path usable through S2F. S2F implements and
  proves the compiler/linker, then its typed post-validation transition alone
  selects `.pype`.
- Inventory and update every suffix-sensitive compiler/loader branch, file
  discovery/glob, package-data rule, resource path, manifest/lock field,
  source-map carrier, CLI argument/help string, validator/generator,
  conformance/traceability declaration, fixture/test, example/documentation
  reference, and editor/Linguist integration. The inventory is a generated,
  blocking receipt rather than an informal `rg` list.
- Treat the filename/suffix change as executable-identity drift. Prove that
  pre-cutover nonterminal runs resolve their exact pinned `.pypeline` artifact
  or take an accepted typed migration/new-attempt/quarantine path; they may not
  silently resume against the renamed `.pype` source.
- Specify that S2F's receipt-consuming transition rejects `.pypeline` for new
  authoring, compilation, package admission, and certification. Until then,
  the current path remains usable. Any later temporary legacy reader is read-only,
  explicit, expiry-bound, incapable of routing or admission, and covered by a
  deletion owner and negative tests. Historical/archive evidence remains
  immutable and is not rewritten to manufacture proof.
- Freeze `docs/arnold/pype-authoring-contract.md` before compiler cutover.
  Every `.pype` contains exactly one canonical top-level `@workflow`; every
  durable root or child workflow has its own `.pype`. Private local steps and
  pure helpers may remain in that file but cannot cross its boundary or receive
  independent identity. Reusable steps/effects/schemas/policies/helpers live in
  `.py`; `.py @workflow` is preview-only. Cross-file topology imports only the
  canonical workflow without source execution. Dynamic/conditional/star
  imports, re-export laundering, cycles, recursive calls, duplicate identities,
  and ambiguous same-coordinate resolution are stable errors. Legitimate
  package-version digest evolution is not a namespace collision. Bounded-loop constructs are not
  call recursion. The existing package descriptor owns optional default
  selection and freezes one logical workflow through every durable carrier.
  S1 freezes the executable expected-red corpus and identities; S2F implements
  it, and `GO-FORMAT` must pass before S2R.
- Freeze the versioned canonical executable-closure digest algorithm and its
  closed syntactic exclusion list under the adopted IDs
  ...
  Publish content-addressed machine schemas for the IR, envelope, graph lock,
  tagged scalar/reference encodings, and mutation corpus before S2F starts;
  undefined encodings reject. The component digest hashes normalized IR plus
  every statically reachable declared dependency, constant, schema, policy,
  prompt, model/tool binding and private/shared behavior slice. Formatting,
  comments and path provenance may be excluded; implementation-curated
  “semantic” omissions may not. Conservative extra drift is acceptable; silent
  under-digesting is not. Direct dependency contract requirements enter the
  component digest; selected concrete descendant digests enter the separately
  hashed transitive graph lock. Admission/checkpoints/effects/proof bind both.
- Classify executable Python by responsibility. Literal constants, types, and
  digest-bound pure deterministic helpers are allowed; topology, product
  routing, workflow-state mutation, suspension, child creation, direct I/O, and
  effectful/retryable work must use supported constructs or a typed step/effect
  boundary. Freeze diagnostic codes and “extract into a step” rewrites rather
  than silently executing arbitrary top-level or helper code. Preview may emit
  a warning and run only under fresh ephemeral identity and fake/ephemeral-only
  effects with no durable effect history; durable sandbox is a separately
  declared mode, and admitted/certified modes reject before authority.
- Freeze ordinary Python/third-party import freedom for shared `.py` step
  bodies versus static Arnold topology/component resolution. Define exact
  graph-lock/binding fields for distribution/version/artifact, optional
  feature, Python/runtime environment and plugin selection; add import-time
  effect/mutable-registration, ambient dependency, imported-topology and
  undeclared-effect bypass negatives.
- Freeze the common immutable policy authoring/lowering envelope: stable
  kind/schema, recursively immutable canonical values, scope/attachment,
  provenance, precedence/override and digest. Define exact component-versus-
  graph-lock digest placement and reject raw config-map authority, callables,
  open routes, ambient/mutable defaults and hidden overrides.
...
- Require that same validated manifest to bind the exact
  `bounded-incident-projection-handoff.json`: crash-safe incremental checkpoint
  or bounded snapshot-plus-tail behavior, truncation/store-incarnation/restore
  invalidation, concurrent-append correctness, full-rebuild parity, and the
  57,000-event latency/peak-memory receipt. Record it as a reuse-only Native
  launch and S5B input. Custody owns the implementation; S1 must not create a
  Native projector or accept an informal benchmark.
...
- Prove that every authoritative decision-consumption and terminal/arbitration
  ...
  for S2R's two-client contention tests. Application read/check/write,
...
- Inventory the admitted stores, APIs, queries, recovery services, projections,
  action validator, outbox/reconciliation, generic conformance fixtures, and
  every authoritative reader/consumer or operator-facing action preparation
  path. Record them as reuse-only dependencies; create no fallback
  implementation. Writer inventory alone cannot prove a carrier inert.
...
- Freeze `arnold.workflow.event_envelope.v2` and its bidirectional audit
  correlation: run, workflow/step occurrence, loop/rework/retry/reentry
  generation, execution attempt, platform agent session, model/tool/effect,
  trace/span, usage/cost and protected log/transcript artifacts. Every record
  returns to owning occurrence/source and any consuming decision/terminal;
  provider IDs are optional provenance, reverse indexes are rebuildable, and
  logs cannot route or authorize.
...
- Freeze the independent source-oracle, raw-export, audit-normalizer and
  ...
  fixtures; S2R integrates it with production runtime at GO-0.
...
- Freeze an immutable nonvolatile field floor. Every trace/evidence receipt pins
  the exact field-table/allowlist version. An amendment needs a
  content-addressed amendment receipt, independent approval/re-verification and
  replay of every affected dependent receipt; a newer table cannot silently
  validate older evidence.
...
- Freeze `docs/arnold/workflow-execution-mode-dispositions.yaml` as the one
  machine-readable execution-mode/disposition/store registry for `authoring_preview`,
...
- Freeze one enforcement-disposition registry. Every restriction is exactly
  ...
  Together these machine-readable registries are the sole normative mode and
  disposition source. Markdown tables and golden assertions are generated or
  informative renderings and fail validation if they drift.
- Reserve a separate versioned completion candidate-outcome registry for C1.
  The enforcement dispositions above describe claim severity and mode gates;
  completion outcomes describe the semantic result under evaluation. Freeze
  the requirement for one total generated boundary mapping that rejects
  unknown pairs. S1 must not merge the domains, predefine C1's outcome
  semantics, or permit an unversioned local table.
- Freeze the mode-by-capability access matrix for authority/control stores,
  admitted product history/checkpoints, effects/idempotency, proof/evidence,
  artifact blobs, caches and sandbox/preview stores. Classify logical
  capability, credentials, namespace, keys/indexes, retention and discovery,
  not merely the physical service. Sharing a blob backend is permitted only
  through fully isolated non-production capabilities that cannot be discovered
  or promoted as admitted history/evidence.
...
- Freeze generated-manifest schema/version/hash evolution before S2R changes
...
- Freeze named enclosing-loop typed exits and canonical schema-qualified
  ...
  digesting.
- Inventory the named-exit topology, target-loop identity, intervening durable
  scopes and unwind order as input to C1. C1 owns the
  `superseded_by_named_exit` completion/control-terminal definition; S2R owns
  its concrete loop/unwind instance and fresh-instance reentry.
...
- Add extraction disposition to each normative row for S7's content-addressed
  Platformization handoff: core primitive, public candidate, experimental
...
- Compile every route through a separately source-mapped finite route key or
  statically named finite predicate set. Whole-payload, open-string and
  undeclared payload-field discriminants reject even if a trace can attribute
  their value after the fact.

## Semantic gate

- The renamed `.pype` source lowers to the same pre-product-migration semantic
  set as the pinned `.pypeline` baseline; the only accepted difference is the
  explicitly recorded executable/source identity transition.
- An executable target/expected-red corpus freezes exact-one workflow files,
  private local steps/helpers, packages containing multiple separate workflow
  files, canonical imports and aliases, repeated call sites, preview-only `.py`
  workflows, package default/explicit selection, and digest-bound pure helpers.
  It also freezes zero/two workflows, private-member imports, step-to-step/
  workflow calls, transitive helper effects, source execution, dynamic
  `.pype`/topology imports, cycles/recursion/collisions, hidden durable work,
  descriptor disagreement,
  pure file moves, logical renames, and extract/inline migrations. S1 closes
  when the current implementation fails each unimplemented target for the
  expected reason; S2F owns making format target positives pass and negatives
  fail; S2R owns runtime target closure.
...
- S1 does not select `.pype` for new work or reject `.pypeline`; it hands S2F a
  complete staged migration, compatibility inventory, and negative corpus.
- Synthetic missing, extra, red, stale, unbound, unconsumed,
  cross-incarnation, self-certified and pre-proof-map-hash mutations are
  required for the stage and final validators; a validator written by the
  producing milestone cannot exempt its own edge cases.
...
- The staged migration inventory omits any live source, compiler/loader suffix
  branch, package rule, resource path, source map, manifest/lock field, CLI,
  validator, generator, test/example, or editor/tooling integration that still
  treats `.pypeline` as the current authored format; any such current path is
  disabled before S2F's accepted transition; or historical evidence is
  rewritten instead of classified.
- Workflow selection depends on filename or declaration order; a `.pype` has
  zero/multiple workflows; a private local definition crosses files; linking
  executes source; or raw I/O, effects, route ownership, suspension, or
  workflow mutation hides inside an untyped helper instead of producing a
  source-local boundary diagnostic.
