...
C1 is the inserted execution of Completion M1. Native hosts the milestone in
the same program frontier; it does not replace or reinterpret the preserved
Completion proposal, M1 source brief, or supersession crosswalk that own the
neutral semantics.
...
3. Define internally versioned schemas and canonical serialization/hash
   versions from this first kernel milestone for
   ...
   serialization foundation, including the persisted binding envelope/version
   even though C2 completes its fields and authoritative compatibility begins
   only at S2R GO-0. Public API stability remains Platform S6-only.
4. Define stable semantic `obligation_id`, canonical `spec_hash`, tombstoning,
   and exact reuse identity. Executable obligation identity is exactly
   `(spec_hash, obligation_id)`. A semantic obligation ID is never reused for a
   changed meaning, and a human-readable ID or mutable semantic-version counter
   cannot authorize reuse.
5. Implement the mechanical durable-subject versus pure-helper predicate as a
   versioned, machine-evaluable compiler rule and a static omission lint.
   ...
   executable digest. A helper that hides any durable behavior fails at its
   authored and call spans.
...
7. Define one canonical versioned completion candidate-outcome registry,
   including
   ...
   for their total generated boundary mapping. Every Megaplan, product, or
   platform completion registry is a strict generated and tested projection of
   the canonical completion registry; never merge the two semantic axes into
   one enum or allow an unversioned local mapping.
...
9. Generate shadow specs for representative workflows, durable steps, dynamic
   ...
   contracting pure helpers. The canonical authoring surface asks authors only
   for durable intent and domain obligations; deterministic tooling derives
   mechanical obligations and generates/pins machine bindings plus
   human-readable worksheets and Markdown reports. Those renderings are
   disposable non-authoritative projections. Static lint and admission reject
   omitted durable declarations/contracts; pure helpers require no completion
   contract.
10. Reproduce and execute the captured M10/M11 fixture as a blocking golden
    exemplar. Legacy `done` without an accepted
    ...
    evidence for unrelated subjects must remain intact. C2 and every milestone
    beyond shadow are blocked unless at least this false pass is reproduced
    from captured inputs and rejected without manually curated evidence.
...
- executable identity is exactly `(spec_hash, obligation_id)`, semantic IDs
  are stable/non-reused, and persisted schema versions are present from C1;
- the machine-decidable durable/helper lint has positive, omission, and
  hidden-effect negatives;
...
- the reproduced false-pass golden exemplar is a blocking accepted proof row
  for C2 and GO-0, not a narrative regression promise;
