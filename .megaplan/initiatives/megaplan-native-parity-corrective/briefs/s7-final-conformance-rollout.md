# S7 - Native-Topology Conformance on the M11 Proof Framework

## Objective

Extend—not duplicate—the admitted Custody M11 conformance framework with
Native-specific topology and identity proof. Close only when clean checkout,
wheel/sdist, and pinned cloud runtime demonstrate the same authored semantics,
authority decisions, custody ownership, WBC history, and disposable projections.

`../GOLDEN_TRACE_CONTRACT.md` is the human-reviewed required proof-map input.
An independent static source oracle and separately implemented raw-event audit
verifier check Python/runtime equivalence. It is never runtime route or repair
authority.

## Required work

- Create the corrective target's new machine gate:
  `docs/arnold/megaplan-native-parity-conformance.yaml`,
  `docs/arnold/megaplan-native-parity-traceability.yaml`, and
  `scripts/validate_megaplan_native_parity_conformance.py`, consumed with
  `final-proof-map.json` by the chain's final validation hook. Extend compatible
  evidence from the older `native-platform-followup` ledger only through
  explicit reconsumption; do not mutate or treat its `.pypeline` validator/
  ledger as target proof.
- Prove `workflow.pype` and the workflow files it invokes as children are the only live
  authored workflow sources in checkout, wheel/sdist, and cloud. Fail on any
  `.pypeline` source, accepted suffix alias, package-data rule, resource path,
  compiler/loader branch, source map, manifest/lock field, CLI surface,
  validator/generator, current test/example, editor/Linguist mapping, or
  downstream handoff that still treats `.pypeline` as current.
- Permit `.pypeline` only in classified immutable historical/archive evidence
  and, until its recorded expiry, the explicit pinned-artifact legacy reader.
  Prove neither can author, compile, admit, route, resume by alias, or satisfy
  conformance. Delete the reader when no live pinned occurrence requires it.
- Prove the exact-one `.pype` contract in checkout, wheel/sdist, and pinned
  cloud: canonical workflow imports and aliases, packages with multiple
  separate workflow files, private local steps/helpers, shared `.py` steps,
  descriptor-owned optional default and explicit selection, transitive locks,
  and definition/import/call/source maps. Reject zero/two workflows, private
  imports, durable `.py` topology, last-declaration/path selection, cycles/
  collisions/recursion, dynamic/conditional/star imports, source/descriptor
  mismatch, step→step/workflow, helper→step/workflow/effect, and hidden durable
  code before authority. Prove bounded-loop IR is not recursion.
- Prove identity and migration behavior: pure physical moves retain logical and
  executable identity while updating provenance; logical rename, extraction,
  inline, hostability change, private/shared promotion, and behavior drift
  require explicit migration/new-attempt/quarantine. Converter/refactor
  receipts and exact pinned legacy resolution must agree across package forms.
- Prove positive helper behavior: allowed pure helpers retain transitive
  dependency/call-site source maps, deterministic replay, and behavior-relevant
  digest drift; forbidden I/O/effects reached through a helper fail with both
  the originating and call spans.
- Generate exact set equality across authored source nodes/children, lowered
  topology, registered WBC producers, executed runtime node/child attempts,
  authority-increasing actions, semantic checkpoint/reentry coordinates, and
  projection consumers.
- Generate exact set equality across authored typed decision occurrences,
  exactly one accepted Run Authority decision, and consumed runtime
  transitions/actions. Reject orphan, duplicate, unaccepted, stale-fence,
  inferred, outcome-mismatched, or multiply consumed decisions.
- Generate exact completion inventory equality across every admitted durable
  subject, S2F call-site template, runtime occurrence, immutable
  `CompletionBinding`, required evidence obligation, evaluated
  `CompletionVerdict`, accepted Run Authority decision/effect, and terminal
  candidate outcome. Pure helpers and projections have no binding.
- Reconsume the exact C1/C2 manifests, S2R
  `completion-kernel-enablement-receipt.json`, every later Native completion
  receipt, and the final current `completion-divergence-ledger.json` hash. Fail
  on missing, stale, replaced, unresolved-blocking, or cross-incarnation
  evidence.
- Prove no competing completion schema/evaluator/decoder/writer, evidence or
  waiver registry, acceptance transaction, or durable store exists. Rerun the
  neutral-package import lint and adapter-copy/reverse-dependency negatives.
- Re-execute the false-done/`REVIEW` corpus, immutable rework admission,
  verifier-independence, absence/set proof, waiver-taint, named-exit,
  internal-wire reader/writer/decoder, restore/replay, and projection
  deletion/forgery invariance suites in checkout, clean wheel/sdist, and cloud.
- Consume S5B's exact 57,000-event bounded-query receipt and Custody's
  `bounded-incident-projection-handoff.json`; prove no Native query or fixer
  falls back to full-history recomputation. Custody remains the implementation
  and benchmark owner.
- Join every authoritative consumption/arbitration site to the certified
  linearizable conditional operation at the canonical production store/service.
  Require exact production adapter/store/schema provenance and consume the
  owning cutover's two-independent-client contention/crash receipt; serialized
  schedules, read/check/write and local locks cannot pass.
- Reuse M11's persistence, terminal uniqueness, effect ambiguity,
  stale-fence/epoch, transfer/reclaim, recovery, projection rebuild,
  installed-runtime, captured replay, independent verification, and zero-bypass
  fixtures. Add only Native topology mutations/compositions they cannot express.
- Generate row evidence containing row ID, carrier kind, source span/policy,
  semantic identity, all three external identity joins, proof test, executable
  runtime trace, exact contract versions, commit/tree, installed artifact, and
  content hashes.
- Make compatibility/purity/deletion/coupling/installed-runtime failures part
  of the blocking validator result; auxiliary red checks cannot be ignored.
- Reject inherited statuses, whole-file-hash substitution, hand-authored
  evidence, stale manifests, support labels, shadow enforcement, trace-shape
  fixtures without behavior, and receipts/projections presented as authority.
- Run the complete split-outcome suite from clean wheel/sdist and the pinned
  custody-compatible cloud runtime.
- Delete remaining component/handler route tables, compatibility authority
  paths, false-pass evidence generators, and hidden route readers/writers only
  after negative and rollback gates pass.
- Produce `final-proof-map.json` deliberately from accepted proof artifacts;
  the chain's required `final_conformance_gate` must pass and its validation
  receipt must be included in completion evidence.
- Make
  `.megaplan/initiatives/megaplan-native-parity-corrective/platformization-handoff-manifest.json`
  a mandatory named final-proof-map row. The Native completion manifest must
  hash that exact artifact and its schema/content bindings; Platformization's
  `chain_completed + require_manifest` and explicit artifact preconditions
  must feed an S1 intake gate that matches the exact path and current hash to
  one validated manifest proof row.
- Make `completion-kernel-c1-manifest.json`,
  `completion-kernel-c2-manifest.json`,
  `completion-kernel-enablement-receipt.json`, and
  `completion-divergence-ledger.json` mandatory named final-proof-map rows.
  The Platformization handoff binds their exact paths/hashes, schema and
  serialization versions, candidate-outcome registry, total enforcement-
  disposition boundary mapping, decoder/compatibility matrix, proof corpus,
  legacy-writer retirement, S5 vertical-slice receipts, bounded-projection
  receipt, and extraction/coupling inventory.
- Land the S1-planned proof-map plumbing: the chain runner passes the declared
  map to the validator, the validator consumes the complete per-sprint receipt
  registry, and missing/extra/unknown/unconsumed/stale/non-executed/non-commit-
  bound/red receipts are blocking. The validation receipt binds the proof-map
  hash before its own receipt is appended.
- Run synthetic missing, extra, red, stale, unbound, unconsumed,
  cross-incarnation, self-certified and pre-proof-map-hash mutations against
  both the stage-gate and final validators. The validator producer cannot be
  its sole verifier.
- Run the retained old current-tree ledger/evidence bundle as a negative fixture;
  it must fail even after all path/hash records are refreshed.
- Execute all six golden scenario families and every mandatory mutation against
  checkout, clean wheel/sdist, and pinned cloud. Compare normalized
  ordered/partial-order and multiset traces from the same composed run history;
  row-local or cross-run stitched evidence fails.
- Export raw primary-store events and prove raw event-ID/multiplicity equality
  before normalization. Run an audit normalizer/verifier with disjoint code
  provenance from production lowering/runtime trace adaptation; only the
  golden contract's versioned volatile-field allowlist may elide a field and
  unknown fields reject. Pin the immutable nonvolatile-floor digest and exact
  table version. A table/allowlist amendment requires a content-addressed
  amendment receipt, independent re-verification and invalidation/replay of
  every affected dependent receipt.
- Mutate arbitration/consumption fields that must never be volatile: semantic
  key, participant/result vocabulary, accepted and loser identities, CAS
  sequence, precedence and store incarnation/high-water cursor. Elision,
  relabeling or reclassification of any such field fails.
- Run six future-extension mutations: new gate outcome, dynamic review lens,
  retry policy, human decision, override, and external effect. Each must be
  expressible at the Python topology or declared call-site policy/effect
  vocabulary with mechanically regenerated bindings. Handler/auto/metadata-only
  additions must fail or remain inert.
- Enforce structural readability/edit locality: one topology representation;
  one call to one authored reusable delivery cycle from every entry route; the
  small durable primitive set; generated mechanical identity/control-plane
  bindings; local policy; closed vocabulary/exhaustiveness; and at most two
  manual authoritative declarations per extension.
- Apply an advisory complexity budget: target at most three nested control
  levels, roughly 120 nonblank semantic lines for the top-level and 80 per named
  subworkflow, with no embedded route/interface/policy dictionaries. Exceptions
  require a reviewed readability receipt and AST/IR proof of no duplicated
  route/effect/policy authority; the structural conditions, not line counts,
  are binding.
- Run the Native composed-history explanation/repair preflight for every golden
  family; delete/rebuild it and prove identical behavior and causal answers.
- Run deterministic-subset mutations covering ambient time/randomness, process/
  environment state, unordered traversal, mutable globals, reflection/dynamic
  import/eval, unmanaged concurrency, direct I/O, and exception-driven routing.
  Replay twice with identical recorded boundary results and require identical
  semantic, decision, and checkpoint traces.
- Assert stable source-local diagnostic code, authored file/span, semantic path,
  violated rule, and supported rewrite for illegal `break`/`continue`, dynamic
  policy, hidden handler route, ambient I/O/nondeterminism, and unhandled closed
  outcome. Runtime/replay failures must preserve the same authored location.
- Compare every NP-GT family, including A/B/C, through the in-process authoring
  harness and installed execution under identical recorded boundaries. The
  harness must use the production lowerer/transition semantics; normalized
  lifecycle/admission traces must match within the declared virtual-time and
  wall-latency budget, and local success cannot satisfy custody-adoption gates.
- Run `NP-DX-001` edited-step repeat, `NP-DX-002` changed-code resume
  rejection, `NP-DX-003` durable-sandbox fork, and `NP-DX-004` unsupported
  preview through every public
  `authoring_preview`, `durable_sandbox`, `comparison`, `admitted_production`
  and `certification` entry. Edited working-tree repetitions must be easy and
  successful as fresh experiments; changed-code resume of the admitted source
  occurrence must reject before body/effect intent. A durable fork preserves
  immutable source-boundary provenance but receives new sandbox authority, WBC,
  Custody, history and sandbox effect/idempotency identity. Unsupported preview
  may produce functional output but no durable or certification claim.
- Attempt every mode crossing: preview/sandbox/comparison relabel or promotion,
  source-history append, production effect/key reuse, comparison resume and
  local success substituted for certification. All fail. Equal candidate and
  admitted content digests do not make comparison history authoritative.
- Validate every diagnostic against S1's disposition registry across all five
  modes. `always_hard` cannot downgrade; `automatic` rules require no manual
  migration/version ceremony for fresh experiments; `production_admission_gate`
  and `stable_publication_gate` block only their distinct claims;
  `authoring_advisory` rules do not block preview or
  sandbox iteration; and `non_durable_only` never emits durable proof.
- Mutate prompt content without renaming it, tool schema, model parameters,
  exhausted budgets, and cache contents; inject a crash after durable LLM/tool
  result before checkpoint. Reject drift/forgery, preserve durable budget
  consumption, and never repeat the completed call on replay.
- Mutate checkpoints with oversized inline data, mutable-path substitution,
  missing/expired artifacts, digest mismatch, and incompatible schema. Each
  fails into the declared repair/migration/quarantine disposition.
- Prove exact v1 artifact resolution for suspended runs after v2 deploy and
  reject premature garbage collection across checkout, wheel/sdist, and cloud.
- Prove namespace isolation for repeated delivery generations, same-kind fanout
  siblings, and concurrent runs with identical product IDs.
- Mutate Plan Contract and generated manifest/lock metadata; neither may add a
  product route or satisfy the action envelope.
- Mutate generated-manifest schema/version/hash and exercise old/new workers
  against old/new manifests. Unsupported or ambiguous combinations reject
  before body/effect intent through the declared compatibility/migration path.
- Mutate normalized product/Plan Contract `provides`, `assumes`,
  `pre_existing`, interface path/signature, and excluded presentation fields.
  Evidence-affecting edits must pin/migrate/quarantine; presentation-only edits
  must not cause false drift, and no edit may waive evidence mid-run.
- Run named enclosing-loop exit, canonical decision-value, frozen fanout,
  keyed-reducer completion-order, declared phase-error, typed reconfigure, and
  agentic inner-call protocol mutations. Reject sentinel/exception/payload route
  smuggling, mutable sibling context, ambient config, and unjournaled tools.
- Prove comparison namespace exclusion/non-resumability/non-promotion and the
  per-cutover all-plane writer invariant. Inject unregistered legacy and
  candidate writers; both fail before body/effect intent. Consume—not
  reimplement—the M11 backup-restore fence/epoch and repair-revalidation
  fixtures.
- Forge, omit and relabel comparison admission tokens/provenance and change
  governed producer/query registry versions. Comparison history must remain
  excluded, while valid versioned producer registration remains atomic and
  compatible with the pinned M11 platform-contract identity.
- Consume the durable-record ownership/restore matrix for every Native-added
  loop ledger, decision-consumption join, comparison registry and proof
  registry. Each is inside M11's rollback-resistant boundary or passes its own
  restore-then-replay mutation; a Native side authority-consumption store fails.
- Require the restore receipt at the record's introduction/authority cut, not
  first at S7. Every per-milestone proof-registry receipt binds canonical store
  incarnation/restore generation and raw-history high-water cursor so rollback,
  truncation or cross-incarnation stitching fails.
- Derive all lowered-IR arbitration sites/participant families and require
  exact equality with the owning cutover's versioned policy and forced-race
  receipts.
- Enforce the auto scheduler allowlist and canonical repair-request
  revalidation. Mutate retry/escalation/cap/cost/stall/model/config/resume/
  terminal tables and forged/stale request preconditions.
- Name and mutate `_core/workflow_data.py:WORKFLOW` and
  `_ROBUSTNESS_OVERRIDES` across every supported robustness level; after cutover
  they are absent or inert on every runtime/auto/CLI entry path.
- Attribute every golden route divergence to a declared outcome/decision value;
  non-vocabulary payload mutations cannot alter routing.
- Require zero diagnostic codes without a supported primitive/example or an
  explicit deliberate-non-support boundary recipe. Run the S1-defined timed
  ten-task author simulation as a blocking readability receipt.
- Run every NP-GT family including A/B/C through local and installed execution
  with identical recorded boundaries. Normalized lifecycle/admission traces are
  identical and meet the declared virtual-time and wall-latency budget; local
  success remains insufficient for release.
- Prove every S3A/S3B/S4/S5B typed outgoing seam has expired or is structurally
  route-inert, and emit a content-addressed Native-to-Platformization handoff
  manifest: reusable candidates/dependencies, exact typed contract snapshots,
  the exact `pype-authoring-contract.md` version, compiler/diagnostic/converter/
  minimal-preview surfaces, accepted `GO-FORMAT` and source/package/identity/
  legacy receipts, golden adapters, zero-Megaplan-import proof for generic
  primitives, exact completion-kernel implementation/receipts and final
  divergence-ledger hash, completion candidate-outcome registry and distinct
  platform enforcement-disposition mapping, internal persisted-wire/decoder
  promises, false-done/`REVIEW` and bounded-projection proofs, legacy completion
  writer retirement, coupling, exclusions and executed extraction
  classification.
  Distinguish what S2B productizes from what Native already proves. Do not
  extract patterns inside S7 or let Platformization regenerate product
  migration proof; Platformization owns neutral extraction/recomposition from
  this exact handoff.

## Semantic gate

- Canonical source discovery, package contents, manifests/locks, source maps,
  CLI/tooling, and the Platformization handoff agree on `.pype`; new
  `.pypeline` input is deterministically rejected and cannot pass through an
  alternate loader or compatibility path.
- The selected logical workflow and canonical import/lock graph are identical
  across checkout, wheel/sdist, and cloud. Declaration/resource ordering and
  pure moves are identity-inert; logical or behavioral changes take the
  explicit compatibility/migration/new-attempt/quarantine path.
- Source/lowering/runtime topology and dynamic child sets are equal; every
  route, loop, fanout/reducer, suspension/reentry, call-site policy, and
  terminal outcome is source-determined.
- Authored decision occurrences, accepted Run Authority decision IDs/outcomes/
  CAS sequences, and consumed runtime transitions/actions are exactly equal.
- Installed and cloud execution match checkout topology and split-outcome
  behavior.
- Zero hidden product-route authority remains in components, handlers,
  runtime maps, `_core`, compatibility/native-program projections, auto, or CLI.
- All golden families preserve same-run order, partial-order sibling freedom,
  occurrence multiplicity, exact joins, effects, and one terminal across all
  three install forms.
- Exact completion inventory equality, no competing completion writer,
  cumulative neutral import lint, final divergence-ledger disposition, S2R
  internal-wire compatibility, and projection/restore invariance are green.
- All six extension mutations pass at the intended edit point and fail from
  handler/auto/metadata-only bypasses; the one-topology/readability contract
  has a reviewed acceptance receipt.
- Determinism, diagnostics/source maps, local-harness equivalence, LLM/tool
  replay/budgets/cache, checkpoint payloads, pin retention, namespace isolation,
  and contract-stack non-authority all have blocking accepted receipts.
- The five execution modes and enforcement dispositions pass `NP-DX-001`
  through `NP-DX-004`; edited-step iteration and explicit forks are low-friction
  while every unsafe promotion, identity reuse and silent changed-code resume
  remains blocked.
- Addressed exits, canonical decisions, frozen/keyed fanout, declared errors,
  typed reconfigure, agentic protocol, product-contract pinning, comparison
  provenance, all-plane writer proof, scheduler/repair boundaries, concrete
  legacy-table inertness, and all three measurable ergonomics gates are green.
- Production-store CAS provenance/contention, governed registry evolution,
  manifest mixed-worker compatibility, receipt incarnation/high-water binding,
  comparison-token forgery and volatile-arbitration-field mutations are green.

## Custody-adoption gate

- Every authoritative runtime action is covered by the admitted validator in
  enforce mode with current independent fence and epoch, exact target, and
  required WBC boundary evidence.
- The final proof consumes the pinned M11 conformance receipt and proof map;
  neither is copied, reimplemented, or replaced by Native-only status.
- The final validator demonstrably consumed `final-proof-map.json`; existence
  and hashing without semantic consumption cannot pass.
- Projection/evidence deletion, rebuild, forgery, and replay cannot increase
  authority; WBC remains durable history, never a grant or lease.

## Minimum blocking regressions

1. Source-to-lowered semantic preservation, including dynamic fanout.
2. Authored/lowered/runtime/WBC-producer set equality, plus typed decision
   occurrence/accepted-RA-decision/consumed-transition equality.
3. Missing/stale grant, fence, lease, and epoch enforcement for every action.
4. Evidence/projection cannot authorize dispatch, resume, retry, completion,
   cancellation, publication, or delivery.
5. Exact WBC lifecycle and terminal uniqueness for every node/child.
6. Task ID + batch identity + item path child coordinates.
7. Process-death suspension/resume at the same semantic point.
8. Identity preservation through cross-host transfer/reclaim.
9. Legacy route mutation cannot alter canonical behavior.
10. Deterministic projection rebuild and zero projection authority.
11. Checkout/wheel/sdist/cloud topology and trace equivalence.
12. Pinned M11 receipt plus Native topology proof; no auto-publish/status/hash
    substitute.
13. `NP-GT-001` through `NP-GT-006` (including A/B/C) same-run ordered/multiset
    equivalence across checkout/wheel/cloud.
14. Heterogeneous stale-worker rejection before body/effect intent.
15. GO-2 covers every external-effect protocol class with direct
    crash/cross-host reconciliation proof or an independently verified
    equivalence record; a weaker proxy cannot authorize another class. S5B
    consumes the exact receipt, the old writer is reader/use-def inert, and no
    dual-write interval exists.
16. Closed cancel/publish/deliver/terminal CAS arbitration.
17. Six future-extension edit-locality mutations and structural readability.
18. Rebuildable Native composed-history explanation/preflight is causally
    complete, request-only, and behaviorally inert.
19. Deterministic Python allow/deny enforcement and source-local diagnostics;
    replay with recorded boundaries is trace-identical.
20. Local authoring harness uses production lowering and normalizes to installed
    traces without becoming release authority.
21. LLM/tool prompt/model/schema/budget/cache/result identity prevents duplicate
    replay and budget reset.
22. Checkpoint inline bounds and immutable artifact-reference validation.
23. Suspended-version exact resolution and no premature pinned-artifact GC.
24. State/checkpoint/artifact/effect/cache namespace isolation.
25. Plan Contract and generated manifest/lock remain non-routing and
    non-authoritative.
26. Product/Plan Contract digest pinning and evidence-obligation drift.
27. Named enclosing-loop exits and no sentinel/exception/payload routing.
28. Canonical decision values, frozen fanout bindings, and completion-order-
    independent keyed reducers.
29. Closed typed phase errors, typed reconfiguration, and the S1 disposition
    for agentic inner calls: either a proved durable implementation with no
    outer route hints or deterministic rejection plus experimental handoff.
30. Quarantined comparison provenance and one admitted writer behind the shared
    validator across every execution plane/cutover.
31. Reused M11 restore-resistant fence/epoch and repair-revalidation proofs.
32. Auto scheduler allowlist and concrete `workflow_data.py`/robustness-table
    inertness.
33. No payload smuggling, zero undispositioned diagnostics plus timed author
    tasks, and every-family local/installed trace and latency equivalence.
34. Named-exit target closure/intermediate unwind/fresh-instance reentry;
    attempt-terminal/retry/aggregate-terminal separation; declared
    cancellation-pending-reconciliation obligations.
35. Per-application migration decisions, duplicate-human rejected-late facts,
    conditional agentic budget reserves and repair redispatch/invalidation
    classes.
36. Raw-event independent verification, lowered arbitration-site equality,
    Native durable-record restore ownership, typed seam expiry and the
    content-addressed Platformization handoff manifest.
37. Certified production-store CAS contention/provenance; governed registry
    and manifest evolution; proof-receipt incarnation/high-water binding;
    comparison-token and volatile-arbitration-field mutations.
38. Mode/disposition conformance: edit/repeat/fork succeeds with fresh isolated
    identity and provenance; changed-code resume, preview/comparison promotion,
    admitted-history append, production effect-key reuse and diagnostic
    downgrade fail across checkout, installed package and cloud.
39. `.pype` suffix conformance: canonical and named sources, discovery,
    compiler/loader, package data/resources, manifests/locks, source maps,
    CLI/help, validators/generators, tests/examples, editor/Linguist support,
    and the Platformization handoff agree; no live `.pypeline` admission path
    remains.
40. `.pype` authoring/identity conformance: exact-one files, canonical imports,
    private/local and shared/`.py` boundaries, package default/explicit
    selection, aliases, pure-helper provenance/digest drift, transitive locks,
    source/package correspondence, preview-only `.py` workflows, physical
    moves, logical migrations, converter/refactor receipts and cross-file
    diagnostics agree across checkout/wheel/cloud. Zero/two workflows, private
    imports, step/helper leaf-law violations, cycles/recursion/dynamic
    `.pype`/topology imports, descriptor mismatch, transitive forbidden I/O and
    hidden durable Python fail closed; no downstream carrier may change the
    frozen logical workflow.
41. Completion-kernel conformance: C1/C2 remain non-authoritative until the
    exact S2R GO-0 transition; every later cutover consumes that receipt and
    current divergence-ledger hash; every primitive has one total aggregation
    instance; false-done/`REVIEW`, immutable rework admission, waiver taint,
    proof modes, decoder compatibility, restore/projection invariance, no
    duplicate writer, and 57k bounded-query consumption pass.
42. Platform handoff conformance: the exact Native completion implementation
    and proof corpus are content-addressed for neutral extraction, while stable
    public authoring/API publication remains Platform S6-only.

## Do not close if

- The final report says implemented while any blocking check is red.
- The final chain still points at
  `validate_native_representation_conformance.py` or the older
  `megaplan-native-representation-{conformance,traceability}.yaml` baseline
  instead of the corrective target gate.
- The proof generator begins from rows already labeled implemented.
- A clean installed end-to-end workflow has not actually executed.
- Any current source/tool/package/proof/handoff surface still accepts or emits
  `.pypeline`, or a legacy reader can do more than resolve an exact pinned
  pre-cutover artifact.
- A workflow is inferred from declaration order/path, discovery executes author
  code, a `.pype` has zero/multiple workflows or exposes a private member, or
  durable/effectful work can execute outside a typed step/effect boundary.
- The validator did not consume the declared proof map or its pre-receipt hash
  is absent from the validation receipt.
- The refreshed old self-declared/hash-only ledger passes.
- The golden oracle or composed explanation is used to choose a runtime route,
  or any golden trace is stitched from different runs.
- An ordinary extension requires a handler/auto/metadata route addition or
  multiple manually synchronized identity/control-plane registries.
- Ambient nondeterminism, a forged cache, invalid checkpoint reference, missing
  pinned executable, or namespace collision can pass or silently recompute.
- CAS proof relies on serialized/local execution, a receipt can cross store
  incarnations or omit its history cursor, or registry/manifest evolution can
  admit an unsupported worker or comparison producer.
- A comparison record becomes admitted, an unregistered plane executes, a
  product-contract edit waives evidence, or any measurable ergonomics gate is
  advisory/red.
- A C1/C2 record is authoritative before S2R GO-0, a later gate omits the
  current divergence-ledger hash, a legacy completion writer remains live, the
  two disposition registries collapse, or public API stability is claimed
  before Platform S6.
- Any runner infers or silently changes mode, fresh experiment identity requires
  production migration ceremony, or preview/sandbox/comparison can produce an
  admitted/certification claim or reuse production effect/history identity.
