...
completion_obligation_id: "artifact.landed-and-validated"
...
completion_evidence_window:
  subject_id: "fixture-token:subject-v1"
  occurrence_id: "fixture-token:occurrence-v1"
  attempt_id: "fixture-token:attempt-v1"
  generation_id: 0
  source_digest: "fixture-token:source-v1"
  runtime_digest: "fixture-token:runtime-v1"
  dependency_lock_digest: "fixture-token:lock-v1"
  store_id: "fixture-token:store-v1"
  store_incarnation: "fixture-token:store-incarnation-v1"
  restore_generation: 0
  cursor_start_exclusive: 0
  cursor_end_inclusive: 420
  custody_target: "fixture-token:custody-target-v1"
  custody_epoch: 3
  authority_fence: 17
  wbc_version: "fixture-token:wbc-m11"
  admitted_child_set_digest: "fixture-token:children-v1"
...
50. Executable obligation identity is exactly
    `(completion_spec_hash, completion_obligation_id)`. Semantic obligation IDs
    are stable and never reused for changed meaning; changing either tuple
    coordinate invalidates reuse.
51. Candidate outcome is proposed and selected before its applicable
    obligations are evaluated. `blocked` and `waived` carry their typed proof;
    quarantine remains nonterminal unless an independently admitted terminal
    policy explicitly permits and proves a terminal disposition.
52. Presence, complete-capture absence, set equality and aggregation remain
    distinct proof modes. Absence is `unknown` without a complete-capture
    receipt over the normative evidence-window tuple.
53. Every parent aggregation has a total mapping for each child disposition,
    propagates waiver taint transitively, preserves admitted multiplicity and
    rejects double contribution of one evidence item or child terminal.
54. Verifier independence is proved by implementation provenance, producer
    identity, trust class/domain and direct primary-evidence access, never by a
    caller label or wrapper around the producer.
55. One canonical completion candidate registry exists; every
    product/platform completion registry is a strict generated and tested
    projection. The distinct enforcement-disposition registry remains its own
    canonical semantic axis and their total generated mapping rejects unknown
    pairs.
56. The neutral completion kernel imports neither Megaplan nor Arnold product
    policy. Product adapters point inward from product packages and import lint
    rejects reverse dependencies or copied semantic adapters.
57. Authors declare durable intent plus domain obligations once; deterministic
    tooling generates mechanical obligations, pins bindings and renders
    disposable worksheets/Markdown. Admission/lint reject a missing durable
    contract. Pure helpers require no contract or binding.
...
| Delete every generated completion worksheet/Markdown/status projection, restart and rebuild | Canonical binding/verdict/decision/effect identity is unchanged and no new action occurs |
| Reuse an obligation ID under a changed spec hash or changed semantic meaning | `(spec_hash, obligation_id)` identity mismatch/tombstone rejects reuse |
| Evaluate obligations before selecting the candidate, omit typed blocked/waived proof, or terminalize quarantine without admitted policy | Candidate evaluation rejects |
| Claim absence without complete capture over the normative evidence window | Result remains `unknown`; it cannot satisfy completion |
| Omit a child disposition, drop waiver taint, collapse multiplicity or count one item twice | Concrete aggregation fails at compile/admission or evaluation |
| Relabel the same producer/verifier code through another wrapper or trust label | Verifier-independence proof fails |
| Restore otherwise identical evidence under another store incarnation | Evidence window and restore drill reject cross-incarnation reuse |
| Add a product-local completion outcome or adapter copy not generated from the canonical registry | Registry/import lint fails |
| Add a durable step without intent/contract or add a pure helper with an independent binding | Static/admission lint rejects the durable omission and the helper binding |
