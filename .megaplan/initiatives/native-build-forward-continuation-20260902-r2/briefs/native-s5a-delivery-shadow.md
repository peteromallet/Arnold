...
Prove the full delivery/review topology and every external-effect protocol class without live writes.

## Scope

Build execute DAG/batches, approvals, review fanout/reducer, rework loop, infrastructure retry, cap/human outcomes in non-authoritative shadow; inventory and prove each effect class.

## Locked decisions

- The controlling milestone definition is `docs/arnold/native-megaplan-build-forward-plan-2026-08-24.md` §5, **Native S5A — Delivery shadow and per-effect-class GO-2 proof**, as qualified by its `Revision 2026-08-24 (post-reconciliation)`.
- Native representation obligations remain the row-level contract in `docs/arnold/megaplan-native-representation-alignment-plan.md` §Traceability Matrix. This milestone owns or affects: **Execute batches/gates; execute-review loop; review fanout/caps; behavior parity; D7–D10**. An `enabled` row is substrate, never implemented conformance.
...
- Does each claimed equivalent effect class share identical idempotency, ambiguity, reconciliation, and compensation semantics?
...
- Zero live writes. Effect-class inventory equality is mandatory. Missing protocol class blocks S5B. Evidence distinguishes intent, outcome, ambiguity, reconcile, and compensation.
...
- D7–D10 goldens, DAG partial resume, races, ambiguity reconciliation, dry-run comparison, effect-class equality, GO-2 receipt, and zero-live-write proof.

## Touchpoints

- arnold_pipelines/megaplan/execute/; arnold_pipelines/megaplan/handlers/execute.py; arnold_pipelines/megaplan/handlers/review.py
...
- No dual write, incomplete inventory, test-only effect path, or narrative equivalence.
