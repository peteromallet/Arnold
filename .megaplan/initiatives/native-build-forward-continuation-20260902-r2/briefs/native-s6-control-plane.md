# Native S6 — Override, recovery, auto-drive, and projection adoption

## Outcome

Move product control-plane meaning into source/policy and demote every observer to read-only projection.
...
- Native representation obligations remain the row-level contract in `docs/arnold/megaplan-native-representation-alignment-plan.md` §Traceability Matrix. This milestone owns or affects: **Override full surface; auto-drive/liveness; handler purity; behavior parity; D11–D13, D15**. An `enabled` row is substrate, never implemented conformance.
...
- Which mechanics qualify for later Platformization without importing maintenance policy?
...
- Status, watchdog, auditor, and projections are powerless. Repair revalidates current authority. Reconfiguration is typed and evented.
...
- arnold_pipelines/megaplan/handlers/override.py; auto/control/status/watchdog/auditor surfaces
...
- No maintenance-domain route policy, observer mutation capability, state-derived next step, or hidden override dispatch.
