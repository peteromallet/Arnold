...
- The native descendant therefore completed roughly 36 seconds after the fixed parent deadline. The parent killed only the CLI wrapper, returned a terminal failure, and discarded the descendant's late valid output. The request prompt was about 83 KiB and below the configured 700,000-character pre-dispatch cap; there is no evidence of prompt-limit starvation.
