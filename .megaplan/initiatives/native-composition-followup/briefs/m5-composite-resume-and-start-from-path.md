...
- `tests/arnold/pipelines/megaplan/`
  ...
  Include execute-gate resume scenarios for approve, deny, cancel, resume,
  bare no-review-to-done, deferred-human verification, and protected action
  approval through declared suspension coordinates.
...
- Matrix rows owned or affected: Prep clarification gate; Human decision/suspension; Execute approval/no-review/deferred-human gates; Execute/review/rework loop; Path-addressed checkpoints; Auto-drive/event/liveness transitions.
- Expected status change: suspension/resume rows gain implementation proof on the file-backed/native substrate; DB-backed durability remains platform-owned.
- Proof artifacts: kill/suspend/resume tests for child workflows and loops, Megaplan resume tests for critique/revise/tiebreaker/execute/review/human paths, execute-gate resume tests for approve/deny/cancel/no-review/deferred-human/protected action paths, legacy cursor compatibility tests, start-from-path fixture, replay-consistency CI.
- False-pass guard: resume must start from persisted path coordinates, not handler-local fields or implicit `current_state` guesses.
- Deferrals: DB-backed resume, durable human wait reattach, cancellation, and reconcile-on-resume are platform M1/M4/M5.
- Canonical source paths/imports: every migrated Megaplan cursor surface must route through `arnold.pipeline.resume` or fail closed with a migration diagnostic.
