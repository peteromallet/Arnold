...
```bash
python -m arnold.pipelines.megaplan init /Users/peteromalley/Documents/megaplan/.megaplan/initiatives/native-python-pipelines-execution-plan.md \
  --profile partnered-5 \
  --robustness thorough \
  --depth high \
  --vendor codex \
  ...
  --prep-direction "Treat this as epic milestone 1 only: design and implement the native foundation behind a feature flag. Scope to decorators, compiler/briefs/state-machine skeleton, checkpoint/frame-stack cursor, runtime dispatch loop, contract bridge, graph projection, and a test-only proving pipeline. Do not convert live Megaplan. Explicitly resolve the first real pilot target divergence: evaluate vibecomfy_executor versus folder_audit/jokes and record the chosen next milestone handoff."
```
