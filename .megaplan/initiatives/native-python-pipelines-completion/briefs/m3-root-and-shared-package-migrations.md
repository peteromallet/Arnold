...
- Matrix rows affected: Plan artifact/version metadata; Canonical source path reconciliation; Behavior parity with existing Megaplan.
- Expected status change: substrate `enabled` by proving the native-first contract works across multiple real packages before canonical Megaplan M3.5.
- Proof artifacts: package contract tests, validator-clean package assertions, native-truth runtime tests, and explicit private-baseline inventory for any remaining graph builders.
- False-pass guard: a package can look structurally migrated while still executing through split-brain graph builders or stale imports.
- Deferrals: canonical Megaplan flagship behavior remains M3.5; final trace/golden cleanup remains M5; report-level composition conformance remains the composition epic.
- Canonical paths/imports: every migrated package must record its final package import path and any intentionally private legacy baseline path.
