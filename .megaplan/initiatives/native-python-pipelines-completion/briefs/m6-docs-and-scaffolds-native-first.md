...
- Matrix rows affected: Source readability; Canonical source path reconciliation; Golden trace regeneration guard.
- Expected status change: substrate `enabled` for native-first docs/scaffolds only. This milestone must avoid overclaiming the later composition end state.
- Proof artifacts: doc search results for graph-era guidance, scaffold generation tests, generated-doc diff, and links to the composition follow-up for nested workflows.
- False-pass guard: teaching a flat native stage list as the final model would conflict with the report even if graph-era language is gone.
- Doctrine gate: docs must explain that native-first `Pipeline.native_program`
  packages are transitional dispatch substrate. The final Megaplan authoring
  direction is compositional workflow source compiled to runtime manifests, not
  hand-built graphs, flat native lists, or native shell projection alone.
- Deferrals: workflow nesting, tree traces, path resume, stable unit IDs, dynamic fanout, and handler-purity conformance remain owned by composition M0-M6.
- Canonical paths/imports: docs must name current public import paths and clearly mark compatibility paths as deprecated/internal where retained.
