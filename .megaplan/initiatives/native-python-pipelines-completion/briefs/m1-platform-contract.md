...
- Matrix rows affected: Plan artifact/version metadata; Path-addressed checkpoints; Canonical source path reconciliation; Behavior parity with existing Megaplan.
- Expected status change: substrate `enabled` only. This milestone creates native execution, resume, registry, and projection truth; it does not claim Megaplan report conformance.
- Proof artifacts: executor-selection tests, registry/validator tests, resume-through-native tests, projected-shell `native_program` assertions, and CLI describe/run coverage.
- False-pass guard: a non-null `native_program` is insufficient unless the projected shell points at the actual canonical source/import surface users run.
- Doctrine gate: `Pipeline.native_program` is a dispatch and compatibility
  substrate only. M1 must not describe native shell projection, route labels, or
  native traces as final Megaplan authoring conformance.
- Deferrals: visible Megaplan product control flow remains owned by native composition M1/M6; durable human waits remain owned by platform M4/M6.
- Canonical paths/imports: record the live import surfaces touched here, including both `arnold.*` and any surviving `arnold_pipelines.*` compatibility paths.
