...
**M2 is the observation/coherence substrate — it makes failures visible and fail-closed, it does NOT repair, rebind, resume, or prove recovery.** Those capabilities live in M3b (delivery-tail closure: exact-commit publication, immutable runtime promotion, atomic CAS rebind of manifest/marker/chain/seed, same-occurrence resume) and M4 (dispatch: operational classifiers + fenced fixer requests). Do not over-claim M2's reach in handoffs or review evidence: M2 detects disagreement and drift; it does not cure the underlying defect families. In particular, the m1-era immediate engine fixes — the multi-wave accepted-attempt projection fix (`5768774d20`/1a062605b2) and the `_plan_state_from_payload` NameError fix (`8eed682d09`) — are immediate engine fixes with focused regression tests, NOT proofs that M2's shadow substrate cured those defect families; keep them credited as engine fixes in the evidence record.
...
- **Multi-wave WBC retention regression (protects the F2 no_accepted_attempt class)**: a multi-batch execute whose accepted attempts span MULTIPLE WBC batch waves must retain EVERY accepted subject — replay/read of the accepted-attempt projection across all waves returns the full accepted set with zero unexplained shadow divergence (no wave's accepted attempts dropped, shadowed, or re-classified as no_accepted_attempt). This is a regression criterion that must PASS on every M2 milestone; it detects (does not fix) the legacy collector's multi-wave drop.
