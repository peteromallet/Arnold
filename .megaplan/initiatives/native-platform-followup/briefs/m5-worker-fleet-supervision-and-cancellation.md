...
- Matrix rows owned or affected: Dependency-aware execute batches; Review infrastructure retry and cap outcomes; Timeout/deadline policy; Auto-drive/event/liveness transitions.
- Expected status change: platform `enabled` for liveness, cancellation, and worker supervision around the already-visible workflow.
- Proof artifacts: lease claim/takeover tests, double-run prevention, cancellation tests, stuck-run escalation fixtures, poison quarantine tests, progress-signal tests, and capacity gate tests.
- False-pass guard: supervision cannot infer progress solely from hidden stage mutation or collapse cancellation into opaque handler abort logic.
- Deferrals: final production conformance and Megaplan native-representation preservation check are M6.
- Canonical paths/imports: worker status and cancellation must report stable workflow paths from the composition trace/audit layer.
