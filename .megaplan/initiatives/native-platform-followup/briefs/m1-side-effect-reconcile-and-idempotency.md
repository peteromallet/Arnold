...
completed through M7 and
`.megaplan/initiatives/native-composition-followup/chain.yaml` has completed
through M6.
...
- Matrix rows owned or affected: Dependency-aware execute batches; Auto-drive/event/liveness transitions; Behavior parity with existing Megaplan.
- Expected status change: platform `enabled` for side-effect safety; it must preserve composition-owned visible routes rather than replacing them with hidden reconcile decisions.
- Proof artifacts: idempotency-key tests, reconcile-action table, interrupted git/file resume fixtures, composite path reconcile tests, and fail-closed diagnostics.
- False-pass guard: a successful resume that guesses worktree state or silently mutates unknown user changes is not valid proof.
- Deferrals: broker approval gates are M2; DB-backed durable storage is M4; worker leases and cancellation are M5.
- Canonical paths/imports: reconcile hooks must operate on path/attempt metadata emitted by the composition runtime, not Megaplan handler-local fields.
