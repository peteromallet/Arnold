...
- Matrix rows owned or affected: Prep clarification gate; Human decision/suspension; Execute approval/no-review/deferred-human gates; Review infrastructure retry and cap outcomes; Timeout/deadline policy; Auto-drive/event/liveness transitions; Path-addressed checkpoints; Gate flag/debt/fallback handling.
- Expected status change: platform `enabled` for durable storage/persistence; report semantics must remain in workflow structure or declared policy.
- Proof artifacts: backend-swappable checkpoint tests, DB-backed resume/trace/audit/human-gate suite, event replay tests, liveness policy tests, and DBOS/Postgres decision note.
- False-pass guard: moving state into a DB does not prove native representation if route decisions become hidden runtime side effects.
- Deferrals: worker leases/cancellation are M5; final platform conformance and preservation checks are M6.
- Canonical paths/imports: DB-backed resume must persist and restore composition path identity produced by M4/M5 of the composition epic.
