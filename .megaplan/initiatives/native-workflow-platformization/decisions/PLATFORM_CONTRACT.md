...
- Ownership is orthogonal to Step-IO presence, nullability, defaults,
  cardinality and candidate-outcome applicability. Optional omission records
  absence/default without repair; missing required applicable model semantics
  repairs; omitted hints/proposals use declared neutral values; derived and
  protected fields are never requested. Defaults cannot invent semantics.
...
- Agent finish is intercepted before session release. Compilation returns one
  typed disposition: safe deterministic fill, candidate ready, same-live-session
  repair, replacement repair session, changed-evidence generation, or repeated-
  fingerprint fixer escalation. Same-session repair is the default for local
  required omissions and each continuation is a distinct attempt. Session loss
  changes cost only; session identity is provenance and never authority. A
  declared budget forbids identical reminder loops.
...
| **S2A — product-neutral runtime enforcement** | Promote the accepted runtime/adapters in place; implement shared validation/lowering interfaces, lifecycle/root hosting, local test kit, repeat/fork, model-session finish interception/continuation, mode/store isolation, identity/namespaces, loops/joins/resources/cancellation, RA/Custody/WBC generation, locks/receipts, manifest evolution, effects, traces, independent profile re-derivation and real-store contention. | The canonical runtime, repair-continuation and authority substrate is executable and fault-tested without a fork or shadow implementation. S2B need not invent lifecycle, session control, admission, authority or durable-state semantics. |
| **S2B — `.pype` authoring core** | Productize the one-workflow-per-file parser/linker, conservative digest canonicalizer, source correspondence, distribution/package selection, converter, identity-aware transactional refactors, minimum scriptable commands, source maps, plus Native's frozen Step-IO-referencing response-directory materializer/compiler, optionality/applicability projection and typed repair dispositions over S2A. | The S4-blocking format/package/identity/refactor and step-authoring core is one product-neutral experimental SDK. S3 can add DX without changing semantics; S4 need not invent authoring, identity, packaging or migration rules. |
| **S3 — developer experience and tooling** | Complete the CLI/editor/navigation/format/lint/topology/preview/test experience, concise generated prompts, ownership-aware response-directory/child-record editing, normalized-parse previews, same-session repair diagnostics, actionable tracebacks, unfamiliar-author tasks and pinned p50/p95 benchmarks using only S2B/S2A semantics. | The rigorous format is usable without alternate parser/runtime meaning or manual machine-envelope reconstruction; ordinary agents need not understand custody internals; S4 receives a complete local authoring loop. |
...
| `PWC-AF-39` | Schema-generated step authoring | StepAuthoringSpec duplicates no Step-IO schema; five ownership classes remain orthogonal to Step-IO required/optional/conditional rules; monotone proposal merges compile deterministic candidates from hash-bound response directories; derivations cannot reach ambient/ledger state; protected/derived forgery, hint narrowing, final-chat authority, cross-attempt stitching and direct package acceptance fail; normalized rendering exposes a valid-but-wrong nested parse; attempted finish defaults safe omissions, continues the same session for required repairs, survives session loss through a narrow replacement, rejects changed evidence and escalates repeated fingerprints at budget; and the four-axis unrelated consumer succeeds without Megaplan. | Native S2F/S3A, S2A, S2B, S3, S5, S6 |
