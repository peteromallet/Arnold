...
Deliver deterministic format/lint/topology/preview/test/editor workflows over the exact S2A/S2B contracts.

## Scope

Build CLI/editor/navigation, package editing, source-mapped diagnostics, concise generated prompts, normalized parse/repair, completion inspection, author tasks, and numeric usability/performance baselines.

## Locked decisions

- The controlling milestone definition is `docs/arnold/native-megaplan-build-forward-plan-2026-08-24.md` §5, **Platform S3 — Developer tooling**, as qualified by its `Revision 2026-08-24 (post-reconciliation)`.
- Native representation obligations remain the row-level contract in `docs/arnold/megaplan-native-representation-alignment-plan.md` §Traceability Matrix. This milestone owns or affects: **Source diagnostics/local harness; canonical readability; H5–H7; D14–D15**. An `enabled` row is substrate, never implemented conformance.
...
- Which measured author tasks and latency thresholds define usable without weakening admission?
...
- Tooling consumes the same parser, linker, locks, and admission semantics. Views are generated and non-authoritative. Edits are crash-safe.
...
- Unfamiliar-author task results, diagnostics corpus, deterministic local harness, transactional edit recovery, benchmarks, and parity with admitted runtime behavior.

## Touchpoints

- compiler diagnostics; CLI/editor integrations; S1 DX corpus; S2B authoring package
...
- No weaker preview compiler, manual generated metadata, tooling-specific semantics, or stable-publication claim.
