...
Doctrine reconciliation: explicit-node workflow data and manifest execution in
this historical initiative are migration/runtime substrate. They must not be
read as forbidding the current native-representation target where Megaplan
semantics are authored in visible compositional Python source and compiled to a
`WorkflowManifest` for runtime/replay/inspection.
...
- Explicit-node manifests are not the final Megaplan authoring doctrine when
  compositional workflow source can express the same semantics.
