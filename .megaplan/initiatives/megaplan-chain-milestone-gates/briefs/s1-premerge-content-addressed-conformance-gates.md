...
14. Upgrade the completion supersession crosswalk—81 retained requirements plus
    two intentionally changed historical constraints—from descriptive prose
    to mechanically resolvable traceability. Every row must bind an exact
    source artifact plus line/rule or excerpt hash, canonical owner milestone
    labels, and concrete conformance/proof rule IDs. Add a validator that
    rejects missing sources, unknown owners, duplicate IDs, nonexistent proof
    rules, and unconsumed rows. Emit a content-addressed
    `completion-crosswalk-readiness.json`; do not alter the byte-preserved
    historical briefs or proposal.
15. Bind the approved editable runtime identity used by the M11 successor:
    exact `editible-install` commit, import root, installed distribution
    metadata, parser/driver module hashes, and cloud worker runtime provenance.
    Downstream local/cloud preflight must fail on a different commit, import
    root, disabled/stale editable path, or shadowing installed copy. Emit
...
The bootstrap completion manifest must also bind
`completion-crosswalk-readiness.json` and the upgraded crosswalk.
