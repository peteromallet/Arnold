...
This document is supporting background. The main plan captures the controlling direction.

Domain exploration backing this plan:

- `docs/arnold/runauthority-domain-exploration-synthesis.md`

Controlling architecture decision:

- `docs/arnold/runauthority-architecture-decision.md`

The decision doc supersedes any broader wording below that implies a single god-view or a generic reducer that owns publication, runner, recovery, and task semantics directly. The chosen structure is a narrow generic `RunAuthorityKernel`, Megaplan-specific authority binding, and sibling operational views composed into a `MegaplanPlanView` facade where useful.
...
Introduce a generic Arnold `RunAuthorityKernel`, then implement Megaplan's authority binding and composed operational views on top of it.
...
The relationship should be composition, not inheritance:
...
`RunAuthorityKernel` should be generic and domain-neutral. It should contain:
...
- idempotency and compare-and-swap commit semantics.

`MegaplanAuthorityBinding` and `PlanExecutionView` should derive Megaplan-specific state from immutable inputs and verified evidence:
...
- ready frontier;
- task/sense-check satisfaction.

`RunnerView`, `PublicationView`, `HumanGateView`, and `MegaplanRecoveryView` remain sibling views. They can be rendered together as `MegaplanPlanView`, but their policies are not owned by the generic kernel.

All orchestration consumers should read the smallest relevant view, not independently interpret raw artifacts and not necessarily the full `MegaplanPlanView` facade.

The views are not authority in storage. They are deterministic, cacheable projections of authority-bearing evidence at `(run_revision, journal_cursor, observation_set_digest)`. Authority should live in append-only decision events plus immutable content-addressed evidence.
