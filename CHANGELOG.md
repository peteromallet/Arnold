...
- **`_auto_attribute_unclaimed_paths`** (`megaplan/execute/quality.py`): runs after execute result merge and before `_check_done_task_evidence`. Captures the worktree's git-status snapshot, identifies paths not claimed by any task, finds done tasks with empty `files_changed` AND empty `commands_run`, and attributes the unclaimed paths to those tasks. Marks each such task with `auto_attributed_files: true` so audits distinguish system-inferred from model-reported. Mirrors the marker into task updates so task-native execution artifacts reflect attribution.
