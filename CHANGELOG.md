...
- **Briefs**: durable idea briefs and epic chain specs are standardized under `.megaplan/briefs/`; the top-level `briefs/` directory was removed. Single-plan briefs use `.megaplan/briefs/<slug>.md`; epics use `.megaplan/briefs/<epic-slug>/chain.yaml` plus sibling milestone briefs.
- **Benchmarks**: the old top-level `evals/` / `benchmarks/` helper package was removed. Its only live helper, `swe_bench.py`, now lives under `tests/fixtures/` and is imported only by `tests/test_swe_bench.py`.
- **Scripts/tools**: the one-off `tools/` directory was removed. Its only live utility, `silent_failure_census.py`, now lives with the other operator scripts under `scripts/`; `docs/silent-failure-census.md` was updated to use the new command path.
...
- **Vendored Shannon runtime**: the patched Shannon fork moved from repo-root `vendor/shannon/` to `megaplan/vendor/shannon/`, so editable and packaged installs resolve the runtime relative to the installed `megaplan` package instead of depending on a loose top-level repo directory.
