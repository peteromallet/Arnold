...
cost lines via :func:`~arnold.runtime.costing.media_cost.compute_media_cost`, and
...
def account_media_cost_from_result(
    ...
    *,
    ...
    provider:
        Provider name passed to :func:`~arnold.runtime.costing.media_cost.compute_media_cost`
        (e.g. ``"openai"``).
    model:
        Model name passed to ``compute_media_cost`` (e.g. ``"dall-e-3"``).
    pricing_rows:
        Pricing table to search.  When ``None`` (the default),
        :data:`~arnold.runtime.costing.media_cost.DEFAULT_MEDIA_PRICING` is used.
