...
* :func:`arnold.pipeline.patterns.subpipeline_call` is the
...
``ctx.artifact_root / artifact_subdir``), not from in-process state.
"""
...
class SubloopStep:
    ...
    ``artifact_subdir``: subdir under ``ctx.artifact_root`` where the child
    ...
    def run(self, ctx: StepContext) -> StepResult:
        ...
        child_root = Path(ctx.artifact_root) / subdir
        child_root.mkdir(parents=True, exist_ok=True)

        import dataclasses

        child_ctx = dataclasses.replace(ctx, artifact_root=child_root, state=dict(ctx.state) if isinstance(ctx.state, Mapping) else {})
