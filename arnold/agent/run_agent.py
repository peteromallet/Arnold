...
from arnold.agent.hermes_cli.env_loader import load_hermes_dotenv
...
from arnold.agent.model_tools import get_tool_definitions, handle_function_call, check_toolset_requirements
...
from arnold.agent.agent.prompt_builder import (
...
from arnold.agent.agent.context_compressor import ContextCompressor
from arnold.agent.agent.prompt_caching import apply_anthropic_cache_control
from arnold.agent.agent.prompt_builder import build_skills_system_prompt, build_context_files_prompt, load_soul_md
from arnold.agent.agent.usage_pricing import estimate_usage_cost, normalize_usage
from arnold.agent.agent.display import (
    KawaiiSpinner, build_tool_preview as _build_tool_preview,
    get_cute_tool_message as _get_cute_tool_message_impl,
    _detect_tool_failure,
    get_tool_emoji as _get_tool_emoji,
)
from arnold.agent.agent.trajectory import (
...
from arnold.agent.utils import atomic_json_write
...
class ToolInvocationEnvelope:
    """Optional side-channel payload for media-capable agent-loop tools."""
...
def configure_logging(
    ...
    from arnold.agent.agent.redact import RedactingFormatter
...
class AIAgent:
    ...
    def __init__(
        ...
        # Optional per-call media side channel for agent-loop tools. Existing
        # production tools still return plain strings via handle_function_call();
        # synthetic or future media-capable handlers can opt in without
        # changing tool message content.
        self._tool_call_media_usage: dict[str, tuple[MediaUsage, ...]] = {}
        self._synthetic_tool_handlers: dict[str, Callable[..., Any]] = {}
        ...
        if self.api_mode == "anthropic_messages":
            ...
            from arnold.agent.agent.anthropic_adapter import _is_oauth_token as _is_oat
        ...
        from arnold.agent.tools.checkpoint_manager import CheckpointManager
        ...
        from arnold.agent.tools.todo_tool import TodoStore
        self._todo_store = TodoStore()
        
        # Load config once for memory, skills, and compression sections
        try:
            from arnold.agent.hermes_cli.config import load_config as _load_agent_config
        ...
        if not skip_memory:
            try:
                ...
                if self._memory_enabled or self._user_profile_enabled:
                    from arnold.agent.tools.memory_tool import MemoryStore
        ...
        if not skip_memory:
            try:
                if honcho_manager is not None:
                    ...
                    from arnold.agent.honcho_integration.client import HonchoClientConfig, get_honcho_client
                    hcfg = HonchoClientConfig.from_global_config()
                    self._honcho_config = hcfg
                    if self._honcho_should_activate(hcfg):
                        from arnold.agent.honcho_integration.session import HonchoSessionManager
        ...
        self.session_media_cost_status = "unknown"
        self.session_media_cost_source = "none"
        self.session_media_cost_lines: list[CostResult] = []
    ...
    def reset_session_state(self):
        ...
        self.session_media_cost_status = "unknown"
        self.session_media_cost_source = "none"
        self.session_media_cost_lines = []
        self._tool_call_media_usage.clear()
    ...
    def _activate_honcho(
        ...
        *,
        ...
        from arnold.agent.tools.honcho_tools import set_session_context
    ...
    def _build_system_prompt(self, system_message: str = None) -> str:
        ...
        from arnold.agent.hermes_time import now as _hermes_now
        now = _hermes_now()
    ...
    def _cap_delegate_task_calls(tool_calls: list) -> list:
        ...
        """
        from arnold.agent.tools.delegate_tool import MAX_CONCURRENT_CHILDREN
    ...
    def _create_openai_client(self, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
        if self.provider == "copilot-acp" or str(client_kwargs.get("base_url", "")).startswith("acp://copilot"):
            from arnold.agent.agent.copilot_acp_client import CopilotACPClient
    ...
    def _abort_request_client(self, request_client_holder: dict, *, reason: str) -> None:
        try:
            if self.api_mode == "anthropic_messages":
                from arnold.agent.agent.anthropic_adapter import build_anthropic_client
    ...
    def _try_refresh_nous_client_credentials(self, *, force: bool = True) -> bool:
        ...
        try:
            from arnold.agent.hermes_cli.auth import resolve_nous_runtime_credentials
    ...
    def _try_refresh_anthropic_client_credentials(self) -> bool:
        ...
        try:
            from arnold.agent.agent.anthropic_adapter import resolve_anthropic_token, build_anthropic_client
    ...
    def _try_activate_fallback(self) -> bool:
        ...
        try:
            ...
            if fb_api_mode == "anthropic_messages":
                ...
                from arnold.agent.agent.anthropic_adapter import build_anthropic_client, resolve_anthropic_token, _is_oauth_token
    ...
    def _describe_image_for_anthropic_fallback(self, image_url: str, role: str) -> str:
        ...
        description = ""
        try:
            from arnold.agent.tools.vision_tools import vision_analyze_tool
    ...
    def _build_api_kwargs(self, api_messages: list) -> dict:
        ...
        if self.api_mode == "anthropic_messages":
            from arnold.agent.agent.anthropic_adapter import build_anthropic_kwargs
        ...
        if self._supports_reasoning_extra_body():
            if _is_deepseek_direct:
                ...
                if self.reasoning_config is not None:
                    ...
                    _eff = rc.get("effort")
                    if _is_openrouter:
                        # OpenRouter's reasoning.effort vocab is
                        # minimal|low|medium|high|xhigh (verified live 2026-05:
                        # effort="max" 400s, "xhigh" is accepted). Map the
                        # DeepSeek-only `max` token down to xhigh, OpenRouter's
                        # ceiling. DeepSeek's direct route (handled above) keeps
                        # max intact.
                        if _eff == "max":
                            rc["effort"] = "xhigh"
                    elif _eff in ("xhigh", "max"):
                        # Nous/Vercel reasoning vocab is low/medium/high; clamp
                        # the higher tokens down to avoid a 400 on routes we
                        # can't probe.
                        rc["effort"] = "high"
    ...
    def _supports_reasoning_extra_body(self) -> bool:
        ...
        if "models.github.ai" in self._base_url_lower or "api.githubcopilot.com" in self._base_url_lower:
            try:
                from arnold.agent.hermes_cli.models import github_model_reasoning_efforts
    ...
    def flush_memories(self, messages: list = None, min_turns: int = None):
        ...
        try:
            ...
            from arnold.agent.agent.auxiliary_client import call_llm as _call_llm
            _aux_available = True
            try:
                response = _call_llm(
                    task="flush_memories",
            ...
            if not _aux_available and self.api_mode == "codex_responses":
                ...
                from arnold.agent.agent.anthropic_adapter import build_anthropic_kwargs as _build_ant_kwargs
            ...
            if self.api_mode == "codex_responses" and not _aux_available:
                ...
                from arnold.agent.agent.anthropic_adapter import normalize_anthropic_response as _nar_flush
            ...
            for tc in tool_calls:
                if tc.function.name == "memory":
                    try:
                        ...
                        from arnold.agent.tools.memory_tool import memory_tool as _memory_tool
                        result = _memory_tool(
    ...
    def register_synthetic_tool_handler(
        self,
        tool_name: str,
        ...
        *,
        ...
        """Register a test-only or synthetic tool handler for this agent instance."""
        self._synthetic_tool_handlers[tool_name] = handler
            ...
            self.tools.append(
                {
                    ...
                    "function": {
                        "name": tool_name,
                        "description": description or f"Synthetic tool {tool_name}",
                        "parameters": {"type": "object", "properties": {}},
                    },
                }
            )

    def _record_tool_call_media_usage(
        self,
        tool_call_id: str | None,
        media_usage: MediaUsage | list[MediaUsage] | tuple[MediaUsage, ...] | None,
    ) -> None:
        """Store normalized media usage for a tool call without touching its text result."""
        if not tool_call_id:
            return
        normalized = media_usage_from_hook_metadata(media_usage)
        if normalized:
            self._tool_call_media_usage[tool_call_id] = normalized

    def get_tool_call_media_usage(self, tool_call_id: str) -> tuple[MediaUsage, ...]:
        """Return media usage registered for a tool call, or an empty tuple."""
        return self._tool_call_media_usage.get(tool_call_id, ())

    def _update_session_media_cost_summary(
        self,
        media_cost_lines: tuple[CostResult, ...],
    ) -> None:
        """Track media cost status/source separately from token cost scalars."""
        if not media_cost_lines:
            return

        self.session_media_cost_lines.extend(media_cost_lines)

        statuses = {line.status for line in self.session_media_cost_lines}
        if "unknown" in statuses:
            self.session_media_cost_status = "unknown"
        elif len(statuses) == 1:
            self.session_media_cost_status = next(iter(statuses))
        else:
            self.session_media_cost_status = "mixed"

        sources = {line.source for line in self.session_media_cost_lines}
        self.session_media_cost_source = (
            next(iter(sources)) if len(sources) == 1 else "mixed"
        )

    def _account_tool_call_media_cost(self, tool_call_id: str | None) -> tuple[CostResult, ...]:
        """Price a tool call's media usage and add only known amounts to the session total."""
        if not tool_call_id:
            return ()

        media_usage = self.get_tool_call_media_usage(tool_call_id)
        if not media_usage:
            return ()

        media_cost_lines = compute_media_cost(
            self.provider,
            self.model,
            media_usage,
        )
        for cost_line in media_cost_lines:
            if cost_line.amount_usd is not None:
                self.session_estimated_cost_usd += float(cost_line.amount_usd)
        self._update_session_media_cost_summary(media_cost_lines)
        return media_cost_lines

    @staticmethod
    def _serialize_cost_line(cost_line: CostResult) -> dict[str, Any]:
        """Return a JSON-safe representation of a cost line for session summaries."""
        return {
            "amount_usd": float(cost_line.amount_usd) if cost_line.amount_usd is not None else None,
            "status": cost_line.status,
            "source": cost_line.source,
            "label": cost_line.label,
            "fetched_at": (
                cost_line.fetched_at.isoformat()
                if isinstance(cost_line.fetched_at, datetime)
                else None
            ),
            "pricing_version": cost_line.pricing_version,
            "notes": list(cost_line.notes),
        }

    def _unwrap_tool_invocation_result(
        self,
        result: Any,
        *,
        tool_call_id: str | None = None,
    ) -> str:
        """Return the tool's text result and capture any side-channel media usage."""
        if isinstance(result, ToolInvocationEnvelope):
            self._record_tool_call_media_usage(tool_call_id, result.media_usage)
    ...
    def _invoke_tool(
        ...
        """
        synthetic_handler = self._synthetic_tool_handlers.get(function_name)
        if synthetic_handler is not None:
            result = synthetic_handler(
                function_args,
                task_id=effective_task_id,
                tool_call_id=tool_call_id,
                agent=self,
            )
            return self._unwrap_tool_invocation_result(result, tool_call_id=tool_call_id)

        if function_name == "todo":
            from arnold.agent.tools.todo_tool import todo_tool as _todo_tool
            return _todo_tool(
            ...
            from arnold.agent.tools.session_search_tool import session_search as _session_search
            ...
            from arnold.agent.tools.clarify_tool import clarify_tool as _clarify_tool
            ...
            from arnold.agent.tools.delegate_tool import delegate_task as _delegate_task
    ...
    def _execute_tool_calls_concurrent(self, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
        ...
        for i, (tc, name, args) in enumerate(parsed_calls):
            ...
            self._account_tool_call_media_cost(tc.id)
    ...
    def _execute_tool_calls_sequential(self, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
        ...
        for i, tool_call in enumerate(assistant_message.tool_calls, 1):
            ...
            messages.append(tool_msg)
            self._account_tool_call_media_cost(tool_call.id)
    ...
    def _emit_context_pressure(self, compaction_progress: float, compressor) -> None:
        ...
        """
        from arnold.agent.agent.display import format_context_pressure, format_context_pressure_gateway
    ...
    def _handle_max_iterations(self, messages: list, api_call_count: int) -> str:
        ...
        try:
            ...
            if self.api_mode == "codex_responses":
                ...
                if self.api_mode == "anthropic_messages":
                    from arnold.agent.agent.anthropic_adapter import build_anthropic_kwargs as _bak, normalize_anthropic_response as _nar
            ...
            if final_response:
                ...
                if self.api_mode == "codex_responses":
                    ...
                    from arnold.agent.agent.anthropic_adapter import build_anthropic_kwargs as _bak2, normalize_anthropic_response as _nar2
    ...
    def _run_conversation_impl(
        ...
        while api_call_count < self.max_iterations and self.iteration_budget.remaining > 0:
            ...
            try:
                if self.api_mode == "codex_responses":
                    ...
                    from arnold.agent.agent.anthropic_adapter import normalize_anthropic_response
        ...
        result = {
            ...
            "media_cost_status": self.session_media_cost_status,
            "media_cost_source": self.session_media_cost_source,
            "media_cost_lines": [
                self._serialize_cost_line(cost_line)
                for cost_line in self.session_media_cost_lines
            ],
        }
...
def main(
    ...
    # Handle tool listing
    if list_tools:
        from arnold.agent.model_tools import get_all_tool_names, get_toolset_for_tool, get_available_toolsets
        from arnold.agent.toolsets import get_all_toolsets, get_toolset_info
