...
Add, remove, or reorder entries here — both the setup wizard and
provider selection will pick up the change automatically.
"""
...
def _load_legacy_codex_models_module():
    module_name = ".".join(("arnold", "pipelines", "megaplan", "agent", "hermes_cli", "codex_models"))
    return importlib.import_module(module_name)


def _load_legacy_copilot_auth_module():
    module_name = ".".join(("arnold", "pipelines", "megaplan", "agent", "hermes_cli", "copilot_auth"))
    return importlib.import_module(module_name)


def _load_legacy_anthropic_adapter_module():
    module_name = ".".join(("arnold", "pipelines", "megaplan", "agent", "agent", "anthropic_adapter"))
    return importlib.import_module(module_name)
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    for pid in _PROVIDER_ORDER:
        ...
        has_creds = False
        try:
            from arnold.agent.hermes_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def detect_provider_for_model(
    ...
    if direct_match:
        ...
        has_creds = False
        try:
            from arnold.agent.hermes_cli.auth import PROVIDER_REGISTRY
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to omp's canonical provider ids.
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    try:
        from arnold.agent.hermes_cli.auth import resolve_api_key_provider_credentials
...
def provider_model_ids(provider: Optional[str]) -> list[str]:
    ...
    if normalized == "openai-codex":
        return _load_legacy_codex_models_module().get_codex_model_ids()
    ...
    if normalized == "nous":
        ...
        try:
            from arnold.agent.hermes_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
...
def _fetch_anthropic_models(timeout: float = 5.0) -> Optional[list[str]]:
    ...
    """
    try:
        anthropic_adapter = _load_legacy_anthropic_adapter_module()
    except ImportError:
        return None

    token = anthropic_adapter.resolve_anthropic_token()
    if not token:
        return None

    headers: dict[str, str] = {"anthropic-version": "2023-06-01"}
    if anthropic_adapter._is_oauth_token(token):
        headers["Authorization"] = f"Bearer {token}"
        headers["anthropic-beta"] = ",".join(
            anthropic_adapter._COMMON_BETAS + anthropic_adapter._OAUTH_ONLY_BETAS
        )
    else:
...
def copilot_default_headers() -> dict[str, str]:
    ...
    """
    try:
        return _load_legacy_copilot_auth_module().copilot_request_headers(
            is_agent_turn=True
        )
    except ImportError:
        return {
...
def _fetch_ai_gateway_models(timeout: float = 5.0) -> Optional[list[str]]:
    ...
    if not base_url:
        from arnold.agent.hermes_constants import AI_GATEWAY_BASE_URL
