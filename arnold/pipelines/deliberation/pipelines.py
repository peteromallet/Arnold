"""Private graph-era deliberation pipeline construction helpers.
...
def _build_graph_pipeline(name: str = "deliberation", **kwargs: Any) -> Pipeline:
    """Private graph baseline — delegates to :func:`_build_initial_pipeline`.

    Accepts ``name`` and any keyword arguments that
    :func:`_build_initial_pipeline` expects.  When *profile* and
    *workers* are not provided, returns an empty pipeline suitable
    ...
    if profile is None or workers is None:
        ...
        f"to build the private deliberation DAG baseline."
            ) from None

    prompts = kwargs.pop("prompts", None)
    return _build_initial_pipeline(profile=profile, workers=workers, prompts=prompts)
