...
from arnold.pipeline.types import Edge, ParallelStage, Stage, StepContext, StepResult
...
# ---------------------------------------------------------------------------
# Native runtime steps and declarations
# ---------------------------------------------------------------------------


_NATIVE_STAGE_ORDER: tuple[tuple[str, str], ...] = (
    ...
    ("layer_mid_panel", "layer_mid_synth"),
    ("layer_mid_synth", "layer_low_panel"),
    ("layer_low_panel", "layer_low_synth"),
    ("layer_low_synth", "final_report"),
    ("final_report", "halt"),
)


def _next_version(path: Path, suffix: str) -> int:
    highest = 0
    for candidate in path.glob(f"v*.{suffix}"):
        match = re.match(rf"^v(\d+)\.{re.escape(suffix)}$", candidate.name)
        if match:
...
def _ctx_from_native(raw_ctx: object) -> StepContext:
    if isinstance(raw_ctx, dict):
        ...
        return StepContext(
            ...
            state=state,
            inputs=inputs,
            mode=str(raw_ctx.get("mode", state.get("mode", "native"))),
        )
    ...
    return StepContext(
        ...
        state=dict(raw_state) if isinstance(raw_state, Mapping) else {},
        inputs=dict(raw_inputs) if isinstance(raw_inputs, Mapping) else {},
        mode=str(getattr(raw_ctx, "mode", "native")),
    )
...
class DeliberationStep:
    """Native runtime shell for one deliberation phase."""

    name: str
    next_label: str
    output_label: str = "artifact"
    output_suffix: str = "md"
    kind: str = "produce"

    def run(self, ctx: StepContext) -> StepResult:
        state = dict(ctx.state) if isinstance(ctx.state, Mapping) else {}
        artifacts = state.get("_deliberation_artifacts")
        artifact_map = dict(artifacts) if isinstance(artifacts, Mapping) else {}

        out_dir = Path(ctx.artifact_root) / self.name / self.output_label
        out_dir.mkdir(parents=True, exist_ok=True)
        version = _next_version(out_dir, self.output_suffix)
        artifact_path = out_dir / f"v{version}.{self.output_suffix}"
        artifact_path.write_text(
            _render_native_artifact(self.name, state, artifact_map),
            encoding="utf-8",
        )

        artifact_map[self.name] = str(artifact_path)
        patch: dict[str, Any] = {
            "_deliberation_artifacts": artifact_map,
            "_deliberation_last_stage": self.name,
        }
        if self.name == "final_report":
            patch["final_report"] = str(artifact_path)

        return StepResult(
            outputs={self.output_label: artifact_path},
            next=self.next_label,
            state_patch=patch,
        )


def _render_native_artifact(
    stage_name: str,
    state: Mapping[str, Any],
    artifacts: Mapping[str, Any],
) -> str:
    idea = state.get("idea") or state.get("input") or "unspecified idea"
    lines = [
        f"# deliberation/{stage_name}",
        "",
        f"Idea: {idea}",
    ]
    if artifacts:
...
def _run_native_stage(ctx: object, stage_name: str, next_label: str) -> StepResult:
    ...
    return _json_safe_step_result(
        ...
        ).run(_ctx_from_native(ctx))
    )


@phase(name="question_gen")
def native_question_gen(ctx: object) -> StepResult:
    return _run_native_stage(ctx, "question_gen", "human_gate")


@phase(name="human_gate")
def native_human_gate(ctx: object) -> StepResult:
    return _run_native_stage(ctx, "human_gate", "draft_plan")


@phase(name="draft_plan")
def native_draft_plan(ctx: object) -> StepResult:
    return _run_native_stage(ctx, "draft_plan", "layer_high_panel")


@phase(name="layer_high_panel")
def native_layer_high_panel(ctx: object) -> StepResult:
    return _run_native_stage(ctx, "layer_high_panel", "layer_high_synth")


@phase(name="layer_high_synth")
def native_layer_high_synth(ctx: object) -> StepResult:
    return _run_native_stage(ctx, "layer_high_synth", "layer_mid_panel")


@phase(name="layer_mid_panel")
def native_layer_mid_panel(ctx: object) -> StepResult:
    return _run_native_stage(ctx, "layer_mid_panel", "layer_mid_synth")


@phase(name="layer_mid_synth")
def native_layer_mid_synth(ctx: object) -> StepResult:
    return _run_native_stage(ctx, "layer_mid_synth", "layer_low_panel")


@phase(name="layer_low_panel")
def native_layer_low_panel(ctx: object) -> StepResult:
    return _run_native_stage(ctx, "layer_low_panel", "layer_low_synth")


@phase(name="layer_low_synth")
def native_layer_low_synth(ctx: object) -> StepResult:
    return _run_native_stage(ctx, "layer_low_synth", "final_report")


@phase(name="final_report")
def native_final_report(ctx: object) -> StepResult:
    return _run_native_stage(ctx, "final_report", "halt")


@pipeline(name="deliberation", description="Native linear deliberation pipeline")
def deliberation_native(ctx: object) -> Any:
    state = yield native_question_gen(ctx)
    state = yield native_human_gate(ctx)
    state = yield native_draft_plan(ctx)
    state = yield native_layer_high_panel(ctx)
    state = yield native_layer_high_synth(ctx)
    state = yield native_layer_mid_panel(ctx)
    state = yield native_layer_mid_synth(ctx)
    state = yield native_layer_low_panel(ctx)
    state = yield native_layer_low_synth(ctx)
    state = yield native_final_report(ctx)
    return state


def build_native_program():
    return compile_pipeline(deliberation_native)
...
def run_native(
    *,
    ...
    initial_state = {"idea": idea, "mode": mode}
