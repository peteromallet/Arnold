...
Counterpart to ``HandlerStep`` (which shells out via subprocess).
...
M4 (T5): Relocated to ``arnold.pipelines.megaplan.runtime.inprocess_step``
from the retired ``arnold.pipelines.megaplan.stages.inprocess_step``.

Production callers (``megaplan/auto.py`` after Sprint 3 integration)
should use ``HandlerStep`` so the existing per-phase subprocess
