...
def _refuse_engine_pin_drift(metadata: dict[str, Any], env: ExecutionEnvironment, *, phase: str) -> None:
    ...
    valid_waived_signature_drift = (
        set(drift) == {"engine_signature"}
        and _engine_overlap_waiver_invalid_reason(
            latest_engine_overlap_waiver(metadata),
            env,
            phase=phase,
            now=None,
        )
    ...
    if valid_waived_signature_drift:
        return
    if drift:
        raise isolation_cli_error(
            "engine_pin_drift",
...
def _engine_overlap_waiver_invalid_reason(
    ...
    *,
    ...
    # drift check in _refuse_engine_pin_drift still catches engine root/commit
    # changes before a phase runs.
