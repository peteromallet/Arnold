...
def consume_engine_write_isolation_waiver(
    metadata: dict[str, Any] | None,
    env: ExecutionEnvironment,
    *,
    phase: str,
    ...
    """Consume a scoped operator waiver for local engine-write isolation.

    The normal mutating-phase path requires a provider to prove the engine is
    unwritable before worker dispatch. On local dogfood runs that proof is not
    always available because the operator owns both the engine checkout and the
    target checkout. This helper keeps the bypass explicit: the latest
    ``waive-engine-overlap`` record must match the current target/work/engine
    scope and current engine identity, and each worker launch consumes one run.
    The after-worker engine drift check still remains authoritative.
    """

    if not isinstance(metadata, dict):
        raise isolation_cli_error(
            "engine_write_isolation_unverified",
            "engine write isolation is not verified and no scoped operator waiver is available",
            env=env,
            extra={"phase": phase, "waiver_invalid_reason": "missing"},
        )
    waiver = latest_engine_overlap_waiver(metadata)
    invalid_reason = _engine_overlap_waiver_invalid_reason(
        waiver,
        env,
        phase=phase,
    ...
    if invalid_reason is not None:
        raise isolation_cli_error(
            "engine_write_isolation_unverified",
            "engine write isolation is not verified and the scoped operator waiver is not valid",
            env=env,
            extra={
                "phase": phase,
                "latest_engine_overlap_waiver_id": waiver.get("id") if isinstance(waiver, dict) else None,
    ...
    _consume_engine_overlap_waiver(metadata, waiver, phase=phase, now=now)
...
def _refuse_engine_pin_drift(metadata: dict[str, Any], env: ExecutionEnvironment, *, phase: str) -> None:
    ...
    same_checkout_dogfood_signature_drift = (
        set(drift) == {"engine_signature"}
        and record.get("engine_commit") == env.engine_commit
        and env.engine_dirty
        and str(env.target_root) == str(env.engine_root)
        and str(env.work_dir) == str(env.engine_root)
    )
    if same_checkout_dogfood_signature_drift:
        return
    if drift:
        raise isolation_cli_error(
            "engine_pin_drift",
...
def _engine_overlap_waiver_invalid_reason(
    ...
    *,
    ...
    # engine_signature is intentionally NOT checked here because it changes on
    # every uncommitted harness edit (dirty flag), which makes local dogfood
    # impossible after harmless harness changes.  The scope (paths) and commit
    # checks are sufficient to keep the waiver scoped; the global engine-pin
    # drift check in _refuse_engine_pin_drift still catches actual engine
    # identity changes before a phase runs.
