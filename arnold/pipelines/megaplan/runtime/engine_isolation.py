...
def engine_write_barrier(
    ...
    *,
    ...
    env_vars = env_vars or os.environ
    if env_vars.get("MEGAPLAN_ALLOW_UNVERIFIED_ENGINE_WRITE_ISOLATION", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
        ...
        return EngineIsolationProof(
            provider="explicit_unverified_local_waiver",
            trusted_container=False,
            engine_write_denied=False,
            target_write_allowed=True,
            same_user_chmod_accepted=False,
            diagnostic="operator_set_MEGAPLAN_ALLOW_UNVERIFIED_ENGINE_WRITE_ISOLATION",
        )
