...
from arnold.pipelines.megaplan._core import atomic_write_text, get_effective


def _canonical_instructions() -> str:
    return (
        resources.files("arnold.pipelines.megaplan")
...
def _resolve_bundle_path(data_name: str) -> Path:
    return Path(str(resources.files("arnold.pipelines.megaplan").joinpath("data", data_name)))
...
def handle_regen_composed() -> dict[str, Any]:
    ...
    composed_dir = resources.files("arnold.pipelines.megaplan").joinpath("data", "_composed")
    ...
    for name, computed in targets.items():
        ...
        if current != computed:
            ...
            changed.append(name)
    data_dir = Path(str(resources.files("arnold.pipelines.megaplan").joinpath("data")))
