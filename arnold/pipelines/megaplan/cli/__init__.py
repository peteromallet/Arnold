...
from arnold.pipelines.megaplan.loop.handlers import (
    handle_loop_init,
    handle_loop_pause,
    handle_loop_run,
    handle_loop_status,
...
from arnold.pipelines.megaplan.execute.step_edit import handle_step
from arnold.pipelines.megaplan.observability.doctor import handle_doctor
from arnold.pipelines.megaplan.observability.trace import handle_trace
from arnold.pipelines.megaplan.resolutions import SUPPORTED_USER_ACTION_RESOLUTION_STATES
...
def handle_ticket(args: argparse.Namespace) -> int:
    ...
    from arnold.pipelines.megaplan.handlers.tickets import TICKET_DISPATCH
    from arnold.pipelines.megaplan.tickets.registry import touch as _registry_touch
...
def handle_migrate_local_plans(root: Path, args: argparse.Namespace) -> StepResponse:
    del root
    from arnold.pipelines.megaplan.store.legacy_migration import migrate_local_plans
...
def _handle_introspect(root: Path, args: argparse.Namespace) -> int:
    ...
    from arnold.pipelines.megaplan.observability.introspect import build_introspect_payload
...
def _handle_cost(root: Path, args: argparse.Namespace) -> int:
    from arnold.pipelines.megaplan.observability.cost import handle_cost
        ...
        for disp in dispositions:
            ...
            if disp.traceback:
                for tb_line in disp.traceback.rstrip("\n").splitlines():
                    print(f"    {tb_line}")
        return 0
    if action == "upgrade-cursor":
        from arnold.pipelines.megaplan.cli.cursor_upgrade import run_upgrade_cursor

        plan_dir = getattr(args, "plan_dir", None)
        if not plan_dir:
            print("pipelines upgrade-cursor: missing plan directory", file=sys.stderr)
            return 1
        return run_upgrade_cursor(plan_dir, dry_run=not bool(getattr(args, "write", False)))
    if action == "new":
        from arnold.pipelines.megaplan._pipeline.registry import _SCAN_ROOTS, _cli_name
        ...
        for dir_path, pkg_prefix in _SCAN_ROOTS:
            if pkg_prefix == "arnold.pipelines.megaplan.pipelines":
        ...
        module_content = _pipeline_scaffold_module_content(
            cli_name=cli_name,
...
def _pipeline_scaffold_module_content(
    *,
    cli_name: str,
    module_stem: str,
    driver: str,
...
def _native_scaffold_module_content(*, cli_name: str, module_stem: str) -> str:
    native_entrypoint = f"{module_stem}_native"
    return (
        f'"""Native-authored pipeline scaffold for ``{cli_name}``."""\n'
        f"\n"
        f"from __future__ import annotations\n"
        f"\n"
        f"from typing import Any\n"
        f"\n"
        f"from arnold.pipeline.native import (\n"
        f"    compile_pipeline,\n"
        f"    decision,\n"
        f"    parallel,\n"
        f"    phase,\n"
        ...
        f"\n"
        f"\n"
        f"def _state(ctx: Any) -> dict[str, Any]:\n"
        f"    raw = ctx.get(\"state\") if isinstance(ctx, dict) else getattr(ctx, \"state\", None)\n"
        f"    return dict(raw) if isinstance(raw, dict) else {{}}\n"
        f"\n"
        f"\n"
        f'@phase(name="draft")\n'
        f"def _draft(ctx: Any) -> dict[str, Any]:\n"
        f'    return {{"draft": _state(ctx).get("draft", "TODO: produce draft")}}\n'
        f"\n"
        f"\n"
        f'@phase(name="review_fast")\n'
        f"def _review_fast(ctx: Any) -> dict[str, Any]:\n"
        f'    return {{"fast_review": "TODO: add first review"}}\n'
        f"\n"
        f"\n"
        f'@phase(name="review_deep")\n'
        f"def _review_deep(ctx: Any) -> dict[str, Any]:\n"
        f'    return {{"deep_review": "TODO: add second review"}}\n'
        f"\n"
        f"\n"
        f"def _merge_reviews(results: list[Any]) -> dict[str, Any]:\n"
        f"    merged: dict[str, Any] = {{}}\n"
        f"    for index, result in enumerate(results, start=1):\n"
        f"        if isinstance(result, dict):\n"
        f"            merged[f\"review_{{index}}\"] = dict(result)\n"
        f"        else:\n"
        f"            merged[f\"review_{{index}}\"] = result\n"
        f'    return {{"reviews": merged}}\n'
        f"\n"
        f"\n"
        f'@decision(name="approval", vocabulary=frozenset({{"ship", "revise"}}))\n'
        f"def _approval(ctx: Any) -> str:\n"
        f'    return "revise" if _state(ctx).get("needs_revision") else "ship"\n'
        f"\n"
        f"\n"
        f'@phase(name="revise")\n'
        f"def _revise(ctx: Any) -> dict[str, Any]:\n"
        f'    return {{"revision": "TODO: revise from review notes"}}\n'
        f"\n"
        f"\n"
        f'@phase(name="publish")\n'
        f"def _publish(ctx: Any) -> dict[str, Any]:\n"
        f'    return {{"status": "ready"}}\n'
        f"\n"
        f"\n"
        f'@pipeline("{cli_name}", description=description)\n'
        f"def {native_entrypoint}(ctx: Any) -> Any:\n"
        f"    state = yield _draft(ctx)\n"
        f"    for branch in parallel(\n"
        f"        [_review_fast, _review_deep],\n"
        f"        reducer=_merge_reviews,\n"
        f'        name="review_panel",\n'
        f"    ):\n"
        f"        state = yield branch(ctx)\n"
        f'    if _approval(ctx) == "revise":\n'
        f"        state = yield _revise(ctx)\n"
        f"    state = yield _publish(ctx)\n"
        f"    return state\n"
        f"\n"
        f"\n"
        f"def _validate_derived_graph(graph: Pipeline) -> None:\n"
        f'    required = {{"draft", "review_panel", "approval", "publish"}}\n'
        f"    missing = required.difference(graph.stages)\n"
        f"    if missing:\n"
        f"        raise RuntimeError(f\"derived graph missing stages: {{sorted(missing)}}\")\n"
        f"\n"
        f"\n"
        f"def build_pipeline() -> Pipeline:\n"
        f'    """Return the native-authored ``{cli_name}`` graph projection."""\n'
        f"    program = compile_pipeline({native_entrypoint})\n"
...
def _setup_init_worktree(args: argparse.Namespace) -> None:
    ...
    from arnold.pipelines.megaplan.bakeoff.worktree import (
        branch_exists,
...
def main(argv: list[str] | None = None) -> int:
    ...
    if argv and argv[0] == "cloud":
        from arnold.pipelines.megaplan.cloud.cli import _register_cloud_subcommands, run_cloud_cli
    ...
    if argv and argv[0] == "resident":
        from arnold.pipelines.megaplan.resident.cli import (
    ...
    if argv and argv[0] == "bakeoff":
        from arnold.pipelines.megaplan.bakeoff.cli import _register_bakeoff_subcommands, run_bakeoff_cli
    ...
    if args.command == "auto":
        from arnold.pipelines.megaplan.auto import run_auto
    ...
    if args.command == "chain":
        from arnold.pipelines.megaplan.chain import run_chain_cli
    ...
    if args.command == "tiebreaker":
        from arnold.pipelines.megaplan.prompts.tiebreaker_orchestrator import run_tiebreaker_cli
