...
from arnold.pipelines.megaplan.types import StepResponse
from arnold.pipelines.megaplan.planning.state import STATE_BLOCKED
from arnold.pipelines.megaplan.user_actions import FALLBACK, OMIT
...
def _compute_user_action_blockers(
    ...
    from arnold.pipelines.megaplan.resolutions import load_user_action_resolutions
...
def handle_audit(root: Path, args: argparse.Namespace) -> StepResponse:
    ...
    if getattr(args, "audit_action", None) == "query":
        from arnold.pipelines.megaplan.receipts.query import handle_audit_query

        return handle_audit_query(root, args)
    if getattr(args, "audit_action", None) == "report":
        from arnold.pipelines.megaplan.receipts.report import handle_audit_report
