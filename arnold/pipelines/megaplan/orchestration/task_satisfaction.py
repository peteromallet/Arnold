...
def is_task_satisfied(
    ...
    ``allowed_heads`` relaxes head freshness: evidence observed at any commit in
    this set is treated as fresh relative to ``current_head``. This is used when
    the repository HEAD has advanced after evidence was produced (e.g., the
    chain committed an execute batch) but the evidence still describes the
    current tree because the new HEAD is a descendant of the observed commit.
    """
    ...
    stale_evidence = _stale_evidence(linked_refs, current_head, current_code_hash, allowed_heads=allowed_heads)
