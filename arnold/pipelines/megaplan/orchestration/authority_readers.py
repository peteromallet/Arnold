...
def corroborated_completed_task_ids(
    ...
    *,
    ...
    allowed_heads = None
    if current_head and resolved_plan_dir is not None:
        allowed_heads = _ancestor_heads(resolved_plan_dir, current_head)
    ...
    for task in task_records:
        ...
        if resolved_plan_dir is not None:
            _emit_authority_divergence_diagnostics(resolved_plan_dir, task, decision)
...
def _ancestor_heads(plan_dir: Path, head: str) -> set[str]:
    """Return all ancestor commits of *head* (including *head* itself).

    Evidence produced at an ancestor commit is still valid after the repository
    advances, because the new HEAD is a descendant of the evidence's observed
    commit. Returns an empty set if git is unavailable.
    """

    try:
        completed = subprocess.run(
            # Cap ancestor traversal: evidence older than the recent execute→gate
            # window is irrelevant for freshness relaxation.
