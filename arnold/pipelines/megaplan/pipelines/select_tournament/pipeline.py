"""Native declaration and graph builder for the ``select-tournament`` pipeline.
...
def _project_native_pipeline(candidate_list: tuple[str, ...]) -> Pipeline:
    ...
    program = compile_pipeline(select_tournament)
...
def _build_legacy_graph_pipeline(
    ...
    pipeline = _project_native_pipeline(candidate_list)
    return dataclasses.replace(pipeline, native_program=None)
