...
:class:`CreativeStep` implementations.  The graph builder remains canonical;
...
def _ctx_from_native(raw_ctx: object) -> StepContext:
    ...
    artifact_root = getattr(raw_ctx, "plan_dir", None) or getattr(
        raw_ctx,
        "artifact_root",
        ".",
    )
...
def build_pipeline(
    ...
    *primary_criterion*, when set, is threaded through to each stage as
    ...
    graph = Pipeline(stages=stages, entry="prep")
    return replace(
        graph,
