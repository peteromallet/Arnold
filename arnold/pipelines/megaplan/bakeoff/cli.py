...
def run_bakeoff_cli(root: Path, args: argparse.Namespace) -> int:
    ...
    if action == "run":
        ...
        if supervisor_tier_routing_on():
            from arnold.pipelines.megaplan.supervisor.bakeoff_runner import run_bakeoff_run_handler as _supervisor_bakeoff_handler

            return _supervisor_bakeoff_handler(root, args)

        from arnold.pipelines.megaplan.bakeoff.orchestrator import run_bakeoff_run_handler  # noqa: E402
...
def _load_handlers() -> dict[str, Callable[[Path, argparse.Namespace], int]]:
    from arnold.pipelines.megaplan.bakeoff.handlers import (
        handle_abandon,
        handle_compare,
        handle_merge,
        handle_pick,
