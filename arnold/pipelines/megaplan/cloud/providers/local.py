...
from arnold.pipelines.megaplan.cloud.spec import CloudSpec, LocalSpec
...
class LocalProvider(Provider):
    ...
    def _deploy_dir(self) -> Path:
        from arnold.pipelines.megaplan.cloud.cli import _persistent_deploy_dir
    ...
    def build(self, deploy_dir: Path) -> int:
        del deploy_dir
        self._run(self._compose_argv("build"))
        return 0
    ...
    def deploy(self, deploy_dir: Path, *, secrets: dict[str, str]) -> int:
        ...
        self._run(self._compose_argv("up", "-d"))
        return 0

    def ssh_exec(self, command: str) -> subprocess.CompletedProcess[str]:
        return self._run(self._compose_argv("exec", "-T", "agent", "bash", "-lc", command))
    ...
    def down(self) -> int:
        self._run(self._compose_argv("stop"))
        return 0
