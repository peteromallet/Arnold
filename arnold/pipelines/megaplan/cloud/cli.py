...
def _run_init(root: Path, args: argparse.Namespace) -> int:
    ...
    template = resources.files("arnold.pipelines.megaplan.cloud.templates").joinpath("cloud.yaml.tmpl")
...
def _run_chain_wrapper(root: Path, args: argparse.Namespace, spec: CloudSpec, provider) -> int:
    from arnold.pipelines.megaplan import chain as chain_module
    from arnold.pipelines.megaplan.cloud.preflight import resolve_cloud_chain_runtime_dependencies
...
def _run_supervise_tick(root: Path, args: argparse.Namespace, spec: CloudSpec, provider) -> int:
    ...
    from arnold.pipelines.megaplan.cloud.supervise import cloud_supervise_tick  # noqa: F811
