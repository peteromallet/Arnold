...
def _entrypoint_template() -> Template:
    text = resources.files("arnold.pipelines.megaplan.cloud.templates").joinpath("entrypoint.sh.tmpl").read_text(encoding="utf-8")
    return Template(text)


def _render_resource_template(name: str, values: dict[str, str]) -> str:
    text = resources.files("arnold.pipelines.megaplan.cloud.templates").joinpath(name).read_text(encoding="utf-8")
    return Template(text).safe_substitute(values)


def _dockerfile_template() -> Template:
    text = resources.files("arnold.pipelines.megaplan.cloud.templates").joinpath("Dockerfile").read_text(encoding="utf-8")
    return Template(text)
...
def materialize_deploy_dir(spec: CloudSpec, dest: Path) -> None:
    ...
    templates = resources.files("arnold.pipelines.megaplan.cloud.templates")
    wrappers = resources.files("arnold.pipelines.megaplan.cloud.wrappers")
