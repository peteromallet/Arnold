...
# graph that the native runtime executes.
...
def build_pipeline() -> Pipeline:
    """Return the canonical native-backed Megaplan planning :class:`Pipeline`.

    Compiles the native ``@pipeline("megaplan")`` declaration via
    :func:`compile_pipeline` and projects it through phase-keyed
    :func:`project_graph`.  The returned shell carries the compiled
    ``native_program`` so the canonical runtime dispatches natively.
    The private :func:`_build_legacy_graph_pipeline` remains available
    for explicit parity checks under ``ARNOLD_FORCE_LEGACY=true``.
