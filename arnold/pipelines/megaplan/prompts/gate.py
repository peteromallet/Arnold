...
def _gate_prompt(
    ...
    return textwrap.dedent(
        ...
        Read this file first — it contains the expected JSON structure with pre-populated `known_flag_ids`.
...
def _write_gate_template(
    ...
    Pre-populates ``known_flag_ids`` from the flag registry so the model
    can reference concrete flag IDs in ``flag_resolutions`` without
    inventing them from scratch.  The model fills the remaining fields.
    """
    import json

    known_flag_ids: list[str] = []
    try:
