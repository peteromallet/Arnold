...
_CREATIVE_PROMPT_EXPORTS: dict[str, tuple[str, str]] = {
    "_critique_creative_prompt": (
        "arnold.pipelines.megaplan.pipelines.creative.prompts.critique_creative",
        "_critique_creative_prompt",
    ),
    "_critique_joke_prompt": (
        "arnold.pipelines.megaplan.pipelines.creative.prompts.critique_joke",
        "_critique_joke_prompt",
    ),
    "_execute_creative_batch_prompt": (
        "arnold.pipelines.megaplan.pipelines.creative.prompts.execute_creative",
        "_execute_creative_batch_prompt",
    ),
    ...
    "_execute_joke_batch_prompt": (
        "arnold.pipelines.megaplan.pipelines.creative.prompts.execute_joke",
        "_execute_joke_batch_prompt",
    ),
    ...
    "_prep_joke_prompt": (
        "arnold.pipelines.megaplan.pipelines.creative.prompts.prep_joke",
        "_prep_joke_prompt",
    ),
    "_revise_creative_prompt": (
        "arnold.pipelines.megaplan.pipelines.creative.prompts.revise_creative",
        "_revise_creative_prompt",
    ),
    "_revise_joke_prompt": (
        "arnold.pipelines.megaplan.pipelines.creative.prompts.revise_joke",
        "_revise_joke_prompt",
    ),
}


def _load_creative_prompt_export(name: str) -> Any:
    module_name, attr_name = _CREATIVE_PROMPT_EXPORTS[name]
...
def __getattr__(name: str) -> Any:
    if name in _CREATIVE_PROMPT_EXPORTS:
        return _load_creative_prompt_export(name)
...
def _execute_doc_prompt(
    ...
    """Lazy adapter that avoids a doc-prompt package import cycle."""
    from arnold.pipelines.megaplan.pipelines.doc.prompts.execute_doc import (
        _execute_doc_prompt as _impl,
    )
...
def _execute_doc_batch_prompt(
    ...
    """Lazy adapter that avoids a doc-prompt package import cycle."""
    from arnold.pipelines.megaplan.pipelines.doc.prompts.execute_doc import (
        _execute_doc_batch_prompt as _impl,
    )
...
def _execute_batch_prompt(
    ...
    if is_creative_mode(state):
        from arnold.pipelines.megaplan.pipelines.creative.prompts.execute_creative import (
            _execute_creative_batch_prompt,
        )
...
def _resolve_builder(
    ...
    if is_creative_mode(state) and get_form(creative_form_id(state) or "joke").id == "joke":
        if step == "prep":
            from arnold.pipelines.megaplan.pipelines.creative.prompts.prep_joke import (
                _prep_joke_prompt,
            )
        ...
        if step == "revise":
            from arnold.pipelines.megaplan.pipelines.creative.prompts.revise_joke import (
                _revise_joke_prompt,
            )

            return _revise_joke_prompt
        if step == "execute":
            from arnold.pipelines.megaplan.pipelines.creative.prompts.execute_joke import (
                _execute_joke_prompt,
            )
    ...
    if is_creative_mode(state):
        ...
        if step == "revise":
            from arnold.pipelines.megaplan.pipelines.creative.prompts.revise_creative import (
                _revise_creative_prompt,
            )
