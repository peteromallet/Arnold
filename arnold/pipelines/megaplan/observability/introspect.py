...
def _get_profiles_list() -> list[str]:
    ...
    try:
        result = subprocess.run(
            [sys.executable, "-m", "arnold.pipelines.megaplan", "config", "profiles", "list"],
            capture_output=True,
            text=True,
            timeout=10,
    ...
    try:
        import arnold.pipelines.megaplan.profiles as megaplan_profiles
...
def _parse_decision_skill_profiles() -> list[str]:
    ...
    try:
        import arnold.pipelines.megaplan.data as megaplan_data
