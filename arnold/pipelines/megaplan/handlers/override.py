...
def _override_adopt_execution(
    ...
    reason = args.reason or "Adopted complete execution artifact after post-worker engine drift."
    timestamp = now_utc()
...
def _override_set_robustness(
    ...
    from arnold.pipelines.megaplan.profiles import ROBUSTNESS_ACCEPTED, normalize_robustness
...
def _current_phase_spec(phase: str, state: PlanState, root: Path) -> str:
    ...
    if profile_name:
        try:
            from arnold.pipelines.megaplan.profiles import load_profiles, resolve_profile
...
def _override_set_vendor(root: Path, plan_dir: Path, state: PlanState, args: argparse.Namespace) -> StepResponse:
    ...
    from arnold.pipelines.megaplan.profiles import _swap_premium_spec
...
def handle_override(root: Path, args: argparse.Namespace) -> StepResponse:
    ...
    if action in {"refresh-engine-pin", "adopt-execution"}:
        pass
