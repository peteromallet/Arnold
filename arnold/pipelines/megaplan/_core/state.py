...
def restorable_boundary(operation: str) -> Iterator[None]:
    ...
    from arnold.pipelines.megaplan.drivers import current_substrate
...
def apply_session_update(
    ...
    *,
    ...
    from arnold.pipelines.megaplan.workers import update_session_state
...
def set_active_step(
    ...
    *,
    ...
    if mode == "persistent":
        from arnold.pipelines.megaplan.workers import session_key_for
...
def append_history(state: PlanState, entry: HistoryEntry) -> None:
    ...
    try:
        from arnold.pipelines.megaplan._pipeline.flags import effect_ledger_on
        from arnold.pipelines.megaplan.observability.effect_enforcement import journal_then_execute
