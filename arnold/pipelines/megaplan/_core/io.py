...
def atomic_write_text(path: Path, content: str, *, _plan_dir: Path | None = None) -> None:
    ...
    if _plan_dir is not None:
        try:
            ...
            try:
                from arnold.pipelines.megaplan.handlers.shared import _warn_best_effort_emit_failure
...
def read_plan_state_cached(plan_dir: Path, *, mode: "PlanStateReadMode") -> Any:
    ...
    if mode == "authority":
        ...
        from arnold.pipelines.megaplan._pipeline.flags import r1_authority_on
        if r1_authority_on():
            from arnold.pipelines.megaplan.observability.fold import rebuild_state_from_wal
...
def render_final_md(finalize_data: dict[str, Any], *, phase: str = "finalize") -> str:
    from arnold.pipelines.megaplan.orchestration.plan_contracts import render_contract_markdown
...
def detect_available_agents() -> list[str]:
    ...
    import arnold.pipelines.megaplan._core as _core_pkg
