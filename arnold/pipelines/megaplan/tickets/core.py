...
from arnold.pipelines.megaplan.schemas import Ticket, TicketEpicLink
...
def is_cloud_store(store: Store) -> bool:
    ...
    """
    from arnold.pipelines.megaplan.store.db import DBStore  # lazy to avoid import cycles
...
def _resolve_store() -> Store | None:
    ...
    if backend == "db":
        from arnold.pipelines.megaplan.store import DBStore, require_actor_id, resolve_actor_id
