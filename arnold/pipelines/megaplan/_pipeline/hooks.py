...
:class:`~arnold.pipelines.megaplan._pipeline.types.StateDeltaConflict` on
...
``arnold.pipelines.megaplan._pipeline.executor`` to avoid a circular dependency.
"""
...
class MegaplanExecutorHooks(NullExecutorHooks):
    ...
    * ``budget`` — opaque budget object (may be ``None``).
    ...
    :func:`~arnold.pipelines.megaplan._pipeline.adapter.to_canonical_step_context`.
    """
    ...
    def on_step_end(
        ...
        from arnold.pipelines.megaplan._core.activation import LifecycleState as _LS
        from arnold.pipelines.megaplan._pipeline.flags import activation_emit_on as _aeo
