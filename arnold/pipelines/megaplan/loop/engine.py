...
from arnold.pipelines.megaplan.loop.git import git_commit, git_revert, parse_metric
from arnold.pipelines.megaplan.loop.prompts import build_loop_prompt
from arnold.pipelines.megaplan.loop.types import IterationResult, LoopSpec, LoopState, Observation
from arnold.pipelines.megaplan.profiles import apply_profile_expansion
from arnold.pipelines.megaplan.workers import WorkerResult, resolve_agent_mode, run_step_with_worker, update_session_state
from arnold.pipelines.megaplan.runtime.process import kill_group as _kill_process_group
...
def init_loop(name: str, goal: str, command: str, project_dir: str, args: argparse.Namespace | None) -> LoopState:
    ...
    config = {"project_dir": str(Path(project_dir).resolve()), "robustness": getattr(normalized, "robustness", "thorough")}
