...
def _capture_execute_payload(
    *,
    ...
    payload = _normalize_execute_capture_payload(payload, project_dir=project_dir)
    ...
    captured = dict(outcome.legacy_payload)
    # Re-inject head_sha after the generic model seam has projected the payload,
    # because the seam's execute normalizer drops unknown fields and authority
    # readers need head_sha to validate task evidence.
    head_sha = _best_effort_git_head(project_dir)
    if head_sha is not None:
        for task_update in captured.get("task_updates") or []:
            if isinstance(task_update, dict):
                task_update.setdefault("head_sha", head_sha)
    return captured


def _normalize_execute_capture_payload(
    payload: dict[str, Any],
    *,
    project_dir: Path,
) -> dict[str, Any]:
    ...
    for item in normalized.get("task_updates") or []:
        ...
        if head_sha is not None:
            update.setdefault("head_sha", head_sha)
...
def _run_and_merge_batch(
    *,
    ...
    # Defensive re-normalize: ensure every execution-batch artifact carries
    # canonical statuses and head_sha/code_hash evidence context, even if the
    # worker returned a recovered/repaired payload.
    payload = _normalize_execute_capture_payload(payload, project_dir=project_dir)
