    ...
    *,
    ...
    for key, value in metadata.items():
        if key == "vendor_locked":
            ...
            from arnold.pipelines.megaplan.types import CRITIC_MODEL_CHOICES
...
def _pipeline_local_profiles_dir(pipeline_name: str, *, builtin: bool = True) -> Path | None:
    ...
    import arnold.pipelines.megaplan._pipeline as megaplan_pipeline

    if builtin:
        package_file = Path(megaplan_pipeline.__file__).resolve()
        pipelines_root = package_file.parent.parent / "pipelines"
    else:
