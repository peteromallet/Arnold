...
if TYPE_CHECKING:
    from arnold.pipelines.megaplan.drivers import Substrate

from arnold.pipelines.megaplan._core import active_phase_name, find_plan_dir
...
def _run_override_command(
    ...
    *,
    ...
    try:
        from arnold.pipelines.megaplan.cli import load_plan
...
def _status(
    ...
    *,
    ...
    from arnold.pipelines.megaplan.cli.status_view import handle_status
...
def _active_phase_already_completed(
    ...
    if plan_dir is None or not phase or not current_state:
        return False
    try:
        from arnold.pipelines.megaplan._core.workflow import phase_produced_state
    ...
    while iteration < max_iterations:
        ...
        if (
            ...
            if last_review_marker is not None:
                ...
                if nonconverging_tasks:
                    from arnold.pipelines.megaplan.auto_escalation import CATEGORY_POLICY, FailureCategory
        ...
        if (
            ...
            _record_failure(
                ...
                last_artifact=_latest_artifact_name(plan_dir),
                suggested_action=(
                    "Resolve the Megaplan engine isolation drift before resuming. "
                    "Use the recorded drift details to either run from the pinned engine "
                    "or intentionally refresh the engine pin."
                ),
                metadata={
                    "error": error_code,
