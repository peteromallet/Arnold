...
def run_hermes_step(
    ...
    *,
    ...
    if reg_mode == "file_fill":
        ...
        if template_seed_text is not None and output_path.parent != plan_dir:
        ...
        elif step == "critique" and template_seed_text is None and not (output_path.exists()):
            # Preserve custom critique template writer (pre-populates IDs).
            ...
            output_path = _write_critique_template(
                plan_dir, state, select_active_checks(state, robustness, plan_dir=plan_dir),
            )
        elif step == "review" and template_seed_text is None and not (output_path.exists()):
            # Preserve custom review template writer (pre-populates task/sense-check IDs).
            ...
            # Generic seed for gate, finalize, critique_evaluator, critique, review
            # (with seed_text already handled above).
            output_path.parent.mkdir(parents=True, exist_ok=True)
        ...
        if has_file_tool:
            _append_file_fill_instructions(
                output_path, pre_populated=(step == "review")
            )
        else:
    ...
    def _rewrite_output_template(current_output_path: Path | None) -> Path | None:
        ...
        if _retry_reg is not None and _retry_reg.mode == "file_fill":
            if step == "critique":
                from arnold.pipelines.megaplan._core import configured_robustness
                from arnold.pipelines.megaplan.prompts import _write_critique_template
                robustness = configured_robustness(state)
