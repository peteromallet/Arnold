...
def clean_parsed_payload(payload: dict, schema: dict, step: str) -> None:
    ...
    if step == "execute":
        ...
        _normalize_execute_task_statuses(payload)
...
# Execute workers occasionally invent their own vocabulary for task status
# (e.g. "passed"). Normalize common synonyms to the canonical enum so the
# structural audit stays strict without failing on harmless lexical variation.
_EXECUTE_STATUS_SYNONYMS: dict[str, str] = {
    # Terminal statuses are normalized to the canonical "done" so downstream
    # consumers (structural audit, authority readers, chain gate) see one label.
    "done": "done",
    "completed": "done",
    "passed": "done",
    "pass": "done",
    ...
    "failure": "blocked",
    "error": "blocked",
    # Non-terminal planning statuses preserved for in-flight tracking.
    "pending": "pending",
    "todo": "pending",
    "not_started": "pending",
    "in_progress": "pending",
...
def _normalize_execute_task_statuses(payload: dict) -> None:
    for update in payload.get("task_updates", []):
        ...
        if not isinstance(raw, str):
            continue
        canonical = _EXECUTE_STATUS_SYNONYMS.get(raw.strip().lower())
