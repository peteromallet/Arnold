...
from arnold.pipelines.megaplan.runtime.execution_environment import (
    ...
    consume_engine_write_isolation_waiver,
    isolation_cli_error,
...
def _guard_mutating_worker_launch(step: str, state: PlanState, root: Path) -> None:
    ...
    try:
        ...
        if exc.code != "engine_write_isolation_unverified":
            raise
        waiver = consume_engine_write_isolation_waiver(
            state.get("meta") if isinstance(state, dict) else None,
            env,
            phase=step,
        )
        proof = {
            ...
            "diagnostic": "engine_write_barrier_bypassed_by_scoped_operator_waiver",
...
def _verify_engine_after_mutating_worker(
    ...
    same_checkout_dogfood_signature_drift = (
        ...
        and before_env.engine_commit == after_env.engine_commit
        and getattr(after_env, "engine_dirty", False)
        and str(getattr(after_env, "target_root", "")) == str(after_env.engine_root)
        and str(getattr(after_env, "work_dir", "")) == str(after_env.engine_root)
    )
    ...
    *,
    ...
    for root, source in roots:
        ...
        if overlap != "disjoint":
            ...
            waiver_invalid_reason = _engine_overlap_waiver_invalid_reason(
                waiver,
                env,
                phase=phase or "",
                now=None,
            )
            same_phase_consumed_waiver = (
                waiver_invalid_reason == "consumed"
                and phase is not None
                and isinstance(waiver, dict)
                and waiver.get("consumed_by_phase") == phase
            )
            if waiver_invalid_reason is None or same_phase_consumed_waiver:
            ...
            raise isolation_cli_error(
                ...
                extra={
                    ...
                    "waiver_invalid_reason": waiver_invalid_reason,
                },
            )
        filtered.append(root_str)
...
def _codex_step_cost(
    ...
    """
    from arnold.pipelines.megaplan.pricing.codex import cost_from_codex_usage_dict
...
def _codex_model_flag(model: str | None) -> list[str]:
    ...
    *model* is ``None`` (codex uses its configured default).
    ...
    if model is None:
        return []
    from arnold.pipelines.megaplan.types import _is_codex_model_name
