...
# ── Explicit opt-in native entrypoint ───────────────────────────────────
...
def run_native(
    *,
    ...
    This is the **explicit per-call opt-in entrypoint** for native
    execution.  It requires both:

    * the environment guard ``ARNOLD_NATIVE_RUNTIME=1`` (enforced inside
      :func:`run_native_pipeline`), AND
    * the caller to deliberately invoke this function (per-call opt-in).

    Without this function there is no accidental native path — graph
    execution via :func:`arnold.pipelines.folder_audit.build_pipeline`
    remains the default regardless of the environment flag.
    ...
    NativeRuntimeDisabledError
        When ``ARNOLD_NATIVE_RUNTIME`` is not set to ``'1'``.
    """
