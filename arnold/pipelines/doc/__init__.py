...
``arnold/pipelines/doc/prompts/``. The public Step shells in
``arnold.pipelines.doc.steps`` carry the canonical ``prompt_key`` slots
...
from arnold.pipelines.doc.prompts import DOC_PROMPT_BUNDLE
from arnold.pipelines.doc.steps import (
    AssemblyStep,
    CritiqueStep,
