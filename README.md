...
- **Different models per phase** — pick a named profile (`python -m arnold_pipelines.megaplan init --profile <name>`) or override one phase (`--phase-model execute=claude`). Inspect with `python -m arnold_pipelines.megaplan config profiles list`; define your own in `.megaplan/profiles.toml`. Built-ins include all-Claude, Partnered Codex, all-DeepSeek, and all-open (Kimi/GLM via OpenRouter).
- **Cloud runs** — `python -m arnold_pipelines.megaplan cloud` runs a plan (or a whole chain) on a remote runner over SSH (e.g., the Hetzner agentbox) with a persistent workspace volume, so it outlives your terminal. Ask your agent to `python -m arnold_pipelines.megaplan cloud bootstrap <idea>`. See [docs/cloud.md](docs/cloud.md).
...
- **Custom omp agents & residents** — install the branded `arnold` named agent for omp (`agentbox install-omp-agent arnold`), customize name, description, prompt, or model, and scaffold a Discord resident bot into any repo with `agentbox new-resident <name> --repo <path>`. See [docs/custom-resident-agents.md](docs/custom-resident-agents.md).
...
- **Robustness levels** — `--robustness bare|light|full|thorough|extreme` dials workflow depth and parallelism, from bare (`plan → finalize → execute`) up to parallel critique + parallel review. Legacy aliases (`tiny|standard|robust|superrobust`) are still accepted.
...
- **Custom Discord residents** — scaffold a five-file resident operator bot into any repository (`python -m agentbox new-resident <name> --repo /path/to/repo`), with attested custody for both the project checkout and the imported Arnold runtime. See [docs/custom-resident-agents.md](docs/custom-resident-agents.md).
