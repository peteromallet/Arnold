...
def _write_verdict(
    ...
    verdict_path.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")
    return verdict_path


class JudgeClarity:
    """Deterministic clarity judge retained locally for the M5 eval wrapper."""
    ...
    def run(self, ctx: StepContext) -> StepResult:
        text = Path(ctx.inputs["doc"]).read_text(encoding="utf-8")
        lengths = [len(sentence.split()) for sentence in _split_sentences(text)]
        if len(lengths) < 2:
            stdev_val = 0.0
            score = 1.0
        else:
