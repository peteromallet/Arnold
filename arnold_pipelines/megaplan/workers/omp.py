...
def omp_thinking_level(
    ...
    ``None``/``auto`` leave the RPC client unset (omp default).  ``off``
    disables thinking.  Levels outside the per-model ladder are clamped to the
    nearest accepted tier.  Fireworks maps ``minimal`` to provider ``none``
    (thinking off).
    """
...
def _worktree_mutation_fingerprint(work_dir: Path) -> str | None:
    """Return a fingerprint of the worktree's uncommitted/untracked state.
...
def _prompt_phase_tools(step: str, read_only: bool) -> tuple[str, ...] | None:
    ...
    Execute gets the default full toolset (bash + file tools + …); every other
    phase is read-oriented and only needs read/edit/write alongside the
    structured-output host tool.
    """
    if read_only or step in _EXECUTE_STEPS:
        return None if step in _EXECUTE_STEPS else ("read", "edit", "write")
...
def run_omp_step(
    ...
    *,
    ...
    if not any(os.environ.get(name) for name in alias_names) and provider != "kimi-code":
        # Kimi Code OAuth can be tokenless (browser-less persisted OAuth);
        # other routes need at least one key.
        raise CliError(
            "authentication",
