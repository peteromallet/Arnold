...
def _install_content_tool_call_normalizer(AIAgent) -> None:
    ...
    def _without_null_tool_fields(api_kwargs: dict) -> dict:
        """Return provider kwargs without optional tool fields set to ``None``.

        OpenAI accepts ``tools=null``, but Fireworks validates ``tools`` as a
        list when the field is present.  Finalize intentionally has no tools,
        so preserve the semantic distinction by omitting null-valued tool
...
def _terminal_finish_reason(result: dict) -> str | None:
    """Return the terminal assistant finish reason from a provider result."""

    direct = result.get("finish_reason")
...
def _raise_for_terminal_provider_failure(result: dict, *, step: str) -> None:
    """Surface terminal transport/output failures before output parsing.

    In particular, a length-truncated finalize graph is not a structural JSON
    candidate.  Sending its surviving fragment to the structural-repair turn
    can produce a schema-shaped but semantically empty graph (for example
    ``tasks: []``), obscuring the provider-capacity failure and wasting another
    model call.
    """
    ...
    if step == "finalize" and output_capacity_exhausted:
        raise CliError(
            ...
            (
                "Hermes provider exhausted its output budget for step "
                "'finalize'; the task graph is incomplete and must be retried "
                "on a larger-output route or with a bounded finalize prompt."
            ),
            extra={"provider_failure_category": "capacity"},
        )
