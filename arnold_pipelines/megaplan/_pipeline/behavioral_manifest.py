...
from arnold_pipelines.megaplan._pipeline import registry as _registry
...
def _build_runtime_topology_projection(
    ...
    *,
    ...
    for stage_name, stage in sorted(pipeline.stages.items()):
        stage_kind = "parallel" if isinstance(stage, ParallelStage) else "stage"
        step_entries: list[Mapping[str, object]]
        if isinstance(stage, ParallelStage):
            step_entries = [_step_projection(step) for step in stage.steps]
            join = _callable_name(stage.join)
            max_workers = stage.max_workers
        elif isinstance(stage, Stage):
            step_entries = [_step_projection(stage.step)]
            join = None
            max_workers = None
        else:
            step_entries = []
...
def _resolve_imported_module_paths(node: ast.ImportFrom, module_file: Path) -> tuple[Path, ...]:
    ...
    elif module.startswith("arnold_pipelines.megaplan.pipelines."):
