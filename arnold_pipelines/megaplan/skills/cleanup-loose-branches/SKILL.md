...
- **Reachable** → mini-survey via `megaplan cloud exec` (read-only, `git -C $WS ...`):
  ```bash
  # Resolve $WS from cloud.yaml repo.workspace; default /workspace/app
  WS=$(yq '.repo.workspace // "/workspace/app"' cloud.yaml 2>/dev/null \
       || awk -F: '/^\s*workspace:/ {gsub(/[ "]/,"",$2); print $2; exit}' cloud.yaml)
  WS=${WS:-/workspace/app}
  megaplan cloud exec "git -C $WS rev-parse --abbrev-ref HEAD; git -C $WS status --porcelain; git -C $WS stash list; git -C $WS for-each-ref --format='%(refname:short)|%(upstream:track)|%(contents:subject)' refs/heads/"
  megaplan cloud exec "tail -40 $WS/.megaplan/cloud-chain.log 2>/dev/null"
  ```
  Three risk buckets: **cloud-only branches** (never pushed), **branch ahead of origin** (chain interrupted mid-push), **dirty volume**.
- **Not deployed / down** → surface "dormant cloud — needs deploy to inspect, or destroy if done." Do **not** deploy just to survey; ask first.

Each cloud workspace is its own section. Recs: `port-and-down` / `port-and-destroy` / `down` / `destroy` / `keep`. `destroy` is non-recoverable at the provider level — same blast radius as `rm -rf` on a sibling variant.
...
- **Cloud workspaces + Codespaces** (per-workspace section)
...
End: "Want to walk through these one at a time?" — wait for explicit go-ahead.

## Phase 2 — walk-through (per-item, explicit approval)
...
## Deep-investigation mode (multi-epic repos, heavy ambiguity)
...
### Five survey upgrades this mode adds
...
cherry-pick), update the durable plan doc before asking for destructive approval.

### Write a consolidation plan markdown document

For any non-trivial cleanup (multiple epics, uncommitted keepers, supersession decisions), **write a durable
plan doc** (e.g. `docs/megaplan/loose-work-consolidation-plan.md`) — the artifact outlives the chat and makes
the reasoning auditable. Recommended sections:
...
Leave explicit **placeholders** (`<!-- VERDICT PENDING -->`) for in-flight agent verdicts and fill them in as
results land, fixing any downstream rows the verdict changes. The doc, not the chat, is the source of truth
for what gets preserved and deleted.
