...
For epics, `<launcher> brief epic` also scaffolds `NORTHSTAR.md` beside `chain.yaml` and declares it through `anchors.north_star`. Edit it before `chain start`. Use the North Star for the durable destination and milestone briefs for local scope.
