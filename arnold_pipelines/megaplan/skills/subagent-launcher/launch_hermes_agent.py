#!/usr/bin/env python3
"""Launch a hermes-backed agentic subagent through omp (Oh My Pi).
...
hermes runtime — the same migration `origin/omp-migration` performs for
megaplan workers.
...
def run(
    ...
    _eprint(
        f"[launch_hermes_agent] model={model} → resolved={selector} "
        f"toolsets={toolset_list or '(none)'} "
        f"max_tokens={max_tokens} context_budget_tokens={context_budget_tokens or '(auto)'}"
    )
    if thinking is not None:
        _eprint(f"[launch_hermes_agent] thinking={thinking}")
    if toolset_list:
        _eprint(
            "[launch_hermes_agent] NOTE: omp gives the full toolset (Bash, Read, "
            "Edit, web, …); the file/web/terminal subset is a superset here."
        )
    if max_tokens and max_tokens != 65536:
        _eprint(
            "[launch_hermes_agent] NOTE: --max-tokens is informational; omp uses "
            "the model's native output ceiling."
        )
    if context_budget_tokens is not None:
        _eprint(
            "[launch_hermes_agent] NOTE: --context-budget-tokens is not supported "
            "through omp (auto-compaction handles context)."
        )
    if session_id:
        _eprint(
            f"[launch_hermes_agent] NOTE: --session-id={session_id!r} ignored — "
    ...
    _eprint(f"[launch_hermes_agent] cwd={cwd or Path.cwd()}")
    try:
    ...
    _eprint(f"[launch_hermes_agent] done in {elapsed:.1f}s (exit={completed.returncode})")
...
def _write_metadata(
    ...
    *,
    ...
    try:
        ...
        _eprint(f"[launch_hermes_agent] warning: could not write metadata: {exc}")
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    # caller (or by the omp-migration worker) — note it and continue.
    if (
        ...
        _eprint(
            "[launch_hermes_agent] NOTE: managed-run env detected; the omp-backed "
