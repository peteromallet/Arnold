...
class RepairCustodyProjection(TypedDict):
    ...
    coalesced_without_live_owner: list[dict[str, Any]]
    request_count: int
    claim_count: int
    attempt_count: int
...
def project_repair_custody(
    *,
    ...
    live_owner_request_ids = set(active_claim_request_ids) | {
        attempt["request_id"]
        for attempt in attempts
        if attempt["request_id"] and attempt["terminal"] is False
    }
    coalesced_without_live_owner = [
        ...
        and _as_text(record.get("related_request_id")) in current_request_ids
        and _as_text(record.get("related_request_id")) not in live_owner_request_ids
    ]
    request_status_counts: dict[str, int] = {}
