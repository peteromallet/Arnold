...
def enqueue_audit_repair_request(
    ...
    *,
    ...
    P2 (optional): when a ``RuntimeTransitionWriter`` and a
    ``chain_spec_sha256`` contract digest are supplied, the declared deviation
    and the considered repair fallback are appended to the incident ledger
    BEFORE the request is created.  The append is synchronous and never
    swallowed: a policy rejection (ValueError) or journal write failure
    (OSError) aborts the enqueue — no event means no repair request.
    """
    incident_audit = (
    ...
    if transition_writer is not None:
        if not chain_spec_sha256:
            raise ValueError(
                "six-hour audit repair request requires chain_spec_sha256 "
                "when a runtime transition writer is supplied"
            )
        try:
