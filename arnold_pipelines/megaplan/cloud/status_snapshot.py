...
def write_cloud_status_snapshot(
    ...
    *,
    ...
    # A caller supplying an explicit snapshot path owns its disposable
    # directory; never rotate that test/alternate snapshot into the live
    # /workspace default.  The canonical default path retains its legacy
    # rotation target.
    if (
        previous is not None
        and previous_path == DEFAULT_PREVIOUS_SNAPSHOT_PATH
        and target != DEFAULT_SNAPSHOT_PATH
    ):
        previous = target.parent / DEFAULT_PREVIOUS_SNAPSHOT_PATH.name
