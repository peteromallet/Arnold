...
def _chain_milestone_total(
    chain_state: Mapping[str, Any],
    remote_spec: Path | None,
) -> int:
    ...
    if remote_spec is None:
        return 0
    try:
        return len(chain_spec.load_spec(remote_spec).milestones)
...
def resolve_current_target(
    ...
    *source_cursor_vector* is an optional M9 canonical projection cursor
    ...
    if (
        run_kind == "chain"
        and plan_name
        and plan_current_state
        and not _is_terminal_plan_state(plan_current_state)
        and _is_terminal_plan_state(chain_last_state)
    ):
        stale_evidence.append(
            _artifact(
    ...
    # Source-cursor metadata is a deterministic description of the source
    # record, not a timestamped read event.  Use the marker's stable mtime so
    # identical filesystem state produces identical resolver output.
    try:
        _observed_at_epoch_ms = marker_path.stat().st_mtime * 1000
    except OSError:
        _observed_at_epoch_ms = 0.0
    ...
    return {
        ...
        "chain_state": {
            ...
            "milestone_total": _chain_milestone_total(chain_state, remote_spec),
