...
def run_tiebreaker_cli(root: Path, args: argparse.Namespace) -> int:
    ...
    if action == "decide":
        from arnold_pipelines.megaplan.handlers import handle_tiebreaker_decide
...
def _run_tiebreaker_audit(root: Path, plan_name: str | None) -> int:
    from arnold_pipelines.megaplan.orchestration.plan_audit import (
    ...
    if plan_name:
        ...
        from arnold_pipelines.megaplan.orchestration.plan_audit import _compute_totals
