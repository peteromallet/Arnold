...
from arnold_pipelines.megaplan.execute import _resolve_execute_approval_mode
from arnold_pipelines.megaplan.execute.batch import _resolve_max_tasks_per_batch
from arnold_pipelines.megaplan.orchestration.execution_evidence import (
    validate_execution_evidence,
)
from arnold_pipelines.megaplan.orchestration.iteration_pressure import (
...
from arnold_pipelines.megaplan._core import compute_global_batches, split_oversized_batches
...
def _report_blocked_lifecycle(ctx: _PolicyContext, spec: PolicySettingSpec) -> ReportValue:
    ...
    return _effective(
        ...
        source=f"{retry_source} + arnold_pipelines.megaplan.orchestration.recovery_policy.RecoveryPolicy",
...
def _report_batch_transitions(ctx: _PolicyContext, spec: PolicySettingSpec) -> ReportValue:
    ...
    if ctx.args is not None and getattr(ctx.args, "max_tasks_per_batch", None) is not None:
        ...
        source = "arnold_pipelines.megaplan._core.get_effective(execution.max_tasks_per_batch)"
...
def _report_evidence_requirements(ctx: _PolicyContext, spec: PolicySettingSpec) -> ReportValue:
    ...
    return _effective(
        ...
        source="arnold_pipelines.megaplan.orchestration.execution_evidence.validate_execution_evidence",
...
def _report_tier_selection(ctx: _PolicyContext, spec: PolicySettingSpec) -> ReportValue:
    ...
    return _effective(
        ...
        source=(
            ...
            if tier_map
            else "arnold_pipelines.megaplan.execute._binding.tier.select_batch_tier"
        ),
...
def _report_iteration_pressure(ctx: _PolicyContext, spec: PolicySettingSpec) -> ReportValue:
    ...
    return _effective(
        ...
        source="arnold_pipelines.megaplan.orchestration.iteration_pressure.compute_iteration_pressure",
    )
...
SETTING_SPECS: tuple[PolicySettingSpec, ...] = (
    ...
    PolicySettingSpec(
        ...
        resolver_ref="arnold_pipelines.megaplan.execute._resolve_execute_approval_mode",
        reporter=_report_review_approval,
    ),
    PolicySettingSpec(
        key="blocked_lifecycle",
        owner="arnold_pipelines.megaplan.execute + arnold_pipelines.megaplan.orchestration",
        summary="Blocked-task retry intent plus orchestration retry budget.",
        resolver_ref="args.retry_blocked_tasks + arnold_pipelines.megaplan.orchestration.recovery_policy.RecoveryPolicy",
    ...
    PolicySettingSpec(
        ...
        resolver_ref="arnold_pipelines.megaplan.execute.batch._resolve_max_tasks_per_batch",
    ...
    PolicySettingSpec(
        ...
        resolver_ref="arnold_pipelines.megaplan.orchestration.execution_evidence.validate_execution_evidence",
    ...
    PolicySettingSpec(
        ...
        resolver_ref="arnold_pipelines.megaplan.handlers.execute._extract_execute_tier_map + megaplan.execute._binding.tier.select_batch_tier",
        reporter=_report_tier_selection,
    ),
    PolicySettingSpec(
        key="iteration_pressure",
        owner="arnold_pipelines.megaplan.orchestration",
        summary="Recurring critique pressure inferred from critique history artifacts.",
        resolver_ref="arnold_pipelines.megaplan.orchestration.iteration_pressure.compute_iteration_pressure",
