...
def _revise_prompt(state: PlanState, plan_dir: Path) -> str:
    ...
    return textwrap.dedent(
        ...
        - For code-mode plans with any `run_tests` success criterion, include or preserve a machine-readable `test_blast_radius` object in the structured output. If finalize feedback says scoped baseline metadata was missing, the revision must add this object rather than only adding prose validation commands.
