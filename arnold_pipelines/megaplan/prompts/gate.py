...
COMPACT_GATE_PLAN_MAX_CHARS = 40_000
COMPACT_GATE_META_MAX_CHARS = 25_000
COMPACT_GATE_FLAG_CONCERN_MAX_CHARS = 300
COMPACT_GATE_FLAG_DETAIL_MAX_CHARS = 120
COMPACT_GATE_SIGNALS_MAX_CHARS = 60_000


def _truncate_block(text: str, *, limit: int, label: str) -> str:
    """Bound a prompt block, keeping the head and a pointer to the full artifact."""
    if len(text) <= limit:
        return text
    omitted = len(text) - limit
    return (
        f"{text[:limit]}\n\n"
        f"[truncated {omitted:,} characters for compact gate prompt; full data "
        f"remains in the plan artifacts on disk ({label})]"
    )


def _compact_open_flags(open_flags: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Bound each open flag to decision-critical fields only.

    Every flag id, severity, status, category and weight is preserved (fail-closed:
    the gate must resolve every blocking flag).  Only the long prose fields
    (concern/evidence/revise_summary) are truncated; the full text stays in the
    flag registry / faults.json on disk.
    """
    ...
    for flag in open_flags:
        compact.append(
            {
                "id": flag.get("id"),
                "concern": _truncate_block(
                    str(flag.get("concern", "")),
                    limit=COMPACT_GATE_FLAG_CONCERN_MAX_CHARS,
                    label="flag registry / faults.json",
                ),
                "category": flag.get("category"),
                "severity": flag.get("severity", "unknown"),
                "status": flag.get("status"),
                "weight": flag.get("weight"),
                "evidence": _truncate_block(
                    str(flag.get("evidence", "")),
                    limit=COMPACT_GATE_FLAG_DETAIL_MAX_CHARS,
                    label="flag registry / faults.json",
                ),
                "revise_summary": _truncate_block(
                    str(flag.get("revise_summary", "")),
                    limit=COMPACT_GATE_FLAG_DETAIL_MAX_CHARS,
                    label="flag registry / faults.json",
                ),
            }
...
def compact_gate_prompt(
    ...
    *,
    ...
    """Build a bounded gate prompt when the normal gate prompt is too large.

    The gate must stay fail-closed: every blocking flag id, severity, status,
    category and weight is preserved, and the decision requirements are kept
    verbatim.  Only redundant or long prose blocks are bounded:
      * the plan and plan metadata are truncated to bounded excerpts,
      * the top-level ``unresolved_flags`` duplicate (same ids as
        ``signals.unresolved_flags``) is dropped,
      * resolved/unresolved flag prose is truncated per flag,
      * long open-flag evidence / revise summaries are truncated.
    The full artifacts remain on disk (``gate_signals_v5.json``, the flag
    registry, ``faults.json``, the plan file) and the worker may read them with
    file tools.
    """
    from ._shared import _render_contracts_block, _resolve_contract_context
    ...
    # Compact gate signals: drop the top-level unresolved_flags duplicate (it is
    # the same flag set as signals.unresolved_flags — proven same ids) and bound
    # the signals block.
    projected_signals = _gate_signals_for_prompt(gate_signals)
    if isinstance(projected_signals, dict):
        projected_signals.pop("unresolved_flags", None)
    signals_block = _truncate_block(
        json_dump(projected_signals).strip(),
        limit=COMPACT_GATE_SIGNALS_MAX_CHARS,
        label="gate_signals_v{iteration}.json".format(iteration=state["iteration"]),
    )
    ...
    if prompt_size_error:
        size_note = textwrap.dedent(
            f"""
            Normal gate prompt overflow:
            The first gate prompt was {prompt_size_error.get("prompt_size", "unknown")} characters with a limit of {prompt_size_error.get("max_chars", "unknown")}. This compact prompt is the fallback path; still produce a usable gate verdict with `flag_resolutions` for every blocking flag.
            """
        ).strip()
    ...
    return textwrap.dedent(
        ...
        Plan metadata (compact excerpt):
        {_truncate_block(json_dump(latest_meta).strip(), limit=COMPACT_GATE_META_MAX_CHARS, label="the plan metadata file")}

        Gate signals (compact; full signals remain in gate_signals_v{state["iteration"]}.json):
        {signals_block}

        {critique_checks_block}

        Unresolved significant flags (compact; full concern/evidence text is in the flag registry / faults.json on disk):
        {json_dump(_compact_open_flags(open_flags)).strip()}
