...
def _finalize_prompt(state: PlanState, plan_dir: Path, root: Path | None = None) -> str:
    ...
    return textwrap.dedent(
        ...
        - `tasks` must be an ordered array of task objects. Every task object must include:
          ...
          - `narrow_tests`: `{{ "selectors": [...], "budget_semantics": "elapsed_wall_clock_v2", "test_budget_seconds": 120, "max_runs": 2 }}`. Use at most 3 changed-behavior selectors; use zero `max_runs` for tasks that require no tests. `test_budget_seconds` is one positive task-level elapsed deadline; the harness, not the model, enforces remaining time as `min(command_timeout, remaining_budget)` at one production seam. Integration/full-suite checks belong to the harness. Selectors MUST be concrete repository-relative pytest path selectors — a file/module path such as `tests/core/store/test_canonical.py` or `tests/core/store/test_canonical.py::test_round_trip`. NEVER put a shell command in a selector: `pytest tests/x.py -q`, `python -m compileall -q astrid/...`, `bash -n ...`, `make ...`, flags, and directory-wide patterns are all INVALID and will be rejected.
