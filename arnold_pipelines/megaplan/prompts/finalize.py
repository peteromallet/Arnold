...
_FINALIZE_SELECTOR_HINT_LIMIT = 48


def _finalize_metadata_prompt_projection(metadata: dict[str, Any]) -> dict[str, Any]:
    """Bound high-cardinality planning evidence before model dispatch.

    Finalize only needs enough test-selection context to assign narrow selectors
    to tasks.  The handler reads the authoritative metadata artifact directly
    when it compiles validation jobs, so sending hundreds of import-graph
    selectors to the model is both redundant and dangerous: it can crowd the
    actual plan out of the context window.
    """

    projected = deepcopy(metadata)
    blast_radius = projected.get("test_blast_radius")
    if not isinstance(blast_radius, dict):
        return projected
    selectors = blast_radius.get("selectors")
    if not isinstance(selectors, list) or len(selectors) <= _FINALIZE_SELECTOR_HINT_LIMIT:
        return projected
    blast_radius["selectors"] = selectors[:_FINALIZE_SELECTOR_HINT_LIMIT]
    blast_radius["selector_projection"] = {
        "authoritative_count": len(selectors),
        "included_count": _FINALIZE_SELECTOR_HINT_LIMIT,
        "omitted_count": len(selectors) - _FINALIZE_SELECTOR_HINT_LIMIT,
        "purpose": (
...
def _finalize_clearance_prompt_projection(clearance: dict[str, Any]) -> dict[str, Any]:
    """Remove repeated receipt identity while preserving every obligation."""

    projected = {
        key: deepcopy(value)
        for key, value in clearance.items()
        if key not in {"resolutions", "source_receipts"}
    }
    resolutions = clearance.get("resolutions")
    if isinstance(resolutions, list):
        projected["resolutions"] = [
            {
                key: deepcopy(row[key])
                for key in ("finding_id", "disposition", "evidence")
                if isinstance(row, dict) and key in row
            }
            for row in resolutions
            if isinstance(row, dict)
        ]
    source_receipts = clearance.get("source_receipts")
    if isinstance(source_receipts, list):
        projected["source_receipt_count"] = len(source_receipts)
    projected["prompt_projection"] = (
        "Repeated artifact hashes and receipt bodies omitted. The handler binds "
        "the final graph against the complete authoritative clearance artifact."
    )
    return projected
...
def _finalize_prompt(state: PlanState, plan_dir: Path, root: Path | None = None) -> str:
    ...
    latest_meta = _finalize_metadata_prompt_projection(
    ...
    critique_clearance = (
        _finalize_clearance_prompt_projection(read_json(clearance_path))
