...
def _plan_prompt(
    ...
    return textwrap.dedent(
        ...
        - For code-mode plans, populate `test_blast_radius` with the model-owned proposal only: `strategy` ("scoped", "full", or "none"), `selectors` (array of `{{"kind": "path", "value": "<path>", "reason": "<why>"}}` objects), `changed_surfaces` (the same flat repo-relative path list as the top-level field), `full_suite_fallback` (boolean), and `rationale` (string). Do not invent `confidence`, `always_run`, or `import_graph`; the harness derives those fields deterministically after validating this proposal. The system merges your proposal with the deterministic floor; you cannot narrow below the floor, but you can widen with additional selectors or escalate to full.
