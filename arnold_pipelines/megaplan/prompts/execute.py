...
def _execute_batch_prompt(
    ...
    prompt = textwrap.dedent(
        ...
        - Each actionable task carries an admitted `write_set` and `narrow_tests` budget. Do not write outside its declared paths. Across the task, run at most the declared selectors and `max_runs`.
        - New plans use `narrow_tests.budget_semantics = "elapsed_wall_clock_v2"` with one positive `test_budget_seconds` deadline. The harness enforces that deadline as actual elapsed wall-clock time at one production seam (`execute.test_budget`): remaining budget is computed before each test subprocess; the subprocess timeout is `min(command_timeout, remaining_budget)`; admission stops when no positive budget remains, irrespective of the sum of declared command timeouts. `max_runs` is enforced at the same seam. Do not invent a second remaining-time formula.
        - Every pytest command, including every baseline or diagnostic run, MUST use exactly this command shape: `timeout <N> python3 -m pytest <selector> -q`. Replace `<N>` with integer seconds no larger than remaining budget and `<selector>` with only the admitted selector or selectors. Record the exact command in `commands_run`; put outcomes such as pass counts and durations in `executor_notes`, not after the command string. Fast commands with large declared timeouts are not rejected merely because timeout sums exceed the task budget.
        - Stored tasks with `max_seconds` and no `budget_semantics` are classified `declared_timeout_sum_v1`; the loader does not rewrite them. For those artifacts the sum of all N values MUST be <= `narrow_tests.max_seconds`.
        - Every diagnostic or pre-change run consumes the same elapsed budget. If the budget is exhausted, stop and return the task blocked with a residual checkpoint instead of widening or looping.
