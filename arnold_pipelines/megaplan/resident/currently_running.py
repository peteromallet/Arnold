...
_RECENT_COMPLETION_WINDOW = timedelta(hours=1)
...
def discover_recently_completed_managed_agents(
    ...
    The managed-agent inventory's bounded ``recent`` projection supplies the
    established truncation boundary.  A run is a completion only when its
    observed terminal status is explicitly ``completed``; failed, interrupted,
    cancelled, superseded, and unknown terminal outcomes are not inferred to
    be successful completions.  Both ends of the one-hour window are
    inclusive, while future timestamps are excluded.
    """
    ...
    return [
        ...
        and str(row.get("terminal_outcome") or "completed").casefold() == "completed"
        and (finished_at := _parse_utc_timestamp(row.get("finished_at"))) is not None
        and timedelta() <= reference - finished_at <= _RECENT_COMPLETION_WINDOW
    ]
...
def render_currently_running(
    ...
    if report.managed_agents_error:
        ...
        lines.append(
            _agents_heading(
                f"{len(agents)} live · {len(completed)} recently completed"
            )
        )
        ...
        lines.append(
            _subsection_heading("✅", "Recently completed", str(len(completed)))
        )
        if completed:
            lines.extend(_render_agent(row, now=now) for row in completed)
        else:
...
def _subsection_heading(icon: str, title: str, count: str) -> str:
    """Return a Discord Markdown H3 subordinate to a status section."""
...
def _effective_session_status(row: Mapping[str, Any]) -> str | None:
    """Prefer the canonical active-repair signal over a stale failure state."""
...
def _managed_agent_snapshot_time(
    ...
    """Choose the renderer's snapshot clock for rolling managed-agent status."""
