"""Feature flag helpers for megaplan.

Centralized single source of truth for MEGAPLAN_TYPED_PORTS and other
feature flags.  All callers must import from here rather than calling
``os.getenv`` directly.

This module was rehomed from ``arnold_pipelines.megaplan._pipeline.flags``
during the M3 burn-down (Step 4).  The env-gate flag helpers are kept here
while the plan flag-registry behaviour remains in
``arnold_pipelines.megaplan.flags``.
...
# ---------------------------------------------------------------------------
# Companion flags for the unified-dispatch strangler.
...
# ---------------------------------------------------------------------------
# Independent flags — these do NOT inherit from the unified-dispatch master
# gate and are controlled only by their own env var.
# ---------------------------------------------------------------------------
