"""Dynamic API key pooling for provider-backed agent execution.

Self-contained (B11 vendored) copy of the generic key pool plus the
megaplan-specific wrappers: blocking guards, the ``_pool`` singleton that
wires envelope/governor context, and the ``acquire_key`` / ``resolve_model``
helpers.  Imports only neutral modules (``arnold.security`` broker/llm-proxy
helpers, ``arnold.runtime.envelope``, megaplan runtime governor/types).
"""
