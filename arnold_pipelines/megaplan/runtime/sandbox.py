"""Project-directory sandbox for agent tool calls.

Background
----------
The agent worker runs megaplan phases through the omp RPC worker.
...
This module is self-contained (vendored B11): it owns both the
boundary-neutral path primitives and the tool-registry wrapper
machinery, so ``SANDBOX_CWD`` is a single process-wide ContextVar with
no SSoT re-export edge.  The old tool-registry wiring behind
``install_sandbox`` is not a live production path; omp pre-exec
...
def _load_tool_registry():
    ...
    ``sys.modules``.  Prefer that explicit shim when present; otherwise fall
    back to a top-level ``tools.registry`` module if one is importable.
    """
    import sys
...
def _unwrap_all_for_tests():
    ...
    in the ``_wrapped_originals`` dict and clears the ``_wrappers_installed``
    flag so tests that monkeypatch fake registries don't carry stale wrapped
    state between tests.
    """
    global _wrappers_installed
    _registry = _load_tool_registry()
