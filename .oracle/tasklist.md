# FROZEN — 2026-08-29 — Typed NBF worker admission, disposition, and scheduling control plane

> **Freeze only after a fresh complete GPT-5.6 Luna v7/v8 sense-check accepts the final plan-v8 and tasklist-v8 digests, followed by a separate fresh GPT-5.6 Sol freeze decision on those same digests.**

## Frozen references

- Settled plan v8 SHA-256: `0ec216cca92a6f99f7d73e78494a46f8acb08e22c506a58948640ea2c57421e1`
- Archived settled plan v7 SHA-256: `3e76fc3c9eeb8fbd6580d1217db341c1c3e9f16a4be3552eadddbef2ccd9276f`
...
- Full schemas, transition rules, crash semantics, prohibited patterns, exact-SHA evidence contract, and completion conditions remain authoritative in settled-plan v8, especially §§4–5 and §§10–12. This tasklist does not supersede them.
...
- The temporary user Sol authorization recorded in `agent_goal.md` remains historical authorization bookkeeping and does not silently change these frozen Luna executors or the Luna-then-Sol freeze order.
...
Batch 3 may record a provisional local SHA-256 for review, but it is explicitly non-authoritative and cannot satisfy final external-inventory evidence. NBF-07 alone records the authoritative external artifact SHA-256 after the exact final candidate SHA is frozen.
...
- `source_inputs_sha256`, generator/discovery-rule versions, `--check`, syntax, and focused pytest output
- Optional provisional local inventory artifact digest, labeled non-authoritative and explicitly not final NBF-07 external-inventory evidence
...
9. Prove candidate cleanliness using the exact executable custody contract:
   - `git diff --quiet` passes for tracked working-tree state;
   - `git diff --cached --quiet` passes for index state;
   - the normalized `git ls-files --others --exclude-standard` set contains no path other than `.oracle/briefs/planner-sol.md`;
   - `.oracle/briefs/planner-sol.md` hashes exactly to `6d070eae4e5a09f7575ffffaef91fc95c3cd1a32095baa3219f050d37e7fe8a0` at every checkpoint;
   - the three resume artifacts named in `custody.md` are tracked after the preserved rebase and therefore satisfy tracked/index cleanliness rather than expanding the untracked allowlist;
   - no wildcard, directory exemption, blanket no-untracked assertion, or dynamically expanded allowlist is accepted.
10. Record:
    - exact candidate commit SHA;
    - immutable source-base SHA;
    - candidate branch;
    - tracked working-tree and index clean proof;
    - normalized exact permitted-untracked path set and the allowlisted artifact SHA-256.
...
- Tracked working-tree and index state are clean when candidate SHA is frozen and remain unchanged through validation, review, acceptance, and push; the only permitted untracked path is the exact hash-verified custody artifact in step 9.
...
- Tracked working-tree and index state were clean when the exact candidate SHA was frozen and remained unchanged, and the normalized untracked set contained only the exact hash-verified custody allowlist.
...
- [ ] Is any Batch 3 inventory artifact digest explicitly provisional and non-authoritative, with the authoritative external SHA-256 owned only by NBF-07 after exact candidate-SHA freeze?
...
- [ ] Does NBF-07 require clean tracked/index state plus exactly `.oracle/briefs/planner-sol.md` as the sole permitted untracked path at frozen SHA-256 `6d070eae4e5a09f7575ffffaef91fc95c3cd1a32095baa3219f050d37e7fe8a0`, before any final check?
...
- [ ] Does the mandatory pre-execution Luna sense-check name and accept the final plan-v8/tasklist-v8 digests before a separate Sol freeze decision, and do later Luna review and Sol pre-push acceptance explicitly name the exact candidate SHA?
