# Sol tasklist conversion brief — settled plan v5

You are the read-only tasklist author. Do not edit files or run mutating commands.
Return only the complete Markdown body for the proposed .oracle/tasklist.md.
...
Plan SHA-256: cf92fa6664c8a45f60c930fdcf7cb4d657bd0906df692252d164468ab60c7042
...
## Clean settled-plan disposition
...
Mechanically convert the settled plan into a concise, execution-ready tasklist.
Do not redesign, widen, omit, or combine away a settled requirement.

Required:
- Seven ordered tasks NBF-01 through NBF-07 and their five natural batches exactly
  as settled; preserve every dependency and synchronization barrier.
- For each task: classification, selected model, user-selection rationale, exact
  scope/files/symbols, acceptance criteria, focused validation, relevant frozen
  goal criteria, and specific North Star principle/anti-pattern.
- Normal is final for every task; selected executor is GPT-5.6 Luna. State that no
  task meets the exceptional [XHARD] threshold and give the short plan-backed
  rationale. Sol remains Oracle only.
- One checkpoint per batch with the complete binary acceptance gate and evidence
  paths the Oracle will review.
- Explicit logical-dispatch/attempt/final-launch semantics, no-WBC closure, chain
  authority checker, repository-wide signal inventory, T8 barrier through NBF-05,
  and one authoritative post-rebase validation owner.
- Huge-run determination: NO; no cumulative big-batch boundaries.
- Sync: commit after each passed batch; push only
  origin/megado-nbf-guard-0826 after final pass; never merge main without explicit
  user approval.
- Header must say PROPOSED and “freeze only after fresh pre-execution review.”
- End with a pre-execution review checklist against North Star, goal, plan digest,
  ownership, dependencies, validation, model policy, and sync authority.

Keep it compact enough for executors; refer to plan sections by digest/heading for
full schema detail rather than restating all 2,700 lines.
