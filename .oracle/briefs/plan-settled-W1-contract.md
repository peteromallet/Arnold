# Settled-plan W1 — contract-completeness lens
...
Challenge whether every frozen criterion and authority boundary has a verifiable,
non-vague implementation/acceptance path. Find missing dependencies, ambiguous
ownership, untestable criteria, stale file/symbol assumptions, or accidental
scope widening. Do not propose extra product scope.

Return:
- `PASS_CONTRACT_COMPLETE`, or ranked findings `CONTRACT-N`.
- For each: material yes/no, exact plan/goal/source evidence, required correction,
  criterion impact, and North Star disposition.
- Under 700 words.
