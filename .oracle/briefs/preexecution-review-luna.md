# Fresh independent pre-execution contract review — NBF Megado

You are the one independent pre-execution reviewer. This is a normal review task
assigned by user policy to GPT-5.6 Luna. Read-only: do not edit files, execute
tests, or propose implementation.

Return exactly one binary verdict:
- `PASS`, followed by a concise evidence-grounded contract assessment; or
- `ISSUES`, followed by a numbered list of concrete blocking findings.
...
- Push only `origin/megado-nbf-guard-0826`, and only after the final pass.
...
Implement the complete contracts from settled-plan §§4.4–4.13 and §§4.19–4.21.
...
- Schema fields and legal transitions match settled-plan §§4.4–4.13 and §§4.19–4.21.
...
- `docs/nbf-signal-inventory.json` records schema version, generator version, repository revision, discovery rules, entries, and deterministic digest.
...
- Inventory digest, `--check`, syntax, and focused pytest output.
...
9. Assign one independent GPT-5.6 Luna reviewer to the complete evidence.
10. Submit completion to the GPT-5.6 Sol Oracle.
11. Push only `megado-nbf-guard-0826` to origin after acceptance.
12. If rebase rewrote an already-published branch, verify the remote tip and use `--force-with-lease`, never unguarded force.
13. Stop before merge and request explicit user approval.
...
- Independent Luna review and Sol Oracle both accept.
...
On PASS: commit final integration if needed, push `origin/megado-nbf-guard-0826`, then stop before merge.
...
8f7a36b5449ee8be5523955af29921f45e30061752d6997e8db359eaec74958a  .oracle/tasklist.md
6959643b945b150af1c9ef695257ddd93379ea8a2ad41d56c6e0f6cd2c443ed4  .oracle/plan.md
d8344a0789eed2b6f638b051b015f9630ea2434e097ae6a02a7f6c8a0ac0a5b8  .oracle/agent_goal.md
d75f89f043603b75425096e1c45ef8e11386fa1e5652d752aac64c373c77dd7e  .oracle/northstar.md

## Review contract

Judge the entire execution contract, not prose style:

1. North Star, agent goal, plan, tasklist, and custody/sync authority agree.
2. Every frozen criterion 1–8 maps to tasks, binary checkpoints, exact focused
   validation, and final evidence.
3. Seven tasks/five batches preserve settled ownership and dependencies:
   NBF-01 → NBF-02 → NBF-03 → NBF-04 → NBF-05 → NBF-06 → NBF-07.
4. Exactly-once physical door ownership, admission attempts, linked child
   dispatches, and final launch cardinality are unambiguous.
5. No-WBC and chain bypasses are closed; authority checker and signal inventory
   are executable evidence, not narrative.
6. Crash/restart, no-launch/unresolved/terminal outcome, composite event, receipt
   derivation, changed-precondition, confirmation, and T8 semantics are preserved.
7. All tasks correctly remain normal/Luna; reject any unsupported [XHARD].
8. One authoritative broad suite owner and no duplicated paid/live validation.
9. Push only origin/megado-nbf-guard-0826; no main merge without user approval.
10. No box-only mutation or scope widening.

Only flag a material contract failure that could let execution pass while a frozen
criterion fails. Ordinary implementation choices are not blockers. Include an
explicit North Star alignment disposition.
