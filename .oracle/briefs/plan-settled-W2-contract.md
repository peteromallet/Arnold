# Settled-plan W2 — contract-completeness lens
...
Challenge whether the revised plan now gives every criterion, authority boundary,
state transition, dependency, and acceptance case a precise verifiable owner.
Look for unresolved ambiguity or contradiction, not stylistic preferences.

Return `PASS_CONTRACT_COMPLETE` or ranked `CONTRACT2-N` findings with:
material yes/no, exact plan/goal/source evidence, required correction, criterion
