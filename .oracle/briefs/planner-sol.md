# Sol planning brief — resumed megado-nbf-guard-0826

You are the read-only PLANNER. Inspect the repository at current HEAD before
proposing work. Do not edit files, create commits, or execute mutating commands.
Return only the complete Markdown body for .oracle/plan.md.

## Complete North Star (immutable snapshot)
...
## Complete frozen agent goal (operational contract)
...
## Current custody and evidence

- Immutable source base: origin/main at 798c50619204010ed3f4297fbb57988fe9381924.
- Worktree branch: megado-nbf-guard-0826, rebased onto that source.
- The checked-in .oracle/tasklist.md is a FOREIGN onboarding-run artifact. Ignore
  it completely as planning input. Do not reuse its batches, models, or scope.
- Read .oracle/custody.md, docs/nbf-failure-ledger.md,
  docs/nbf-grok-verdicts.md, .oracle/findings/sensecheck-sol.txt,
  .oracle/findings/evolution-entry13.txt, and
  .oracle/findings/evolution-entry14.txt as prior evidence. The large transcript
  files may contain CLI chatter; extract only evidence-backed conclusions.
- Inspect current origin/main-derived code because file names, line numbers, and
  existing implementations may have changed since the prior evidence.
- Criteria 7 and 8 are part of the frozen objective even though they were appended
  by plan evolution. Identify what current main already satisfies and plan only
  the remaining delta; do not reimplement shipped foundations.

## Required plan contract

Produce a coherent end-to-end plan covering every agent-goal criterion (1–8):

1. Verified current-state inventory with file/symbol evidence and a criterion-by-
   criterion gap table: already satisfied, partially satisfied, or missing.
2. The simplest safe implementation design. Reuse current seams; enforce one door
   per invariant; delete or consolidate duplicate preflights instead of adding
   parallel layers. Explicitly reject North Star anti-patterns.
3. Natural execution batches with bounded tasks, dependencies, synchronization
   points, exact files/symbols, task-level acceptance criteria, relevant North
   Star principle/anti-pattern, and exact focused validation.
4. Proposed normal versus [XHARD] classification for every task. Normal is the
   presumption. Tag [XHARD] only if the irreducible kernel meets every exceptional
   threshold: sustained subtle reasoning across coupled concerns; plausible
   non-local harm that ordinary validation may miss; and Luna cannot reliably
   execute it from a precise mechanical brief. Give full evidence for any tag.
5. Additional exploration areas only where a material unknown remains, with a
   narrow evidence question for Luna. Prefer zero new exploration if the repo and
   existing evidence settle it.
6. Open questions. Resolve them from the repository when possible; distinguish
   genuine user-authority questions from implementable assumptions.
7. Best-effort total implementation effort estimate and explicit >2 weeks huge-
   run determination. If huge, propose cumulative review boundaries.
8. A validation/completion matrix mapping each criterion to commands, scenarios,
   evidence to capture, and reviewer disposition. One authoritative owner runs
   the broad suite once; no duplicated expensive/live validation.
9. A settled-plan readiness disposition: ALREADY_SETTLED or STILL_FORMING, with
   evidence. The reserved pre-settled critique placeholder remains unexecuted.
10. End with a concise “Planner verdict” stating whether the plan can enter the
    Luna settled-plan streamlining wave now, and list any blocking exploration.

Model policy is user-pinned: GPT-5.6 Luna for all normal exploration, critique,
execution, and independent review; GPT-5.6 Sol for planning/revision, oracle
judgment, and any justified [XHARD] execution. Do not propose another model.

Bias toward elegance: KISS, YAGNI, cut scope that is not pulling its weight.
Aim for complete specificity without ceremonial process.
