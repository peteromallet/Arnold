# Settled-plan W1 — simplification lens
...
Challenge only whether the same outcome can be reached with less work, fewer
layers, fewer handoffs, or reuse of an existing seam. Look especially for
speculative abstractions, duplicate state machines/helpers, unnecessary files,
ceremonial validation, or batches that can safely collapse. KISS/YAGNI is the
priority, but do not cut a frozen criterion.

Return:
- `PASS_NO_MATERIAL_SIMPLIFICATION`, or ranked findings `SIMP-N`.
- For each: material yes/no, exact plan section and source evidence, smallest
  safe change, criterion impact, and North Star disposition.
- Under 700 words.
