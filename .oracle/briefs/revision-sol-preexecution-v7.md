# Sol full-plan revision — final SHA binding and worker disposition outcome
...
## Current settled plan v6

SHA-256: 5718557f013661ba543f5736eddd104d13e0e107a9c148f8b8708ad81387143d

# Plan — Typed NBF worker admission, disposition, and scheduling control plane

## 1. Planning basis and custody

This revision accepts the pre-execution T8 observation-state finding and resolves it with the smallest coherent rule: accepted exhausted worker outcomes—not probes—form the consecutive provider-observation streak. It preserves every frozen product criterion, W4 correction, authority boundary, custody rule, delivery rule, model policy, and merge checkpoint.
...
- Superseded immutable plan-v5 SHA-256: `cf92fa6664c8a45f60c930fdcf7cb4d657bd0906df692252d164468ab60c7042`
...
## 2. Revision delta from immutable plan v5

This revision preserves all accepted v5 contracts and makes one material pre-execution T8 correction:

1. Accepted and canonically recorded `provider_exhausted` worker outcomes—not probes, waits, or probe-success evidence—form the consecutive provider-observation streak.
2. `provider_recovery_verified` remains an evidence-bound, single-use changed precondition that may authorize exactly one linked same-route child after a passed bounded probe, but it does not reset or rekey the existing provider-exhaustion streak.
3. If that authorized child is accepted and records a matching `provider_exhausted` outcome, it is the second consecutive matching worker observation and may establish `provider_degraded`.
4. A successful worker dispatch resets the active provider-observation streak.
5. Another allowlisted changed precondition resets or rekeys the streak only when its canonically derived authoritative before/after identity invalidates the provider-failure key:
   - phase;
   - normalized selected spec;
   - provider failure class;
   - provider epoch identity.
6. Mere time passage, sleep, membership refresh, bounded probe success, or creation/consumption of `provider_recovery_verified` does not change that key.
7. A nonmatching accepted provider-exhausted outcome starts a new keyed streak at one. An intervening accepted non-provider-exhaustion outcome breaks consecutiveness without being reclassified as provider degradation; ordinary failures remain ordinary.
8. Projection identity, replay rules, T8 timing, crash/race tests, validation criteria, and completion conditions now use this keyed streak rule consistently.

No state owner, scheduling loop, projection store, journal, rotator, research task, or `[XHARD]` task is added.

The following v5 corrections remain frozen:
...
- applies normal terminal-fingerprint, provider-observation, and changed-precondition rules;
...
- A losing concurrent writer reloads and does not duplicate a reservation, composite transition, receipt, probe lease, provider observation, changed-precondition consumption, reconciliation, terminal outcome, confirmation record, or launch.
...
| Accepted ordinary failure or worker disposition | Intervenes between exhausted worker outcomes | Break streak; clear active consecutive state | Preserve ordinary/disposition handling; no provider degradation |
...
10. An intervening accepted ordinary failure or worker disposition breaks consecutiveness while retaining its ordinary typed behavior.
11. Two consecutive matching exhausted worker observations establish degradation.
12. Excluded error classes never enter the degradation projection.
13. Duplicate observation IDs do not increment twice.
14. All transitions compare the expected projection version.
15. One caller holds a probe lease.
16. Restart replay yields identical state.
17. Fallback metadata mirrors the projection and is never authoritative.
18. `_advance_configured_spec_fallback` remains the only configured fallback-selection door.
19. An unresolved parent reservation blocks observation-driven child creation.
20. A no-launch parent creates no provider observation and cannot authorize a provider-driven child.
21. The composite event projects route transition and child reservation together.
22. Provider exhaustion is never double-recorded as ordinary failure.
23. Changed-precondition CAS validates canonical producer/evidence and provider-failure-key binding before child reservation.
...
- If the child records an ordinary terminal failure or worker disposition, it remains ordinary/disposition and breaks the prior exhausted-outcome consecutiveness.
...
- let an ordinary failure become provider degradation;
...
- Accepted ordinary failure or disposition breaks consecutiveness without becoming provider degradation.
...
- Recovered terminal outcome applies normal fingerprint and provider-projection rules.
...
23. Define the single typed T8 policy-extension interface consumed later by NBF-06; it may return decisions but cannot own waiting, launching, or observation-state policy.
...
NBF-06 starts only after the synchronization gates for NBF-01 through NBF-05 pass. In particular, the canonical terminal-outcome writer, provider-failure-key and keyed-streak replay contract, evidence-bound changed-precondition producers, controlled launch adapter, receipt-context transport, disposition helper/CLI, durable confirmation contract, authority checker, and generated signal inventory must be frozen and green before T8 edits begin.
...
- Prove an accepted ordinary failure or disposition breaks consecutiveness without becoming provider degradation.
...
- Inject crashes around provider observation, probe result, recovery-event creation, recovery-event consumption, composite append, receipt derivation, cache update, `not_started`, launch entry, launch acceptance, closure return, and terminal-outcome append.
...
- An ordinary terminal failure or disposition breaks consecutiveness but remains on its existing typed path.
...
8. Capture:
   ...
   - success-reset, different-key rekey, key-changing precondition reset/rekey, key-preserving precondition no-reset, and ordinary-failure non-degradation traces;
...
|---|---|---|
| NBF-01 schemas/projection/CAS | Normal / Luna | Contracts, terminal projection, keyed provider replay, evidence binding, receipt derivation, confirmation, and one-record journal behavior have deterministic tests. |
...
| Batch | Estimate |
|---|---:|
| Schemas, terminal projection, keyed provider replay, evidence-bound producers, composite event, confirmation, and reconciliation CAS | 2–2.5 days |
...
| Provider-failure-key integrity | Probe success, recovery authorization, key-changing and key-preserving changes, nonmatching worker outcomes | Canonical key derivation and before/after producer evidence | Probe/recovery events cannot reset; only authoritative key invalidation resets/rekeys; nonmatching exhaustion starts at one; ordinary failures remain ordinary. |
...
| Terminal projection | Success, ordinary failure, provider exhaustion, disposition | Canonical terminal event and reservation projection | Every accepted non-scheduling terminal result records once before closure/consumer projection; only typed provider exhaustion enters the provider-observation path. |
...
| Crash safety | Injection around ledger/cache/receipt/launch/outcome/probe/recovery boundaries | Replay state, keyed streak, and launch count | No partial transition, duplicate child, fabricated outcome, probe-driven reset, circular receipt, or blind retry. |
...
The prior W4 findings and the pre-execution T8 contradiction are resolved at contract, ownership, state-transition, replay, validation, and completion levels:
...
- ordinary failures and dispositions remain typedly ordinary and break exhausted-outcome consecutiveness without entering provider degradation;
...
- keyed streak behavior is deterministic across replay, cache loss, crashes, probe races, and child-reservation races;
...
- NBF-01 owns schemas, replay projection, keyed provider mechanics, receipt derivation, terminal projection, evidence validation, confirmation, and the single ledger primitive only;
...
1. **Final validation is not bound to the final candidate SHA.** NBF-07 runs inventory generation, checker, and the authoritative pytest suite before step 9 commits final/generated changes. The required evidence therefore cannot be attributable to the exact clean SHA later reviewed and pushed. This is worse for `docs/nbf-signal-inventory.json`, whose required `repository_revision` can become stale immediately after the commit containing the generated artifact. Commit all final changes first, then run all SHA-bound validation with no post-validation mutation; define `repository_revision` as a non-self-referential source revision.

2. **Accepted worker dispositions have no lossless `DispatchOutcome` representation.** Settled plan §4.8 requires `worker_terminal_outcome.outcome_kind=worker_disposition`, but §4.13 and NBF-01 enumerate `DispatchOutcome.kind` without `worker_disposition`. NBF-02 nevertheless requires dispositions to enter the canonical terminal writer. Without an explicit disposition-to-terminal writer path or a typed outcome variant, execution can coerce a killed worker into `ordinary_terminal_failure`, violating the frozen typed disposition contract.

North Star disposition: **BLOCKED**.
[launch_hermes_agent] done in 159.2s (exit=0)
0

## Oracle disposition and revision contract

Accept both findings and apply only these minimal corrections:

1. Add explicit `DispatchOutcome.kind=worker_disposition`, carrying the canonical
   disposition ID plus receipt/fingerprint/phase/spec/worker/launch-state context.
   The canonical terminal writer maps it to
   `worker_terminal_outcome(outcome_kind=worker_disposition)`. It is never
   coerced into ordinary failure, never double-appended, and preserves record-
   before-signal, replay, reservation closure, and breaker semantics. Update schema,
   transition table, NBF-01/NBF-02/NBF-04 acceptance/tests, matrix, and completion.

2. Make final candidate validation non-circular:
   - signal inventory contains a deterministic `source_inputs_sha256` over the
     sorted discovered signal-bearing source inputs plus generator/discovery-rule
     version, excluding the generated inventory itself; no embedded git commit or
     self-digest;
   - commit all implementation and generated artifacts first;
   - record exact candidate commit SHA;
   - run inventory `--check`, authority checker, syntax, and the authoritative
     broad suite against that exact clean commit with no later candidate mutation;
   - capture validation logs outside candidate content (durable local evidence
     location named by the run) so evidence creation cannot change the reviewed SHA;
   - Luna review and Sol pre-push gate bind to that SHA; push exactly it; remote
     verification; final Sol completion.
   - Any candidate mutation restarts commit → validation → review.

Preserve all prior W1–W6 decisions, T8 streak correction, delivery authorization,
model policy, and no-main-merge boundary. No new store, service, research, or
[XHARD] work. End exactly `STABILITY: STABLE` unless a real evidence question
remains.
