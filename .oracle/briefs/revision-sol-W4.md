# Sol full-plan revision brief — settled-plan W4
...
SHA-256: 19d37c43207e116877ba0f3b5391fdfd1cf55f8cffda3d11e9869feb8ba734db
...
Working...
SIMP4-1 — **Material: composite event has a receipt-ID circularity**

- **Evidence:** §4.4 says `WorkerAdmissionReceipt` is returned only after the ledger append commits. §4.7 nevertheless requires `provider_route_child_reserved` to contain `child_admission_receipt_id`, while that same append is what creates the child reservation and receipt.
- **Risk:** Implementers must either mint receipt identity before the authoritative event exists, append a prepare record, or split the transaction—contradicting the one-record atomicity requirement.
- **Smallest safe correction:** Remove `child_admission_receipt_id` from the composite event and derive it deterministically from the committed event ID plus child identity during receipt construction/replay. State that replay must reproduce the same receipt ID.
- **Criterion impact:** Blocks composite atomicity, restart replay, linked-child context transport, and criterion 8.
- **North Star disposition:** Required for “one door per invariant” and crash-safe dispatch; one authoritative event must define all derived receipt state.

SIMP4-2 — **Material: pre-spawn exception semantics contradict no-launch fingerprint rules**

- **Evidence:** §4.11 requires an exception before `mark_final_launch_entered()` to become an `ordinary_terminal_failure` and reconcile as `released_no_launch`. §4.8 requires `released_no_launch` to create **no worker terminal fingerprint**. The plan does not define whether the typed outcome is projected as a terminal worker event before reconciliation.
- **Risk:** A straightforward implementation records the ordinary terminal outcome, creates the terminal fingerprint, then releases the reservation. Identical redispatch is consequently blocked despite no worker ever launching; another implementation may suppress the outcome and violate typed outcome transport.
- **Smallest safe correction:** Define pre-entry exceptions as a return-level typed outcome with `launch_state=not_started`; they may cross `PhaseResult`, but must not create a worker terminal event/fingerprint. The ledger records only positive `reservation_reconciled(released_no_launch)` (or an explicitly non-worker outcome linked to it). `ordinary_terminal_failure` becomes terminal-fingerprint-bearing only after launch acceptance.
- **Criterion impact:** Required for final-launch exception normalization, truthful reconciliation, and fingerprint redispatch behavior.
- **North Star disposition:** Directly enforces “deaths speak” without inventing a worker death and prevents the recovery loop from treating a never-launched attempt as a terminal worker failure.

No other material duplicate owner, parallel state authority, or redundant validation remains evident in v4 after the prior W1–W3 corrections.
[launch_hermes_agent] done in 92.1s (exit=0)
0

## Raw W4 contract findings

[launch_hermes_agent] model=codex:gpt-5.6-luna → resolved=openai-codex/gpt-5.6-luna toolsets=['file'] max_tokens=65536 context_budget_tokens=(auto)
[launch_hermes_agent] NOTE: omp gives the full toolset (Bash, Read, Edit, web, …); the file/web/terminal subset is a superset here.
[launch_hermes_agent] cwd=/Users/peteromalley/Documents/Arnold-oracle-nbf
Working...
CONTRACT4-1 — **Material: pre-spawn exception handling contradicts reconciliation safety.**

- **Evidence:** §4.8 permits `released_no_launch` only with positive proof that no final launch was entered or accepted. §4.11 nevertheless says any exception before `mark_final_launch_entered()` becomes a typed terminal outcome and reconciles as `released_no_launch`. Absence of that marker is not positive proof; a buggy or partially executed closure could spawn before marking.
- **Smallest correction:** Require the closure to persist an explicit `not_started` marker before any launch-capable operation, and require deterministic no-launch evidence on every release path. If sequencing evidence is missing, classify as `unresolved_launch`.
- **Criterion impact:** Final-launch exceptions, reservation reconciliation, crash safety.
- **North Star:** Directly protects “deaths speak” and the ban on blind redispatch.

CONTRACT4-2 — **Material: the existing no-WBC path remains an admission bypass.**

- **Evidence:** Current `_impl.py:7374–7399` immediately delegates `run_step_with_worker(..., wbc_dispatch=None)` to `_run_step_with_worker_legacy`. The plan specifies WBC ordering and says native `_impl.py` binds the shared seam, but does not explicitly eliminate or forbid this production fallback. Existing direct callers can omit `wbc_dispatch`; that path can launch without the canonical reservation/receipt seam.
- **Smallest correction:** Make every production `run_step_with_worker` invocation enter `dispatch_with_admission`; either construct the WBC adapter internally or reject `wbc_dispatch=None` under production intent. Add a negative structural test for the no-WBC invocation.
- **Criterion impact:** 1, 2, and 6.
- **North Star:** One admission door; no worker exists behind an unguarded route.

CONTRACT4-3 — **Material: ordinary terminal outcomes are not explicitly projected into fingerprint CAS state.**

- **Evidence:** `WorkerDisposition` carries a fingerprint, but `DispatchOutcome` only lists `terminal_failure`/`provider_evidence`; no canonical terminal-outcome event or projection rule is frozen for ordinary failures and provider exhaustion. §4.11 says the seam “records or reconciles” outcomes without naming the authoritative event consumed by §4.5–4.7.
- **Smallest correction:** Define one strict terminal outcome event containing the semantic fingerprint and require it before reservation closure for every non-scheduling terminal result; make projection and redispatch CAS consume it.
- **Criterion impact:** 4 and 8.
- **North Star:** Prevents anonymous terminal failures from bypassing the unchanged-fingerprint block.

CONTRACT4-4 — **Material: changed-precondition events can be caller-forged.**

- **Evidence:** §4.6 requires unequal `before_content_id`/`after_content_id`, an allowlisted reason, and an evidence reference, but does not require those IDs to be derived from authoritative state or prove the referenced evidence matches the claimed change. Arbitrary two-value IDs could authorize an identical retry.
- **Smallest correction:** Make canonical producers mint reason-specific content identities; CAS must validate evidence-to-identity binding and consume the event atomically. Add forged-event rejection tests.
- **Criterion impact:** 4, 5, and T8 recovery.
- **North Star:** Durable changed preconditions, not judgment-based bypasses.
[launch_hermes_agent] done in 173.4s (exit=0)
0

## Raw W4 sequence findings

[launch_hermes_agent] model=codex:gpt-5.6-luna → resolved=openai-codex/gpt-5.6-luna toolsets=['file'] max_tokens=65536 context_budget_tokens=(auto)
[launch_hermes_agent] NOTE: omp gives the full toolset (Bash, Read, Edit, web, …); the file/web/terminal subset is a superset here.
[launch_hermes_agent] cwd=/Users/peteromalley/Documents/Arnold-oracle-nbf
Working...
## SEQ4-1 — Pre-spawn exception semantics are internally contradictory

**Materiality:** High; affects criteria 4, 7, 9, and crash safety.

**Evidence:** §4.8 says `released_no_launch` creates no terminal worker fingerprint and permits a new reservation. §4.11 and NBF-02 acceptance say an exception before `mark_final_launch_entered()` becomes an `ordinary_terminal_failure` after canonical outcome recording. §4.12 says ordinary terminal failures reach existing consumers, while §4.10 forbids scheduling conditions from entering failure/breaker accounting.

The plan does not define whether a pre-spawn exception:
- records a terminal fingerprint;
- increments failure/breaker counters;
- may be retried identically;
- is merely reconciled as no-launch.

Those outcomes are materially different.

**Smallest correction:** Define a distinct typed `no_launch` outcome, or explicitly specify that pre-spawn normalization is excluded from terminal-failure projection, fingerprint reservation, and breaker accounting. Define ordering: reconciliation first, then consumer projection. Add restart and identical-redispatch tests.

**Criterion impact:** Final-launch exceptions, fingerprint blocking, scheduling/failure separation.

**North Star disposition:** Supports “deaths speak” and prevents an unlaunched worker from producing contradictory death/failure state.

---

## SEQ4-2 — T8 dependencies omit later shared-file owners

**Materiality:** High; creates an executable synchronization race.

**Evidence:** NBF-06 declares dependencies only on NBF-01–03 but edits `auto.py`, `handlers/shared.py`, `phase_result.py`, `phase_result_classify.py`, `recovery_policy.py`, `incident/disposition.py`, and `incident/ledger.py`. NBF-04 also edits `auto.py` and disposition integration; NBF-05 edits the disposition CLI and wrapper behavior. If execution is dependency-driven rather than strictly batch-serialized, NBF-06 can implement against incomplete signal and scheduling transport contracts.

**Smallest correction:** Add NBF-04 and NBF-05 as NBF-06 dependencies, or split shared-file ownership into explicit prerequisite substeps and add a synchronization barrier before T8 implementation/tests.

**Criterion impact:** Typed scheduling transport, death context, T8 breaker isolation, integration validity.

**North Star disposition:** Preserves one scheduling owner and one death-recording authority instead of allowing interim duplicate paths.

---

## SEQ4-3 — Two-scan confirmation lacks a durable contract

**Materiality:** High; directly affects kill safety and restart behavior.

**Evidence:** §4.17/§5 require two separated observations with stable process-start and progress identity. NBF-05 additionally requires cross-invocation watchdog/container incarnation state. No schema, storage owner, atomic update rule, TTL, restart behavior, or exact reset semantics is frozen. A wrapper restart could therefore lose the first scan or accidentally reuse confirmation state.

**Smallest correction:** Freeze a confirmation record keyed by victim PID, process-start identity, relevant progress identity, and watchdog/container incarnation. Specify persistence, atomic replacement, expiry, and reset rules. Add restart, PID reuse, progress advance, and incarnation-change tests.

**Criterion impact:** Signal closure, crash/restart safety, record-before-signal.

**North Star disposition:** Enforces the explicit ban on single-scan supervision verdicts.

---

## SEQ4-4 — Final negative preflight verification is too narrow

**Materiality:** Medium-high; criterion 1 can pass while a forbidden bypass remains.

**Evidence:** §11 scans only `refresh_runtime_launch_seed_for_worker_dispatch` and `require_configured_runtime_launch`. The prohibited patterns and §4.2 also include `worker_launch_preflight`, chain-local refusal logic, standalone memory/source checks, and direct chain launch construction. The structural spy covers exercised paths but does not mechanically prove absence across all physical doors and chain origins.

**Smallest correction:** Add a narrow AST/static checker covering every forbidden authority symbol and launch construction on the three doors plus chain-origin callers; run it in NBF-03 and NBF-07.

**Criterion impact:** Unique admission authority and chain-bypass prevention.

**North Star disposition:** Enforces “one door per invariant” by proving deletion, not merely successful routing.
[launch_hermes_agent] done in 117.7s (exit=0)
0

## Revision contract

Apply every accepted W4 finding across design, ownership, dependencies, tests,
matrix, and readiness. Keep the oracle constraints exactly.

Required corrections:
- derive child receipt ID from committed composite-event ID + child identity;
- define no_launch/not_started as non-worker, non-fingerprint, non-breaker state;
  require controlled launch sequencing; ambiguous evidence becomes unresolved;
- close or reject the production no-WBC path;
- define one canonical post-launch terminal-outcome event consumed by projection;
- bind changed-precondition events to canonical reason-specific producers/evidence;
- make NBF-06 depend on NBF-01..NBF-05;
- freeze durable two-scan confirmation schema/TTL/incarnation/reset;
- add targeted authority-bypass checker in NBF-03 and NBF-07.

Do not add a prepare/commit protocol, scheduler, store, journal, rotator, lease, or
[XHARD] work. This should be a bounded final contract correction. Include a concise
revision delta and end exactly with `STABILITY: STABLE` unless a genuine material
evidence question remains.
