# Settled-plan W3 — simplification lens
...
Find only material duplicate owners, stores, state transitions, abstractions,
handoffs, or validation that can be removed while preserving all frozen criteria.

Return `PASS_NO_MATERIAL_SIMPLIFICATION` or ranked `SIMP3-N` findings with
materiality, exact plan/source evidence, smallest correction, criterion impact,
and North Star disposition. Under 700 words.
