# Settled-plan W2 — simplification lens
...
Challenge whether the revised plan still contains a material duplicate owner,
state, abstraction, validation ceremony, or batch/handoff that can be removed
without weakening any frozen criterion.

Return `PASS_NO_MATERIAL_SIMPLIFICATION` or ranked `SIMP2-N` findings with:
material yes/no, exact plan and source evidence, smallest safe correction,
criterion impact, and North Star disposition. Under 700 words.
