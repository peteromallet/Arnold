# Sol tasklist revision — align with settled plan v6

You are the read-only tasklist author. Return only the complete corrected Markdown
body for .oracle/tasklist.md. Do not edit files or run mutating commands.
...
## Current corrected tasklist proposal
...
Preserve the current tasklist's seven tasks, five batches, all-normal/Luna
classifications, delivery order, external inventory digest, NBF-01 scope
correction, and every acceptance/validation/custody boundary.

Change only what is required to align with plan v6:
- update all settled-plan digest references to v6;
- in NBF-06 and its batch/final matrix, state that accepted exhausted worker
  outcomes form the provider streak;
- provider_recovery_verified authorizes one linked same-route retry but preserves
  the matching streak;
- the matching authorized child's exhaustion may be observation two/degrade;
- worker success resets; authoritative provider-failure-key changes reset/rekey;
  probe success/time passage alone do not;
- preserve CAS/single-use authorization, fallback/return, replay/races, and breaker
  isolation.

Header remains PROPOSED and freeze requires a fresh full pre-execution review.
Do not alter the reachable commit-before-review → Luna → Sol pre-push → push exact
SHA → remote verify → final Sol order. No other redesign or scope change.
