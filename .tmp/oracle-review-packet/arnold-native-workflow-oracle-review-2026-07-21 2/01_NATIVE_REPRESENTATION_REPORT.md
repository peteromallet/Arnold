...
2. Parent and child WBC execution attempts are distinct and joined causally.
   Child attempt terminal is evidence for a typed parent input; it is not root
   workflow completion.
...
A retry normally resumes or reuses the **same child** and its durable terminal/
effect outcome. An explicit **new generation** is a new semantic occurrence
with a new child key/generation, authority/custody, and declared repeat policy;
it is not another spelling of retry. Intent-without-outcome reconciles before
either path advances. Cancellation first fences new child admissions, records
cancel intent, and propagates cancellation. The parent accepts cancellation
only after each required child terminal plus epoch-checked Custody release/
transfer, or declared lease expiry; a parent terminal cannot fictionalize a
child release.
...
suspension/reentry ID, timeout/escalation policy, and possible closed outcomes.
Suspension writes a checkpoint and WBC event. Resume consumes one accepted
typed human decision, validates its schema and executable pins, and reacquires
current Run Authority/Custody before the next product body. An inbox row,
approval marker, CLI status, or old lease cannot resume it.
...
attempt identity, and terminal output ref. A retry is a new attempt joined to
the same semantic occurrence; it cannot overwrite the first history.
...
also pass through the effect intent/outcome protocol.
...
Native Parity needs a few precise constructs so ordinary product behavior does
not leak back into handlers:
...
- **Named enclosing-loop exits.** A typed exit names an enclosing loop and one
  ...
  ancestor and the parent handles it exhaustively. A delivery/rework
  `replan` exit can therefore leave its local loop and re-enter the named
  planning loop without a sentinel or exception.
...
- **Declared durable agentic phase.** A model may choose a bounded, runtime
  ...
  durable ordered inner call ledger. Effectful tools still use intent/outcome.
  The phase cannot emit a hint that chooses the next outer product route.
...
lifecycle terminals. `ask_human` is an internal decision that causes a durable
...
not list positions, keep the two instances isolated.
...
6. **Suspend if needed.** Suppose the internal review decision is
   ...
   policy, and exposes a typed question. While suspended, release-review v1.1
   is deployed. Resume uses pinned v1 if available, or one declared accepted
   state migration; it never resumes against v1.1 merely because the source
   path is unchanged. The answer becomes one accepted typed human decision,
   and the new host reacquires current RA/Custody before continuing.
...
8. **Return and reduce.** The child records one WBC terminal and returns the
   closed `approved(BuildCandidate)` outcome to the parent. The accepted parent
   reducer waits for the exact Linux/macOS child multiset. Child approval does
   not directly accept the release root terminal. Only `release_root_host` may
   map a condition-satisfying `publishable` business result or declared
   lifecycle terminal to a proposed product terminal, which still passes the
   root accepted-decision and terminal-arbitration action boundary.
...
Stage 2 fanout uses a closed `JoinPolicy`, not scheduler timing:

| Policy | Completion rule | Required loser/late behavior |
| --- | --- | --- |
| `all` | Every admitted child reaches an allowed terminal | Fail/cancel behavior and whether partial results survive are declared |
| `any` | First accepted qualifying result wins arbitration | Remaining children receive cancellation; completed effects remain history; late results are durably rejected or retained as non-winning facts |
| `quorum(k)` | `k` qualifying results satisfy the declared predicate | Threshold/failure impossibility, tie precedence, loser cancellation, and late success disposition are declared |
| `reducer_threshold` | Canonical keyed reducer emits a declared threshold outcome | Reducer inputs ignore completion order and preserve exact multiplicity |
...
completion waits for the policy-required child terminal and epoch-checked
Custody release/transfer/expiry dispositions; it cannot erase rejected-late
facts or completed effects.

Child resource contexts are narrowed, never widened. Fanout declares
capability, deadline, cancellation, worker, token, and cost reservations;
charges for attempts/tool calls are committed durably; cache hits, retries,
cancellations, loser shutdown, and late completion follow declared charge and
release/refund rules that reconcile to the parent budget.
...
1. admit the completed Custody Control Plane contract and freeze the semantic,
   identity, trace, and proof model;
...
5. prove the six golden scenario families, negative mutations, installed/cloud
   parity, stale-worker rejection, no-dual-write effects, and composed causal
...
GO-1 proves the front-half source is installed and load-bearing; GO-2 gates
live effects; GO-3 proves stale-worker, race, and projection-forgery rejection
before control-consumer demotion; GO-4 consumes the complete proof map and
blocks completion.

### 9.2 Stage 2 — platformize only proven boundaries

After the Native Parity completion manifest is accepted:
...
1. **Descriptor:** every exported step, subworkflow, and workflow has a
   ...
   memoization dependencies where applicable, and extension points.
2. **Lifecycle:** every component kind executes through one closed protocol;
   ...
   remain distinct from lifecycle/control terminals. Human gates use this same
   durable protocol rather than an out-of-band wait path. Only the root-host
   adapter may propose a root product terminal.
3. **Composition:** all bindings and control propagation are explicit; port,
   ...
   durable parent-loop state, join policy, scope, deadline, cancellation,
   budget, and namespace rules are statically checkable for every supported
   shape.
4. **Isolation:** component instances own disjoint state, checkpoint, artifact,
   ...
   Parent cancellation has an explicit epoch-checked child Custody release/
   transfer/expiry disposition.
...
7. **Evolution:** compatible and breaking changes are defined for Python API,
   ...
   migration/new-attempt path. New-instance compatibility never implies resume
   compatibility.
...
17. **Parent durability/retry:** crashes at every loop-ledger edge preserve
    generation and terminal-consumption CAS; same-child resume reuses durable
    outcomes, while explicit new generations receive new logical identity.
18. **Join/cancel/resources:** all/any/quorum/reducer-threshold policies resolve
    ...
    late facts, narrow child budgets, and require declared child Custody
    release/transfer/expiry before parent terminal acceptance.
...
- resource, budget, cancellation, and deadline semantics;
- root-host terminal mapping; and
...
semantic site
  ...
  -> emitted evidence
  -> positive and negative conformance scenarios
```
...
competing reconfiguration, resume, or migration transition. Wall-clock arrival
order and projection order are never implicit arbitration policies.
...
The following remain explicit audit items rather than silently resolved facts:

- the content and disposition of omitted Oracle question 2 items 2, 3, 5, and
  7, which must be recovered from the canonical Oracle packet;
