...
fresh ephemeral identity and fake/sandbox effects. It cannot compile, admit,
checkpoint, replay/resume, certify, or publish. Exact pinned `.pypeline`,
...
legacy converter, and read-only legacy resolution described above. S2 closes
...
cloud equivalence. `GO-FORMAT` blocks S3A and GO-0; it is not a new sprint.
...
with schema `arnold.megaplan.go_format_receipt.v1`. S2 must produce and validate
it; S3A must independently rerun the validator before work. Narrative status or
a hash-only receipt cannot pass.
...
`front_half.pype`; S4 lands tiebreaker; S5 lands delivery workflows; S6 keeps
...
43. `.pype` format/identity fixtures cover exact-one files, canonical imports,
    ...
    recursion/collisions/dynamic imports, descriptor mismatch, transitive
...
18. GO-FORMAT, GO-0, GO-1A, GO-1B, GO-2, GO-3 and GO-4 are green; live external effects
