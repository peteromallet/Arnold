...
requires
`chain_completed + require_manifest + required_proof_artifacts`: launch must
find that exact path once in the validated completion manifest and match the
current artifact hash to the manifest hash. A separate file-exists or
`contains_text` result cannot satisfy this handoff requirement.
...
The milestone count is seven. S2A owns runtime meaning; S2B owns the
