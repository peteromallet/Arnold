...
Status: normative Native Parity composition oracle and ten-milestone fixture contract
...
- **S2R:** consume GO-FORMAT and integrate the independent proof path with the neutral production runtime; fill generic loop, fanout/fanin, decision, policy, identity, checkpoint and retry/fallback events; implement edit/repeat/fork with isolated identities; register all three planes, pass production-adapter two-client CAS/crash and introduction-time restore proofs, and close GO-0.
...
- **S5A:** make the delivery topology green in comparison/shadow execution, inventory every effect-protocol class, and close per-class GO-2 without live effect authority.
- **S5B:** require the accepted GO-2 receipt at the live authority switch, then make `NP-GT-004` and `NP-GT-005` green against the admitted live-effect and bounded review/rework paths.
