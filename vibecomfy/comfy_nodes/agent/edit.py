    ...
    *,
    ...
    if statements and landed_count == 0 and all((statement.op_kind or "") == "query" for statement in statements):
        query_only_note = (
            ...
            "An exact local `search(...)` hit that prints a `def ClassName(...)` signature is "
            "sufficient installed/provisional schema evidence for simple local node swaps, "
            "field changes, or adding that node; do not require a workflow precedent in that case. "
