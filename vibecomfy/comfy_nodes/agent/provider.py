...
def build_batch_messages(
    *,
    ...
    system = (
        ...
        "Exception: when the target node already exists in the current graph, the task is a narrowly scoped parameter tweak, and the visible graph evidence only exposes stable existing `widget_N` slots, you may land the smallest direct `widget_N` edit on that existing node instead of blocking on workflow precedent. "
