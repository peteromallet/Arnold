...
function appendCandidateDetail(body, panel, message = null, snapshot = null) {
  ...
  appendTextLine(body, `canvasApplyAllowed=${String(canvasApplyAllowed)} canvas_apply_allowed=${String(canvasApplyAllowed)}`, canvasApplyAllowed ? "#4caf50" : "#ffb86c");
  appendTextLine(body, `applyEligibility=${eligibility.reason}`, eligibility.applyable ? "#4caf50" : "#ffb86c");
  appendTextLine(body, `queueAllowed=${String(queueAllowed)} queue_allowed=${String(queueAllowed)}`, queueAllowed ? "#4caf50" : "#ffb86c");
...
function appendFailureDetail(body, panel, snapshot = null) {
  ...
  appendTextLine(body, `canvasApplyAllowed=${String(Boolean(failure.canvas_apply_allowed))} canvas_apply_allowed=${String(Boolean(failure.canvas_apply_allowed))} queueAllowed=${String(Boolean(failure.queue_allowed))} queue_allowed=${String(Boolean(failure.queue_allowed))}`, "#8d93a1");
