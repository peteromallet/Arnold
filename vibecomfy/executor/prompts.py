...
_CLASSIFY_SYSTEM = (
    ...
    "- CRITICAL: NEVER name a technology ecosystem (AnimateDiff, LTX, VHS, "
    "WanVideo) that does NOT appear in the current graph's node types. Naming "
    "a different ecosystem will send research toward irrelevant workflow slices "
    "that cannot be lowered and will hard-fail implementation.\n"
    "- BAD: for a Wan2.2 I2V graph, search_directions mention "
    "\"AnimateDiff/VideoHelperSuite LoRA noise variance\" when AnimateDiff is "
    "not in the graph. GOOD: \"UnetLoaderGGUF noise schedule\", "
    ...
    "research goal should ask for exact local/registry/workflow evidence for "
    "the existing class type and the field/socket being edited, so implementation "
    "can hydrate a schema and make the smallest safe mutation instead of stopping "
    "because unrelated custom nodes are unknown.\n"
    ...
    "ask for exact current terminal classes plus compatible local consumer/"
    "composition nodes, not for a broad replacement workflow.\n"
