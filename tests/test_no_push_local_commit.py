...
from arnold.pipelines.megaplan.chain import load_chain_state, run_chain
...
def test_no_push_start_mode_commits_completed_milestone_locally(tmp_path: Path) -> None:
    ...
    with patch("arnold.pipelines.megaplan.chain._refresh_base_branch", lambda *a, **k: None), \
         ...
         patch("arnold.pipelines.megaplan.chain._commit_phase", wraps=chain_module._commit_phase) as commit_phase:
