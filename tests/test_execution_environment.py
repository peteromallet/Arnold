...
def test_preflight_phase_tolerates_same_checkout_dirty_signature_drift(tmp_path: Path) -> None:
    target_root = tmp_path / "repo"
    _init_repo(target_root)
    state = {
        "config": {"project_dir": str(target_root)},
        "meta": {
            "engine_isolation": {
                "schema_version": 1,
                "pinned": True,
                "created_phase": "init",
                "last_observed_phase": "init",
                "engine_root": str(target_root.resolve()),
                "engine_commit": "abc123",
                "engine_signature": "sha256:before",
            }
        },
    }
    ...
    try:
        env = actual_resolve(root=tmp_path, state=state, engine_root=target_root)
        drifted_env = replace(
            env,
            engine_commit="abc123",
