...
def test_overlap_waiver_tolerates_same_checkout_dirty_signature_drift(tmp_path: Path) -> None:
    target_root = tmp_path / "repo"
    _init_repo(target_root)
    state = {"config": {"project_dir": str(target_root)}, "meta": {}}
    env = resolve_execution_environment(root=tmp_path, state=state, engine_root=target_root)
    state["meta"], waiver = append_engine_overlap_waiver(
        state["meta"],
        env,
        reason="intentional same-checkout dogfood run",
        phase="revise",
        timestamp="2026-06-07T00:00:00Z",
    ...
    drifted_env = replace(
        env,
        engine_signature="sha256:dirty-artifact-signature",
        engine_dirty=True,
    )

    from arnold.pipelines.megaplan.runtime.execution_environment import _engine_overlap_waiver_invalid_reason

    assert (
        _engine_overlap_waiver_invalid_reason(
            waiver,
            drifted_env,
            phase="revise",
            now="2026-06-07T00:00:30Z",
        )
        is None
    )


def test_overlap_waiver_tolerates_dirty_signature_across_different_checkouts(tmp_path: Path) -> None:
    """Waiver stays valid even when engine is dirty and in a different checkout.

    Regression: engine_signature was previously checked and any harness edit
    (dirty flag) would invalidate the waiver for non-same-checkout runs,
    breaking local dogfood.  The fix removes the engine_signature check from
    waiver validation; only engine_commit must match.
    """
    target_root = tmp_path / "target"
    engine_root = tmp_path / "engine"
    _init_repo(target_root)
    ...
    state["meta"], waiver = append_engine_overlap_waiver(
        state["meta"],
        env,
        reason="dogfood run with separate engine checkout",
        phase="execute",
        timestamp="2026-06-07T00:00:00Z",
        expires_after_runs=1,
    )
    # Simulate a dirty engine with a different signature but same commit.
    drifted_env = replace(
        env,
        engine_signature="sha256:totally-different-dirty-hash",
        engine_dirty=True,
    )
...
def test_preflight_phase_tolerates_signature_only_drift_with_valid_waiver(tmp_path: Path) -> None:
    target_root = tmp_path / "repo"
    _init_repo(target_root)
    state = {"config": {"project_dir": str(target_root)}, "meta": {}}

    import arnold.pipelines.megaplan.runtime.execution_environment as env_module

    actual_resolve = env_module.resolve_execution_environment
    try:
        env = preflight_phase(root=tmp_path, state=state, phase="init", engine_root=target_root)
        state["meta"], waiver = append_engine_overlap_waiver(
            state["meta"],
            env,
            reason="intentional local dogfood",
            phase="execute",
            timestamp="2026-06-07T00:00:00Z",
            expires_after_runs=1,
        )
        drifted_env = replace(
            env,
            engine_signature="sha256:after",
            engine_dirty=True,
        )
        env_module.resolve_execution_environment = lambda *_args, **_kwargs: drifted_env  # type: ignore[assignment]

        preflight_phase(root=tmp_path, state=state, phase="execute", engine_root=target_root)
    finally:
        env_module.resolve_execution_environment = actual_resolve  # type: ignore[assignment]

    assert state["meta"]["latest_engine_overlap_waiver_id"] == waiver["id"]
    drift = next(
        item
        for item in state["meta"]["engine_isolation"]["drift"]
        if item.get("field") == "engine_signature"
    )
    assert drift == {
        "field": "engine_signature",
        "pinned": env.engine_signature,
        "observed": "sha256:after",
        "phase": "execute",
    }


def test_preflight_phase_refuses_signature_only_drift_without_valid_waiver(tmp_path: Path) -> None:
    target_root = tmp_path / "repo"
    _init_repo(target_root)
    state = {"config": {"project_dir": str(target_root)}, "meta": {}}
    ...
    assert excinfo.value.code == "engine_pin_drift"
    assert set(excinfo.value.extra["drift"]) == {"engine_signature"}


@pytest.mark.parametrize(
    ("field", "replacement"),
    [
        ("engine_commit", "different-commit"),
        ("engine_root", "other-engine"),
    ],
)
def test_preflight_phase_refuses_root_or_commit_drift_even_with_valid_waiver(
    tmp_path: Path,
    field: str,
    replacement: str,
) -> None:
    target_root = tmp_path / "repo"
    other_engine = tmp_path / replacement
    _init_repo(target_root)
    ...
    try:
        ...
        if field == "engine_root":
            drifted_env = replace(env, engine_root=other_engine.resolve())
        else:
            drifted_env = replace(env, engine_commit=replacement)
        env_module.resolve_execution_environment = lambda *_args, **_kwargs: drifted_env  # type: ignore[assignment]
    ...
    assert excinfo.value.code == "engine_pin_drift"
    assert field in excinfo.value.extra["drift"]
