...
def test_resident_config_defaults_to_omp_glm_52() -> None:
    config = ResidentConfig()
    env_config = ResidentConfig.from_env({})

    assert config.model_provider == "omp"
    assert config.model_name == "zai/glm-5.2"
    ...
    assert env_config.model_provider == "omp"
    assert env_config.model_name == "zai/glm-5.2"
