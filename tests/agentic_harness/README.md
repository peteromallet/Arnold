...
python -m tests.agentic_harness.runner --mode structural --actor fake --tag run
```
...
evidence packs under `tests/agentic_harness/`; the `evidence/` path used in scenario YAML is
the frozen-evidence subdirectory inside each run report.

## How to add a scenario

1. Write a user-shaped brief in `tests/agentic_harness/briefs/<name>.md`.
2. Create a scenario YAML in `tests/agentic_harness/scenarios/<name>.yaml` referencing the brief.
...
4. Run the scenario: `python -m tests.agentic_harness.runner --name <name>`.
