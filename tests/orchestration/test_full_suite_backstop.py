...
def _suite_result(
    ...
    *,
    ...
    collected_ids = list(collected_ids if collected_ids is not None else failures + passes)
...
def _baseline(*, failing: list[str], collected: list[str] | None = None) -> dict[str, object]:
    return {
...
def test_evaluate_full_suite_backstop_delta_gate_matrix(
    ...
    baseline = _baseline(failing=["tests/test_a.py::test_a"], collected=["tests/test_a.py::test_a"])

    def run_current(*, failures: list[str], passes: list[str], collected_ids: list[str]):
        monkeypatch.setattr(
...
def test_run_full_suite_backstop_passed_copies_config_and_forces_full(
    ...
    def fake_run_suite(project_dir, config, *, phase, deadline_seconds, idle_seconds=None):
        del project_dir, deadline_seconds, idle_seconds
        assert phase == "full_suite_backstop"
        seen_config.update(config)
        return _suite_result(tmp_path, status="passed", passes=["tests/test_ok.py::test_ok"])
...
def test_run_full_suite_backstop_raised_runner_exception_is_error(
    ...
    result = run_full_suite_backstop(tmp_path / "plan", tmp_path, {"test_command": "pytest x"})
...
def _write_plan_state(root: Path, plan_name: str, *, current_state: str = "done") -> Path:
...
def test_run_chain_full_suite_backstop_shadow_records_and_advances(tmp_path: Path) -> None:
    spec_path = _write_chain_spec(tmp_path)
    plan_name = "plan-m1"
    ...
    assert json.loads((plan_dir / "full_suite_backstop.json").read_text()) == backstop_result
...
def test_run_chain_full_suite_backstop_enforce_blocks_before_commit(tmp_path: Path) -> None:
    spec_path = _write_chain_spec(tmp_path)
    plan_name = "plan-m1"
    plan_dir = _write_plan_state(tmp_path, plan_name, current_state="finalized")
    save_chain_state(
