...
def test_revise_merge_failure_falls_back_to_prior_floor(
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: (_ for _ in ()).throw(RuntimeError("derive failed")),
    )
...
def test_revise_replaces_stale_prior_full_radius_with_new_scoped_radius(
    ...
    repo.mkdir()
    (repo / "pkg").mkdir()
    (repo / "pkg" / "target.py").write_text("VALUE = 1\n", encoding="utf-8")
    (repo / "tests").mkdir()
    (repo / "tests" / "test_target.py").write_text("import pkg.target\n", encoding="utf-8")
    plan_dir = repo / ".megaplan" / "plans" / "plan"
    ...
    prior_radius = {
        "strategy": "full",
        "confidence": "low",
        "selectors": [{"kind": "path", "value": "tests/test_pipeline_run_cli.py"}],
        "changed_surfaces": ["old/missing.py"],
        "always_run": ["python -m pytest tests/test_pipeline_run_cli.py"],
        "full_suite_fallback": True,
        "rationale": "stale prior full radius",
    }
    candidate_radius = {
        "strategy": "scoped",
        "confidence": "high",
        "selectors": [{"kind": "path", "value": "tests/test_target.py"}],
        "changed_surfaces": ["pkg/target.py"],
        "always_run": ["python -m pytest tests/test_target.py"],
        "full_suite_fallback": True,
        "rationale": "revised scoped radius",
    }
    state: dict[str, Any] = {
        "iteration": 1,
        "current_state": "critiqued",
    ...
    monkeypatch.setattr(critique, "latest_plan_path", lambda *args, **kwargs: plan_path)
    monkeypatch.setattr(critique, "latest_plan_meta_path", lambda *args, **kwargs: meta_path)
    monkeypatch.setattr(critique, "read_json", lambda *args, **kwargs: {"test_blast_radius": prior_radius})
    monkeypatch.setattr(
        critique._pkg,
        "_run_worker",
        lambda *args, **kwargs: (FakeWorker(), "agent", "mode", False),
    ...
    radius = captured_meta_fields["test_blast_radius"]
    assert radius["strategy"] == "scoped"
    assert "tests/test_target.py" in [selector["value"] for selector in radius["selectors"]]
    assert "tests/test_pipeline_run_cli.py" not in str(radius)
