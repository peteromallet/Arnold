...
def test_valid_oversized_task_finding_survives_normalization_and_reaches_gate(tmp_path: Path) -> None:
    ...
    assert payload["flags"][0]["id"] == "scope-god-task-2"
    ...
    assert gate_input["flag_ids"] == ["scope-god-task-2"]
...
def test_partial_mapping_remains_blocking_at_finalize(tmp_path: Path) -> None:
    ...
    update_flags_after_gate(
        plan_dir,
        [{"flag_id": "scope-god-task-2", "action": "verify_fixed", "evidence": "plan_v2.md Step 2", "rationale": ""}],
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            "critique_finding_mapping_invalid",
        ),
