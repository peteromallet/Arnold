...
def _discord_provenance(*, root_run_id: str | None = None) -> dict[str, object]:
    ...
    if root_run_id is not None:
        provenance["root_run_id"] = root_run_id
    return provenance
...
def test_restart_delivers_exactly_one_terminal_reply_across_repeated_sweeps(
    ...
    assert (
        state["records"][0]["delivery"]["ownership"]
        == RESET_DELIVERY_RESTART_TERMINAL
    )
    assert state["records"][0]["delivery"]["discord_message_ids"] == ["reply-1"]


def test_detached_initiating_request_keeps_restart_receipt_but_owns_one_completion(
    tmp_path, monkeypatch
) -> None:
    monkeypatch.setenv(
        DELEGATION_CONTEXT_ENV,
        json.dumps(_discord_provenance(root_run_id="subagent-owner")),
    )
    now = datetime(2026, 7, 11, tzinfo=UTC)
    reservation = prepare_reset_notification(notification_root=tmp_path, now=now)
    mark_reset_succeeded(
    ...
    assert record["delivery"]["ownership"] == RESET_DELIVERY_DETACHED_COMPLETION
    assert record["initiator"]["source_record_id"] == "msg-source"
...
def test_manual_reset_uses_truthful_non_reply_fallback(tmp_path, monkeypatch) -> None:
    ...
    assert state["delivery"]["ownership"] == RESET_DELIVERY_RESTART_TERMINAL


def test_observed_interaction_and_detached_restart_race_emits_no_operational_duplicate(
    tmp_path, monkeypatch
) -> None:
    now = datetime(2026, 7, 11, tzinfo=UTC)
    monkeypatch.delenv(DELEGATION_CONTEXT_ENV, raising=False)
    monkeypatch.setenv(RESET_FALLBACK_CONVERSATION_ENV, "discord:dm:301463647895683072")
    interaction = prepare_reset_notification(
        notification_root=tmp_path,
    ...
    detached = prepare_reset_notification(
        notification_root=tmp_path,
    ...
    assert {row["delivery"]["ownership"] for row in records} == {
        RESET_DELIVERY_EPHEMERAL_INTERACTION,
        RESET_DELIVERY_DETACHED_COMPLETION,
    }
    assert all(row["restart"]["status"] == "succeeded" for row in records)
    assert all(row["delivery"]["status"] == "suppressed" for row in records)
