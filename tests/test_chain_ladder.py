...
def test_engine_pin_drift_is_non_retryable_infrastructure_error() -> None:
    payload = {
        "success": False,
        "error": "engine_pin_drift",
        "message": "Megaplan engine identity changed since isolation evidence was pinned",
        "details": {"phase": "execute"},
    }

    classified = _non_retryable_infrastructure_error_payload("", json.dumps(payload))

    assert classified == payload


def test_pretty_engine_pin_drift_is_non_retryable_infrastructure_error() -> None:
    payload = {
        "success": False,
        "error": "engine_pin_drift",
        "message": "Megaplan engine identity changed since isolation evidence was pinned",
        "details": {
            "phase": "execute",
            "drift": {"engine_commit": {"pinned": "old", "observed": "new"}},
        },
    }
...
def test_infrastructure_error_stops_without_retry_or_authority_divergence(tmp_path: Path) -> None:
    ...
    def fake_drive(root, plan, spec, *, on_phase_complete=None, writer):
        ...
        return DriverOutcome(
            ...
            iterations=1,
            reason="engine_pin_drift: pinned engine commit differs from observed",
            last_phase="execute",
            blocking_reasons=["engine_pin_drift"],
        )
...
def test_ladder_does_not_infinite_loop_on_deterministic_failure(tmp_path: Path) -> None:
    ...
    with patch("arnold.pipelines.megaplan.chain._init_plan", side_effect=fake_init), patch(
        "arnold.pipelines.megaplan.chain.auto_drive", side_effect=fake_drive
    ), patch("arnold.pipelines.megaplan.chain._refresh_base_branch", lambda *a, **k: None):
...
def test_retry_milestone_resumes_resumable_current_plan(tmp_path: Path) -> None:
    ...
    with patch("arnold.pipelines.megaplan.chain._init_plan", side_effect=fake_init), patch(
        "arnold.pipelines.megaplan.chain._drive_plan", side_effect=fake_drive
    ), patch("arnold.pipelines.megaplan.chain._plan_state", return_value="finalized"), patch(
