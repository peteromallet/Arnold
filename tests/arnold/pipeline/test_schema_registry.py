...
def test_generic_schema_registry_exports_no_megaplan_root_resolution() -> None:
    import arnold.pipeline.schema_registry as schema_registry

    assert not hasattr(schema_registry, "create_contract_schema_registry")
    assert not hasattr(schema_registry, "resolve_contract_schema_project_root")
    assert not hasattr(schema_registry, "derive_project_root_from_plan_dir")
