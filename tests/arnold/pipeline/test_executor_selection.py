...
def _write_state(tmp_path: Path, marker: str) -> None:
    (tmp_path / "state.json").write_text(
        json.dumps({"meta": {"executor": marker}}),
        encoding="utf-8",
    )


def test_unmarked_runs_use_graph_executor(tmp_path: Path, monkeypatch) -> None:
...
def test_native_marker_requires_runtime_flag(tmp_path: Path, monkeypatch) -> None:
...
def test_native_dispatch_requires_exact_marker(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("ARNOLD_NATIVE_RUNTIME", "1")
    step = _HaltStep()
    calls: list[Any] = []
