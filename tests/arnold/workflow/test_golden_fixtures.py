...
EXPECTED_CASE_CONTRACTS = {
    ...
    "tiebreaker": {
        "routes": {"branch-a", "branch-b", "tiebreaker"},
        "capabilities": {"agent:judge", "artifact:write", "reducer:tournament"},
        "reentry_ids": {"tiebreaker:round-1", "tiebreaker:winner"},
        "policy_slots": {"budget", "fanout", "idempotency", "retry"},
        "overlay_types": {"explicit-tiebreaker-round"},
        "edge_semantics": {
            "candidate branch participates in explicit tie resolution",
            "single deterministic winner is selected after ambiguous branch scores",
        },
        "node_semantics": {
            "continuation receives only the selected branch",
            "resolve equal-ranked branch recommendations without implicit ordering",
        },
    },
}
