"""Tests for Step 9A/9B/9C (T17) and Steps 11B/12B/12C (T20).
...
Steps 11B/12B/12C: action_gate double-fenced authority gate.
"""
...
# ── T20: ActionGate Steps 11B/12B/12C ───────────────────────────────────────


class TestActionGateStep11B:
    """Step 11B: action_gate rereads RA/Custody/WBC at action time."""

    def test_shadow_mode_when_family_not_enforced(self):
        """Non-enforced family returns SHADOW_AUTHORIZED or SHADOW_BLOCKED."""
        from arnold_pipelines.megaplan.custody.action_gate import (
            ActionGate, ActionGateConfig, ActionFamily,
            ActionGateVerdict,
        )

        gate = ActionGate(
            config=ActionGateConfig(
                enforced_families=frozenset(),  # Nothing enforced
            ),
        )
    ...
    def test_enforced_family_blocks_without_ra(self):
        """Enforced family blocks when RA is not satisfied."""
        from arnold_pipelines.megaplan.custody.action_gate import (
            ActionGate, ActionGateConfig, ActionFamily,
            ActionGateVerdict,
        )
        from arnold_pipelines.run_authority.current_source import (
            CurrentSourceResult, DENIED,
        )

        # RA provider returns DENIED
        ra_result = CurrentSourceResult(DENIED, "no grant", {})

        class FakeGate(ActionGate):
            def _reread_wbc_evidence(self, ref):
                from arnold_pipelines.megaplan.custody.action_gate import (
                    WbcEvidence, WbcEvidenceKind,
                )
                return WbcEvidence(kind=WbcEvidenceKind.STORE, attempt_reference="att-1")
        ...
        try:
            ...
            decision = gate.evaluate(
                ...
                ra_request=CurrentSourceRequest(
                    run_id="r1", run_revision="rev1",
                    coordinator_attempt_id="c1",
                    grant_id="g1", fence_token="f1",
                    subject_attempt_id="s1", decision_id="d1",
                ),
    ...
    def test_enforced_family_blocks_without_custody(self):
        """Enforced family blocks when custody lease is not active."""
        from arnold_pipelines.megaplan.custody.action_gate import (
            ActionGate, ActionGateConfig, ActionFamily,
            ActionGateVerdict, WbcEvidence, WbcEvidenceKind,
        )
        from arnold_pipelines.run_authority.current_source import (
            CurrentSourceResult, SATISFIED,
        )

        class FakeGate(ActionGate):
            def _reread_wbc_evidence(self, ref):
                return WbcEvidence(kind=WbcEvidenceKind.STORE, attempt_reference="a1")

        gate = FakeGate(
            config=ActionGateConfig(
                enforced_families=frozenset({ActionFamily.CLOUD}),
            ),
            custody_lease_provider=lambda aid: None,  # No lease
        )

        import arnold_pipelines.megaplan.custody.action_gate as ag_mod
        original = ag_mod.evaluate_current_source
        ag_mod.evaluate_current_source = lambda v, r: CurrentSourceResult(SATISFIED, "ok", {})
        try:
            decision = gate.evaluate(
                action_family=ActionFamily.CLOUD,
                action_target="deploy",
                custody_attempt_id="att-1",
                wbc_attempt_reference="a1",
            )
        finally:
            ag_mod.evaluate_current_source = original
...
class TestActionGateStep12B:
    """Step 12B: reject synthetic or projection-only WBC evidence."""

    def test_synthetic_wbc_reference_rejected(self):
        """Synthetic wbc-ref-* references are rejected in enforced mode."""
        from arnold_pipelines.megaplan.custody.action_gate import (
            ActionGate, ActionGateConfig, ActionFamily,
            ActionGateVerdict,
        )
        ...
        gate = ActionGate(
            ...
            custody_lease_provider=lambda aid: MagicMock(lease_id="l1", custody_epoch=1),
        )
        ...
        try:
            decision = gate.evaluate(
                action_family=ActionFamily.GIT,
                action_target="push",
                wbc_attempt_reference="wbc-ref-synthetic-001",
                custody_attempt_id="att-1",
            )
        finally:
            ag_mod.evaluate_current_source = original
    ...
    def test_missing_wbc_evidence_blocked(self):
        """Missing WBC evidence blocks in enforced mode."""
        from arnold_pipelines.megaplan.custody.action_gate import (
            ActionGate, ActionGateConfig, ActionFamily,
            ActionGateVerdict,
        )
        ...
        gate = ActionGate(
            ...
            custody_lease_provider=lambda aid: MagicMock(lease_id="l1"),
        )
    ...
    def test_store_evidence_is_authoritative(self):
        """WBC evidence from the store is authoritative."""
        from arnold_pipelines.megaplan.custody.action_gate import (
            WbcEvidence, WbcEvidenceKind,
        )

        store_ev = WbcEvidence(kind=WbcEvidenceKind.STORE, attempt_reference="a1")
        assert store_ev.is_authoritative is True

        synthetic_ev = WbcEvidence(kind=WbcEvidenceKind.SYNTHETIC)
        assert synthetic_ev.is_authoritative is False

        outbox_ev = WbcEvidence(kind=WbcEvidenceKind.OUTBOX)
        assert outbox_ev.is_authoritative is False

    def test_wbc_store_provider_reads_reservation(self):
        """WBC store provider returns STORE evidence when reservation exists."""
        from arnold_pipelines.megaplan.custody.action_gate import (
            ActionGate, ActionGateConfig, ActionFamily,
            ActionGateVerdict, WbcEvidenceKind,
        )
        from arnold_pipelines.run_authority.current_source import (
            CurrentSourceResult, SATISFIED,
        )
        ...
        mock_reservation.global_logical_effect_key = "glek:abc"
        mock_reservation.attempt_id = "att-1"
        mock_reservation.effect_identity.effect_family = "git"
        ...
        gate = ActionGate(
            ...
            custody_lease_provider=lambda aid: MagicMock(lease_id="l1"),
            wbc_store_provider=lambda ref: mock_reservation,
        )
...
class TestActionGateStep12C:
    """Step 12C: enforcement staged by action family."""

    def test_git_family_independent_from_cloud_family(self):
        """Enabling GIT does not enforce CLOUD."""
        from arnold_pipelines.megaplan.custody.action_gate import (
            ActionGateConfig, ActionFamily,
        )
    ...
    def test_default_config_enforces_nothing(self):
        """Default config is shadow-only (no enforcement)."""
        from arnold_pipelines.megaplan.custody.action_gate import (
            ActionGateConfig, ActionFamily,
        )
        ...
        for family in ActionFamily:
            assert not config.is_enforced(family)

    def test_diagnostics_preserved_in_decision(self):
        """Diagnostics include action family, target, and schema version."""
        from arnold_pipelines.megaplan.custody.action_gate import (
            ActionGate, ActionGateConfig, ActionFamily,
        )
        ...
        decision = gate.evaluate(
            ...
            action_target="dispatch_effect",
        )

        diag = decision.result.diagnostics
        assert diag["action_family"] == "effect"
        assert diag["action_target"] == "dispatch_effect"
        assert "gate_schema_version" in diag
        assert decision.timestamp  # Non-empty

    def test_all_families_can_be_enforced(self):
        """All action families can be enabled together."""
        from arnold_pipelines.megaplan.custody.action_gate import (
            ActionGateConfig, ActionFamily,
        )
        ...
        for family in ActionFamily:
            assert config.is_enforced(family)
