...
class TestBuildPipelineSymbol:
    ...
    def test_build_pipeline_accepts_name_kwarg(self) -> None:
        ...
        pipeline is returned, but 'name' is still accepted as a kwarg."""
        from arnold.pipelines.deliberation import build_pipeline
        pipeline = build_pipeline(name="custom_deliberation")
        assert pipeline.entry == "manifest_introspection"
        assert "manifest_introspection" in pipeline.stages
