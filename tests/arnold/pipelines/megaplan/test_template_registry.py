...
#: Phases that must have builder=None (either because builder comes later
#: or because no builder is ever required).
...
class TestBuilderPresence:
    """Builders are None for all phases at T2 (added in T3)."""

    def test_all_builders_are_none_at_t2(self) -> None:
        """No phase should have a builder yet — T3 adds them."""
        for phase in _ALL_PHASES:
            reg = get_template_registration(phase)
            assert reg is not None
            assert reg.builder is None, (
                f"{phase}: builder is not None at T2 — builders are added in T3"
            )

    def test_step_contract_template_builder_is_none(self) -> None:
        """``StepContract.template_builder`` is None for every contract."""
        for phase, contract in STEP_CONTRACTS.items():
            builder = contract.template_builder
            assert builder is None, (
                f"{phase}: StepContract.template_builder is not None at T2"
            )
