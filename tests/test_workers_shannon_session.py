...
def test_shared_session_planner_import_has_no_tmux_side_effects() -> None:
    sys.modules.pop("megaplan.workers.shannon_session", None)
    sys.modules.pop("megaplan.workers.shannon", None)
    sys.modules.pop("megaplan.runtime.process", None)

    module = importlib.import_module("megaplan.workers.shannon_session")

    assert module.plan_session is not None
    assert "megaplan.workers.shannon" not in sys.modules
    assert "megaplan.runtime.process" not in sys.modules


def test_shannon_reexports_shared_session_planner() -> None:
    import megaplan.workers.shannon as shannon
    import megaplan.workers.shannon_session as shared
...
def test_shared_session_planner_pins_new_resume_clear_compact_parity() -> None:
    ...
    from megaplan.workers.shannon_session import _serialize_session_plan, plan_session
...
def test_shared_session_planner_probability_threshold_matches_tmux_selector() -> None:
    ...
    from megaplan.workers.shannon import ShannonConfig
    from megaplan.workers.shannon_session import plan_session
