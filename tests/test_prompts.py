...
def test_write_gate_template_known_flag_ids_prepopulation(tmp_path: Path) -> None:
    """`_write_gate_template` pre-populates ``known_flag_ids`` from the flag registry."""
    from arnold.pipelines.megaplan.prompts.gate import _write_gate_template

    plan_dir, state = _scaffold(tmp_path)
    # _scaffold writes FLAG-001 via save_flag_registry
    output_path = _write_gate_template(plan_dir, state)

    data = read_json(output_path)
    assert "known_flag_ids" in data
    assert "FLAG-001" in data["known_flag_ids"]


def test_write_gate_template_no_flag_registry(tmp_path: Path) -> None:
    """`_write_gate_template` returns empty ``known_flag_ids`` when no flag registry exists."""
    from arnold.pipelines.megaplan.prompts.gate import _write_gate_template
