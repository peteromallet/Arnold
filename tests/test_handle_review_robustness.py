...
def _cap_reached_state() -> dict[str, object]:
    # Three needs_rework cycles >= default cap (3) so the next outcome is the
    # force-proceed branch.
    return {
        "history": [
            {"step": "review", "result": "needs_rework"},
            {"step": "review", "result": "needs_rework"},
...
def test_force_proceed_with_unresolved_must_escalates_to_recoverable_blocked(
    ...
    """DEFECT 2: a real MUST blocker at the rework cap must NOT ship as done."""
    issues: list[str] = []
    ...
    # The force-proceed-to-done message must NOT be the outcome.
...
def test_force_proceed_with_only_cosmetic_items_still_ships_done(
    ...
    """DEFECT 2 healthy path: should/minor residue still force-proceeds to done."""
    issues: list[str] = []
    result, next_state, next_step = megaplan.handlers._resolve_review_outcome(
        tmp_path,
        "needs_rework",
