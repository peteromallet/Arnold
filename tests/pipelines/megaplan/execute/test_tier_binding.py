...
from megaplan.execute._binding.tier import (
    COMPLEXITY_SCALE,
    select_batch_tier,
...
class TestValidateTaskComplexity:
    ...
    def test_complexity_out_of_range_high(self):
        """Complexity 6 (above range) returns an error message."""
        task = {
            "complexity": 6,
            "complexity_justification": "Justification provided.",
        }
...
class TestSelectBatchTier:
    ...
    def test_select_batch_tier_returns_tier_ordinal(self):
        """``select_batch_tier`` returns an integer in 1..5."""
        finalize_data = {
            "tasks": [
                {
                    "id": "T1",
...
# ---------------------------------------------------------------------------
# AST scan — select_batch_tier is the only call site referencing 1..5 scale
# ---------------------------------------------------------------------------


def test_tier_module_only_scale_reference_is_select_batch_tier():
    """``select_batch_tier`` is the only call site referencing the 1..5 scale.
    ...
    """
    import megaplan.execute._binding.tier as mod

    source = textwrap.dedent(open(mod.__file__).read())
    tree = ast.parse(source)

    # Acceptable names that may reference 1..5
    ...
    for node in ast.walk(tree):
        if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef, ast.ClassDef)):
            if node.name in _allowed_references:
                continue
            # Search this node's body for 1..5 pattern
            for sub in ast.walk(node):
                if isinstance(sub, ast.Constant) and isinstance(sub.value, str):
                    if "1..5" in sub.value:
                        pytest.fail(
                            f"1..5 scale referenced outside allowed call sites "
                            f"('COMPLEXITY_SCALE', 'validate_task_complexity'): "
                            f"found in {node.name!r} at line {sub.lineno}: {sub.value!r}"
                        )

    # Also verify select_batch_tier does not directly contain 1..5 literal
    # (it delegates to compute_batch_complexity which may have it, but the
    # tier module shouldn't duplicate it in select_batch_tier)
    for node in ast.walk(tree):
        if isinstance(node, ast.FunctionDef) and node.name == "select_batch_tier":
            for sub in ast.walk(node):
                if isinstance(sub, ast.Constant) and isinstance(sub.value, str):
                    if "1..5" in sub.value:
                        pytest.fail(
                            f"select_batch_tier should not embed 1..5 literal; "
                            f"found at line {sub.lineno}: {sub.value!r}"
                        )


def test_tier_module_exports_complexity_scale():
    """The 1..5 scale is accessible as ``COMPLEXITY_SCALE``."""
    assert COMPLEXITY_SCALE == frozenset({1, 2, 3, 4, 5})
