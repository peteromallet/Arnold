...
| `_NON_APPLYABLE_FORBIDDEN_KEYS` / `_strip_non_applyable_forbidden_fields` route guard (`_CLARIFY_FORBIDDEN_KEYS` legacy alias) | Backend non-applyable route contract guard | `vibecomfy/comfy_nodes/agent/routes.py` uses `_NON_APPLYABLE_FORBIDDEN_KEYS` and `_strip_non_applyable_forbidden_fields` as the active production route-envelope guard for clarify/noop responses; `_CLARIFY_FORBIDDEN_KEYS` is retained only as a no-production-caller legacy alias for documentation/test traceability. | `tests/test_comfy_nodes_agent_edit.py` clarify/noop route tests and `tests/browser/payload_contracts.test.mjs` forbidden-field checks. | N/A — this is an enforcement guard, not a compatibility alias. Keep `_CLARIFY_FORBIDDEN_KEYS` only while ledger/test traceability needs the old name. |
