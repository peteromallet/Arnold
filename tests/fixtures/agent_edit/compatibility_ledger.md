...
| `_CLARIFY_FORBIDDEN_KEYS` route guard | Backend non-applyable route contract guard | `vibecomfy/comfy_nodes/agent/routes.py` defines clarify/noop forbidden-key sets that prevent candidate/apply fields from leaking into non-applyable responses. | `tests/test_comfy_nodes_agent_edit.py` clarify/noop route tests and `tests/browser/payload_contracts.test.mjs` forbidden-field checks. | N/A — this is an enforcement guard, not a compatibility alias. Change only if the non-applyable route contract changes. |
