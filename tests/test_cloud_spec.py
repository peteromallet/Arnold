...
def test_load_spec_parses_extra_repos_and_chain_session(tmp_path: Path) -> None:
    """Spec with top-level extra_repos and chain.chain_session parses correctly."""
    payload = _base_spec(mode="chain")
    payload["extra_repos"] = ["/workspace/lib", "/workspace/shared"]
    payload["chain"]["chain_session"] = "my-session"  # type: ignore[index]
    spec = load_spec(_write_spec(tmp_path, payload))
    assert spec.extra_repos == ["/workspace/lib", "/workspace/shared"]
    assert spec.chain is not None
...
def test_load_spec_rejects_extra_repos_contains_non_string(tmp_path: Path) -> None:
    """Malformed extra_repos (contains non-string element) raises validation error."""
    payload = _base_spec(mode="chain")
    payload["extra_repos"] = ["/workspace/lib", 42]
    with pytest.raises(CliError, match="extra_repos\\[1\\].*non-empty string"):
        load_spec(_write_spec(tmp_path, payload))


def test_load_spec_rejects_extra_repos_contains_empty_string(tmp_path: Path) -> None:
    """Malformed extra_repos (contains empty string) raises validation error."""
    payload = _base_spec(mode="chain")
    payload["extra_repos"] = ["/workspace/lib", ""]
    with pytest.raises(CliError, match="extra_repos\\[1\\].*non-empty string"):
        load_spec(_write_spec(tmp_path, payload))
