...
def test_project_events_collapses_cross_store_envelope_copies() -> None:
    """A migration copy must not create a second sequence prefix."""
    ...
    store = _FakeStore(
        (
            StoredEvent(
                kind="state_written",
                phase=None,
                payload={"state": {"current_state": "initialized"}},
                seq=0,
                transaction_id="tx-0",
                source="file",
            ),
    ...
    assert [event["seq"] for event in projected] == [0, 1]
    assert [event["transaction_id"] for event in projected] == ["tx-0", "tx-1"]
