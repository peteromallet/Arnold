...
def test_watchdog_routes_setup_deviations_through_canonical_environment_custody() -> None:
    text = _wrapper("arnold-watchdog")
    ...
    assert 'launch_chain_tick "$session" "$workspace" "$remote_spec" "$report_items"' in text
    assert 'environment_gone_check "$session" "$workspace" "$remote_spec_path"' in text
    assert 'persist_environment_gone_outcome "$session" "$workspace" "$remote_spec"' in text
    assert '"env_gone_pending" "env-gone strike $env_gone_out/$ENV_GONE_STRIKES' in text
    assert '"environment_gone" "workspace/spec wiped; retired after env-gone strikes"' in text
    assert 'repair_unintended_stop "$report_items" "$session" "$workspace" "$remote_spec" "workspace_missing:' not in text
    assert 'repair_unintended_stop "$report_items" "$session" "$workspace" "$remote_spec" "spec_missing:' not in text
