...
def test_activity_phase_repairing_becomes_repair(fx):
    fx.add_session("s1", plan_name="planA")
    fx.add_chain_health(
    ...
    entry = _by_session(snap, "s1")
    assert entry["status"] == "repairing"
    assert entry["activity_phase"] == "repair"


def test_repair_state_active_when_repairing(fx):
    fx.add_session("s1", plan_name="planA")
    fx.add_chain_health(
        "s1", current_plan_name="planA", last_state="error",
        updated_at=NOW - timedelta(hours=3),
    ...
    snap = fx.build()
    entry = _by_session(snap, "s1")
    assert entry["repair_state"] == "active"
