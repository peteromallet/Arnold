...
2. **The superfixer** keeps the chains alive + advancing. It is three components that act on their own:
   - **1h watchdog** (`arnold-watchdog`, tmux `watchdog`) — scans every cloud session ~hourly, syncs the editable install, treats any unintended nonterminal condition as repairable, **background-dispatches a Kimi goal operator** to root-cause + fix + relaunch, and falls back to a direct tmux relaunch only when repair is unavailable or already failed. Writes `/workspace/watchdog-report.json`.
   ...
   - **6h progress auditor** (`arnold-progress-auditor`) — model-judged (Codex orchestrator + DeepSeek subagents); reviews the last 6h per plan plus watchdog reports. If an unintended stop/stall has no active or recent repair attempt after ~1h, it treats that as a **superfixer bug**, fixes the watchdog/repair trigger/auditor on `editible-install`, pushes, refreshes, and lets the watchdog relaunch. It also fixes genuine non-critical plan deadlocks or documents passive/manual ones. Writes `/workspace/audit-reports/`.
...
- Every session is either **alive + advancing** (fresh `events.ndjson`, milestones progressing), **expected-manual** (for example review-policy PR wait), or **actively under repair** (a Kimi operator running for it, making real progress — not spinning).
- Any unintended stop/stall that is neither expected-manual nor under repair within one watchdog tick is a superfixer failure.
- The 6h auditor lands `FIXED <sha>` or `PASSIVE` verdicts and, when it fixes, pushes to `editible-install` (the watchdog then relaunches). If the 1h watchdog did not try repair for an unintended stop, the auditor fixes the watchdog/trigger rather than the individual epic.
...
- **Green**: watchdog alive, report fresh (<1h), `sessions_seen` covers every marker, every session's newest event is fresh (minutes), expected-manual, **or** under active repair, deadlock count flat, audit verdicts landing. The superfixer is minding itself; do nothing.
- **Red**: any of — report stale (>1h) or missing sessions, an unintended stop/stall neither advancing nor under repair, a repair running but the session not progressing for a long time, deadlock count climbing, no audit verdicts for >6h.
...
3. **A session is neither advancing nor under repair** (the superfixer is not noticing it). First decide if it is expected-manual (`merge_policy: review/manual`, explicit product decision, or active human gate after repair exhausted). If not, check the report: is the session in `items[]` and does it show `repair_dispatched`/`repair_running`? If not, its marker/spec/health detection or repair trigger is broken. Verify the marker at `/workspace/.megaplan/cloud-sessions/<session>.json`, that `ARNOLD_REPAIR_TRIGGER_ENABLED=1` is set, and that `kimi_operator_running`/health detection sees it. If detection or dispatch is broken, that is a superfixer flaw — go to step 4.
