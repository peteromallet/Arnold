...
| `handle_verify_human` | `handlers/verifiability.py:10-91` | 82 | ~98% | State validation, criterion lookup (by index or name), verification persistence to `human_verifications.json`, deferred-must detection, state advancement to `STATE_DONE` when all verified | **No** — operates on `STATE_AWAITING_HUMAN_VERIFY`, which is a terminal state. Not part of the planning pipeline's active stages. |
...
Sprint A's `HumanDecisionStep` creates `awaiting_user.json` de novo. The `handle_resume` flow in `cli.py` will check for `awaiting_user.json` first; if absent, fall through to `ResumeCursor.load()`.
