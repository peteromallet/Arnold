...
> **Scope.** This document covers Sweep 3 from [`docs/decorator_template_emitter_completion.md`](decorator_template_emitter_completion.md): generating typed Python wrappers for ComfyUI custom-node packs so that template emission stops falling back to `raw_call('UnknownClass', '<node_id>', widget_0=...)` whenever a class is missing from `vibecomfy/nodes/`.
...
- Strategy doc: [`docs/decorator_template_emitter_completion.md`](decorator_template_emitter_completion.md) — Sweep 3
