...
All milestones run the built-in `all-codex` profile with `vendor: codex`. This is
intentional: `vendor: codex` alone does not disable tier-model execute routing in
some profiles, so the chain pins the profile to avoid Hermes/DeepSeek execution.

| # | Milestone | Profile | Robustness | Depends on |
|---|---|---|---|---|
| M1 | Triage & safety net (tests fully green, parity fixtures 4→9, dead code gone, hygiene) | `all-codex` | full | — |
| M2 | Shared foundation layer (AST/graph/link utils — the kernel) | `all-codex` | thorough/high | M1 |
| M3 | Collapse schema/validation triad + decompose `provider.py` | `all-codex` | thorough/high | M2 |
| M4 | Collapse 3 eval modules + unify diagnostics | `all-codex` | full | M2 |
| M5a | Split `emitter.py` (3304 LOC) | `all-codex` | thorough/high | M2 (+M4 diag base) |
| M5b | Split `session.py` (1379 LOC) + fix error architecture | `all-codex` | thorough/high | M4 |
| M6 | Convention & API enforcement (**code only**) | `all-codex` | full | M2–M5 |
| M7 | Documentation reconciliation (**docs only**) | `all-codex` | full | M6 |
| — | publish-shared-branch (verify, RESULTS.md, push, open PR) | `all-codex` | light | all |
...
- **Revised 2026-05-24** after an independent two-reviewer sense-check (Claude Opus +
  ...
  - M1 tier raised `directed`→`premium` (the `template_index` near-miss shows M1 needs
    judgment), then the full chain was pinned to `profile: all-codex` + `vendor: codex`
    so execution cannot fall through to Hermes tier models.
