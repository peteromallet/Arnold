# Architecture

Design notes and architecture proposals for VibeComfy authoring, graph editing,
composition, and in-graph Python execution.

- [ContextVar pattern](contextvars_pattern.md)
- [M4 resolution context](m4-resolution-context.md)
- [Python authoring edit surface](python_authoring_edit_surface.md)
- [Python composition DSL plan](python_composition_dsl_plan.md)
- [Python composition DSL review](python_composition_dsl_review.md)
- [Python on the graph](python_on_the_graph.md)
- [VibeComfy exec node design](vibecomfy_exec_node_design.md)
