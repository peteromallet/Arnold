...
## G5 revisions — done (a76d12041e); routing switch to ox-alpha
...
## DEEP check-ins #1–#4 (standing 3-hourly REPORT audits)

- #1 — ox-alpha, receipt `mrc-9cb47a6b…`, read-only @ integration `6c75652572`. North-star GREEN (sole claim seam, sealed T1.x surfaces, T4.1–T4.3 authority chain, M5 report-only, 9/9 safety refs); code quality GREEN (sampled T6.2/T6.3/INT-1 coherent). Drift named: all-role ox-alpha routing makes end-stage reviews same-weights (independence procedural, not model-diverse). Saw T7.2 partials (~289 unexpected in shards 2–8). Recommendations: fce-bisect classification before any G7 work; run EVDIGEST manifest refresh immediately; reclaim disk; flag end-stage review routing explicitly. Both flagged items were then executed (T7.2d, EVDIGEST-2).
- #2 — ox-alpha, receipt `mrc-11e3a41d…`, @ `316d4da764`. Alignment GREEN — confirmed T7.2 stop discipline (`verdict="drift"` halted progression to T7.3/G7). Code quality GREEN. Drift named: non-card production mutations `be92de3932`/`f1b874ff8c`/`3342f56965` ("wip: preserve skill edits") replaced relative skill symlinks with absolute `/Users/peteromalley/...` paths (genuine production defect, owned by T7.2f) and rewrote launcher dispatch; routing flips accepted as operator-directed; Live Tree Authority consistently honored (launch pins deleted, seed store demoted to evidence, no STATE_SCHEMA_EPOCH).
- #3 — Codex Sol, receipt `mrc-65325f47…`, @ `a8479ed862`. **YELLOW implementation / RED promotion readiness.** fce is NOT an ancestor of HEAD (merge-base `e5d14662`, secret-remediation rewrite) — successor claim must cite J3/R002 rewrite map + byte-identical pre-boundary trees, never plain ancestry. Evidence drift: manifest still named `6c75652572`, test-shards.json still `a8058c22fd` while live HEAD moved. Named T7.2f's inversion of the ratified T4.3 typed-skip residual as an unadjudicated authority-contract change. Stale-manifest validator exit 0 does not prove current-head authority. Recommendation: STOP — refresh manifest/test-shards, adjudicate the T4.3 inversion, obtain post-repair broad result before T7.3/promotion.
- #4 — Codex Sol, receipt `mrc-34d493f6…`, @ `dc8276e060`. **RED promotion / YELLOW architectural convergence.** Found P0: `dc8276e060` breaks default-config promotion (generation-root mismatch between creation and verification roots) → seeded the GENROOT chain. Found noncanonical full shard execution sitting untracked in the evidence worktree → quarantined at `2b3a0d3a4b`. Named log gaps (check-ins #1–#3, T7.2 classification/salvage, SOL1, freeze chain) — closed by the sections below. Waiver guidance: PREEXISTING-222 fail-at-both is not a ratifiable blanket waiver without per-node signatures; the 26 fce-only greens belong in an annex, not a waiver table (sibling card scope).

## T7.2 — nine canonical shards once — VERDICT DRIFT (443 unexpected)

- First dispatch `mrc-5848880e…` (started 02:38:09Z) wall-clock killed before close. Shard9 re-run once verbatim + aggregate by T7.2c (`mrc-6001587e…`, exit 0, 1059s): `SHARD9_EXIT=1`, shards 1–8 untouched.
- Partition intact: every shard collected exactly 2383. All nine exited 1 — shard1 (7 fails ⊆ inventory) and shard9 expected; shards 2–8 expected 0.
- **Verdict drift: 443 unexpected failures** = 294 (shards 2–8) + 149 (shard9 beyond its 1 surviving known-fail). Only 8 of 24 inventoried failures actually failed (T7.1b repair flipped 13 frozen-time failures green).
- Top offender files: test_watchdog_wrappers.py 79 (generated babysitter script dies `bash syntax error` line 1871), test_runtime_lifecycle.py 69, test_chain_execution_binding.py 24, test_task_feasibility.py 20, test_replay_oracle.py 19, test_occurrence_adopt.py 16, test_reconcile_terminal_finalizer.py 15, test_post_m11_release_evidence.py 14, test_progress_auditor_controller.py 13.
- Aggregate `shards_run.json` (byte-identical copies, sha256-verified) + nine stdout captures committed at `ea7f0bcb2d` (INT-evdig-r006 pre-step). Expectations froze at ancestor `a8058c22fd`; run executed at `6c75652572`.

## T7.2d — shard failure classification — 443 = 97 + 86 + 38 + 222

- First dispatch `mrc-55541e81…` wall-clock killed; resume/incremental retry `mrc-7e8b9b00…` completed exit 0 (2855s). Commit-free; outputs are run-root artifacts (`evidence/shard_failure_classification.json`, `t72d_fce_node_level_results.json`, `t72d_node_split.json`).
- CONTRACT-DRIFT-INTENTIONAL 97 (pass@fce, assert retired behavior): T4.1 MutationCapability denials 22, T4.3 SHA-CAS deletion 26, T5.1 v2-budget admission 39, P2/P3 attestation/queue reshapes, T2.3 classifier renames.
- ENVIRONMENTAL 86 (fail@both, env-proven): 69× runtime_lifecycle — `arnold-runtime-create` hardcodes `/root/.pyenv/...` fallback interpreter at both commits; exporting `ARNOLD_GENERATION_PYTHON` flips a HEAD test green. Plus 17 post_m11/conformance nodes depending on untracked `.megaplan/initiatives` tree (rerun-verified).
- OURS-REGRESSION 38 (pass@fce, no retired-contract reading): committed absolute symlinks (`3342f56965`/`f1b874ff8c`/`be92de3932`), hermes-phase-worker purge deleting test-imported privates (6), wrapper test-assembly truncation (6), catalog/doc lag (7), `_FakeStore` growth (3), `all-codex` profile removal (2), lineage anchors severed by branch topology (4), message surfaces (2).
- PREEXISTING 222 (fail@fce identically, node-level measured): never-green debt incl. bash-syntax extractor truncation (59, exists at fce too), absent `arnold-repair-trigger` wrapper, pre-fce launch-seed gate (replay_oracle 19), `RunAuthorityView` signature drift, reconcile_terminal_finalizer 15.
- judgment_required trio routed onward: T4.3 seam non-refusal and cancellation-liveness `unknown` → SOL1-early; absolute-symlink targets → T7.2f. 26 fce-only flips excluded from totals as flaky/env (listed in JSON extras).

## Salvage chain T7.2e/f/g/h — wall-clock kills, hollow-commit refusal, landings

- Kills: T7.2e dispatches `mrc-1518b5b4…` (05:52Z) + `mrc-e5a4c010…` (05:55Z) and T7.2f dispatch `mrc-897708af…` (05:57Z) died mid-repair leaving verified-dirty disposable worktrees (detached `316d4da764`).
- T7.2e-commit (`mrc-d3c08906…`, exit 0): working diff byte-identical to `t72e-salvage.patch`; all 15 dirty files test-side; pass-after counts equal collect counts (JUnit XML evidence); zero collected-count delta vs HEAD → committed source `84cdd73d96` on `t72e-contract-drift-env-repairs`. Covered 7/16 contract-drift files; rest stayed open for T7.2g.
- HOLLOW-COMMIT REFUSAL — T7.2f-commit (`mrc-5be8f4e7…`, exit 0) verified the salvaged ours-regression diff but REFUSED to commit: the heredoc-aware `_extract_wrapper_function` fix was proven surgical (57/58 extractions byte-identical; sole delta `launch_chain_tick` passes `bash -n`) yet 6 `OURS-REGRESSION` params were still red — committing would ship a knowingly incomplete repair. Worktree left exactly as validated.
- T7.2f2 (`mrc-7f4caa2e…`, exit 0): landed the extractor fix + removed dangling `!` (test_watchdog_wrappers.py:12417) → committed source `5c303da00e` on `t72f-ours-regression-repairs`.
- T7.2g (`mrc-5d92c1ee…`, exit 0, 3071s, honestly scoped partial): committed `a080ccd9c4` on `t72g-remaining-repairs` — 7 files incl. runtime_lifecycle env fixture pinning `ARNOLD_GENERATION_PYTHON=sys.executable` (93/3) and T4.3 rebind fixes passing root CAS (import_root + interpreter, not SHA-256); one stray relative-path edit on the integration worktree reverted immediately; remainder superseded by T7.2h placement fix + SOL-freeze residency follow-up.
- T7.2h (`mrc-85736b82…`, exit 0, 2949s): committed `aaac569541` DIRECTLY atop operator interleave `b1cbfcfb0a` — git-rm of the 4 resurrected from-path artifacts (timeline proven: canonicalized `7093a0d14c` → stale regeneration `80d1c22c71` → resurrection `b2f560aac9`; 3/4 pairs byte-identical, 4th a strict subset), chain.yaml pointer restored, production residency fix (`os.path.abspath`, no symlink follow — POSIX venv bug since `92a31f0e18`), legacy-reference allowlist ratchet 133→165 (all `historical-non-shipped`). Manifest/selection bindings untouched.
- INT-t72repairs (`mrc-c3af8122…`): cherry-picked onto the post-T7.2h base with zero conflicts and patch-id proofs — `a2e6c69b6c`←`84cdd73d96`, `957755095a`←`a080ccd9c4`, `a8479ed862`←`5c303da00e`; guards T6.3 negative-authority 24 passed, T5.1 shard 23 passed; validator exit 0; HEAD `a8479ed862`. Task worktrees/branches deleted after reachability proof → source SHAs now DANGLING (mapping preserved in manifest `repair_sha_mapping`).

## SOL1-early — Sol authority adjudication (`mrc-f9dc5e17…`, Codex Sol, role SOL-REVIEW)

- Q1 T4.3 skip-vs-refuse: **REVERT** the T7.2f refusal; restore the ratified typed skip (`skipped:"no_chain_runtime_identity"`) — refusal fired after selector/marker publication, converting a legitimate no-op into torn resumable delivery.
- Q2 cancellation liveness: **RATIFY-WITH-FOLLOW-UP** — contract is `live while owned`, `expired after cancellation`, `unknown` only when conflicting identity suppresses ownership; failing assertion was pre-cancellation runner-identity interference → isolate the test, do not weaken expectations.
- Q3 fence-check/append TOCTOU: **RATIFY-AS-IS** — atomic ledger append + post-append recheck + deterministic replay join is the accepted bounded design; no cross-store lock.
- Q4 composite T7.2h review: (a) initiative deletions **RATIFY-AS-IS** (R100 moves; resurrected copies stale/subset); (b) generation residency **RATIFY-WITH-FOLLOW-UP** — basename equality insufficient, require exact trusted-root containment (→ dc8276e060 + GENROOT chain); (c) allowlist ratchet **RATIFY-AS-IS**.
- Dual-role caution: this receipt id is NOT the T7.2d classification card — that implementer is `mrc-7e8b9b00…`.

## SOL-FREEZE chain

- SOL-FREEZE (`mrc-fa3004d1…`): FAILED exit −15 — wall-clock kill after 1682.9s, zero output.
- SOL-FREEZE-2 (`mrc-d9a480cf…`, started 10:35:22Z, receipt never closed — commits verified by git graph): landed incrementally directly on the integration branch: `9da7f3a164` restore ratified T4.3 typed skip (freeze 1/3), `379fa7d219` isolate cancellation-liveness test from runner identity (2/3), `dc8276e060` generation residency requires trusted-root containment (3/3).

## GENROOT chain (default-config promotion P0 → unified generation root)

- Origin: DEEP check-in #4 P0 — watchdog promotion built proofs under `.megaplan/generations` while `_verify_dependency_generation_binding` fell back to the reference `runtime-venvs` root → `advance_generation` refused every legitimate default-config promotion.
- GENROOT-FIX attempts `mrc-4cd9d0a1…`/`mrc-7c908f9c…` wall-clock killed; attempt 3 (`mrc-7a02a83e…`, exit 0) diagnosed three competing config names/defaults and delivered `35a51751d3`: single `configured_generations_root()` resolver (precedence ARNOLD_RUNTIME_VENVS_DIR → legacy alias → `$ARNOLD_BASE_DIR/runtime-venvs`) threaded through creation and promotion callers; +275-line `test_promotion_default_config.py`.
- GENROOT-SOL (`mrc-fdb72fb3…`, Codex Sol): **RATIFY-WITH-FOLLOW-UP** — P0 fix ratified (do not revert); MUST finding GENROOT-001: legacy-only configuration still split creation (`arnold-runtime-create:130`, `arnold-gc-sweep:135`, `runtime_references.DEFAULT_GENERATION_ROOT`) from verification; end-to-end legacy-only + precedence tests demanded.
- GENROOT-001 (`mrc-76c579bc…`, exit 0): `ae401e3c74` closes the split — wrappers + `DEFAULT_GENERATION_ROOT` resolve through the shared helper; +4 append-only tests (cross-surface identical-resolution proof, legacy-only promotion).
- Integrations (zero-conflict, patch-id verified): INT-genroot `mrc-51559489…` → `2fe5263614` (also committed `2b3a0d3a4b` quarantine + stray patches); INT-genroot001 `mrc-9358aeef…` → `4243cc13a9` (current frozen authority head). Guards 24/23 each time; 168 tests green at final pick; validator exit 0 both times.

## EVDIGEST-2 + R006 — authority backfill and validator amendment

- EVDIGEST-2 (`mrc-5c27443f…`): manifest authority backfilled `c50ce0f77d…` → `6c75652572…`; tasks 1→20, gates 1→10 (G7 recorded PENDING), findings 2→10, allowances 20 (closed, digests recomputed). Source `8a5e59867a` (dangling) integrated as `e34434d171`.
- R006 (`mrc-16bb576e…`): validator amended for the 2026-08-21/22 cadence — historical ROUTES retained, `UNIVERSAL_ROUTE = ox-alpha` accepts either route everywhere, XHARD triple-phase check replaced by exactly-one-deciding-verdict gate rule; old validator flagged 16 issues against the backfilled manifest, amended validator passes. Source `572ae0db5f` (dangling) integrated as `316d4da764`; 23 validator tests green.
- INT-evdig-r006 (`mrc-66ca9e5d…`) integrated both plus pre-step log/artifact commit `ea7f0bcb2d` (T7.2 aggregate evidence).

## Noncanonical-shardrun quarantine + stray-edit reconciliation

- Quarantine: the first T7.2 run executed a FULL nine-shard pass at `6c75652572` although its brief required collect-only refreezing; its stdout captures were held untracked until `2b3a0d3a4b` committed them under `artifacts/shards-t72-6c75652572/` — deliberately distinct from the canonical `artifacts/shards/` set committed with the aggregate at `ea7f0bcb2d`. Named by DEEP check-in #4.
- Stray-edit reconciliation: INT-t72repairs found two stray test-file edits on the integration worktree — preserved as patches then discarded (`artifacts/integration-stray-test_launch_hermes_agent.patch`, `integration-stray-test_task_feasibility.patch`); uncommitted execution-log content preserved as patch and committed as `6b8f7e886e` (`integration-stray-execution-log.patch`). T7.2g and R006 implementers each self-reported a mid-run accidental integration-worktree edit and reverted immediately with verification. Earlier incidents already logged above (INT-t31 worktree removal; F-002/J3 lost-directive restoration at `0241c065`). Committed non-card mutations (`be92de3932`/`f1b874ff8c`/`3342f56965`) were named by check-in #2; their absolute-symlink defect was repaired within T7.2f.
...
### T74RESUME4 — honest halt at S9: uv.toml amendment executed, neutralized by candidate [tool.uv] config boundary
- S1–S8 carried PASS; all frozen helper digests + S13 script digest (0bee9c89…) + staged uv binary (b65f23a4…) + candidate HEAD (9fe05cda…) re-verified byte-identical at session start. Container epoch unchanged (45404872…, StartedAt 2026-08-17T14:31:15.428579551Z).
- Pre-attempt verification (scratch only): `uv help sync` documents top-level `--link-mode`; unknown `uv.toml` keys hard-rejected (strict parse); sanitized-env probes reproduced the resume3 signature (installed .so nlink=2) and showed an ancestor `/tmp/uv.toml` flips plain projects to copy mode (nlink=1). Literal in-root placement is impossible: `build()` raises `root_not_fresh` on any non-empty `--root`, and the project root is created inside the supervised flow — ancestor placement is the only card-compliant option.
- S9 single authorized attempt (launched 20:30:35Z): census → clone → **runtime-create SUCCEEDED again** (generation `05b40a47…` built past audioop-lts under `uv sync --frozen`) → died at phase=snapshot on the identical `hardlink_unsupported` mypyc `.so`. Supervisor group-proven, zero mutation.
- Root cause isolated post-mortem (bounded scratch bisects; one read-only-tree sync against the actual dead worktree with scratch cache/venv): full flow-env replicas all honor the ancestor config (links=1), but syncing against the real worktree reproduces links=2; tri-variate isolation is decisive — pyproject WITHOUT `[tool.uv]` links=1, comment-only `[tool.uv]` links=2 (ancestor ignored), `[tool.uv]` carrying explicit `link-mode = "copy"` links=1. The candidate's committed `[tool.uv]` table (pyproject.toml:103) makes uv 0.12.5 stop merging ancestor `uv.toml` above the project boundary, structurally neutralizing the amendment FOR THIS CANDIDATE.
- Both card halt-prongs triggered (uv.toml effectively ignored for this candidate AND snapshot still fails) → halted honestly per card; S10–S14 not run; no further remedies improvised. All escape hatches remain out of authority: in-project placement impossible pre-build / would be staged-candidate mutation; UV_LINK_MODE plumbing = candidate code; /etc/uv outside disposable roots; snapshot-rule weakening = ratified-contract change.
- Fence: epoch unchanged; oom=2 oom_kill=2 throughout; ambient `max` drifted 9766825→9847489 (~2.5h incl. ~97 GB protected-root census hashing) — counter-based fence restated as invariant. Staged `/tmp/uv.toml` removed post-halt (digest recorded in verdict). Verdict updated to T74RESUME4 (manifest receipt canary-rollback-t74-resume, validator exit 0). Evidence: evidence/artifacts/t74-resume4-s9-halt/.

### JUDG-S9 — second opinions (Grok 4.6 + Codex gpt-5.6-sol); blended decision
- Codex: primary A (candidate-code UV_LINK_MODE=copy + full T7.3 provenance reset), fallback C (strict named-operation rule), reject B.
- Grok: primary D (PATH-staged uv wrapper forcing UV_LINK_MODE=copy at spawn — PATH is allowlisted; install_sync inherits env at uv spawn, install_sync.py:579-585), fallback A (sandbox-authored, never inherited), reject B and literal C (prepare-split worse than A), reject permanent halt (D untried, in-authority).
- Blend adopted: D first; if D nlink probe or S9 fails -> A per Codex spec (new SHA, both candidates reinstall+supersede receipts, rerun T7.3 + identity-bound T7.4 stages). Evidence attestations required either way: effective link mode copy, nlink=1 across snapshot scope, wrapper bytes+sha recorded, _ENV_PASSTHROUGH not widened, snapshot/restore pass unchanged, zero protected-state delta. B is closed: no snapshot-rule amendment, now or later.

### T74RESUME5 — honest halt at S9 under JUDG-S9 remedy D: link-mode wall eliminated, supervisor budget exhausted
- S1–S8 carried PASS; all frozen helper digests + S13 script digest (0bee9c89…) + staged uv binary (b65f23a4…) + candidate HEAD (9fe05cda…) re-verified byte-identical at session start; container epoch unchanged (45404872…, StartedAt 2026-08-17T14:31:15.428579551Z); /tmp/uv.toml confirmed absent. T74RESUME4 dead debris digest-captured into t74resume5-debris-prestate.sha256 and removed for fresh roots.
- Remedy D staged exactly per blend: uv wrapper (`export UV_LINK_MODE=copy; exec <staged uv 0.12.5> "$@"`, sha256 641843ee…) at box /root/t74-resume5/uv-bin/uv + container /tmp/arnold-t74-resume5-uvwrap/uv, prepended to the flow PATH inside the UNCHANGED _ENV_PASSTHROUGH allowlist (canary_sandbox.py:165 read-verified); zero candidate-code change, zero snapshot-contract change.
- Grok probe PASSED before spending the attempt: [tool.uv]-bearing frozen cffi==2.0.0 project mirroring the candidate's neutralizing condition — baseline sync nlink>1 files=41 → wrapped sync nlink>1 files=0 (1543 files each).
- S9 single authorized attempt (launched 21:42:38Z): containment-audit (~97 GB census, cold page cache) consumed ~15 min; runtime-create SUCCEEDED again (generation `05b40a47…` rebuilt) with **nlink=1 across all 7992 generation files** — the resume3/resume4 hardlink_unsupported wall is mechanically gone in the real flow. At 21:59:02Z the card-pinned 900 s supervisor budget expired mid prepare/snapshot (copy-mode install I/O + full-tree snapshot scan no longer fit after the slow audit) → failed_before_snapshot (supervisor_timeout_sigkill), process group proven gone, nothing mutated. Observed verbatim: build CLI exited 0 reporting the structured timeout outcome; staged assert crashed KeyError 'candidate' on the failure-schema report (arc=1).
- Halted honestly per one-attempt/no-retry constraints; S10–S14 not run. Blend routing recorded: D failure → remedy A per Codex spec (candidate-code UV_LINK_MODE=copy + T7.3 provenance reset) requires fresh card authorization — not executed here. Scratch surfaces (/tmp/arnold-t74-resume5-probe, -uvwrap) removed post-halt with digests recorded; CLEAN_ROOT debris left in place as halt evidence.
- Fence: epoch unchanged; oom=2 oom_kill=2 throughout; ambient `max` drifted 9847489→9847687→9926860 across the session — counter-based fence restated as invariant. Verdict updated to T74RESUME5 (manifest receipt canary-rollback-t74-resume, status halted_s9_supervisor_budget_snapshot). Evidence: evidence/artifacts/t74-resume5-s9-halt/.

### T74RESUME5 — remedy D PROVEN (nlink census 0/7992); halt = supervisor budget
- Wrapper (sha 641843ee…) forced UV_LINK_MODE=copy; probe 41→0 nlink>1; real generation 7992 files nlink>1=0. hardlink_unreachable wall eliminated under candidate [tool.uv]. JUDG-S9 blend satisfied — remedy A (provenance reset) NOT required.
- S9 attempt died at card-pinned --supervisor-timeout 900: one-time cold-cache containment census (~97 GB, ~15 min) consumed the budget before build+snapshot completed. Structural cause cleared; cause is time budget, not mechanism.
- Card amendment (orchestrator): T74RESUME6 raises --supervisor-timeout to 2400 for the prepare/snapshot phase; all else identical. Staged assert KeyError 'candidate' on failure-schema noted (build CLI structured outcome recorded; non-blocking).

### T74RESUME6 — honest halt at S9: budget wall FIXED, hardlink wall stays gone; new advance_generation binding refusal exposed
- S1–S8 carried PASS; all nine frozen/amended helper digests + shipped attestation (5d50475e…) + S13 script (0bee9c89…) + staged uv binary (b65f23a4…) + candidate HEAD (9fe05cda…) re-verified byte-identical IN-CONTAINER at session start; epoch unchanged (45404872…, StartedAt 2026-08-17T14:31:15.428579551Z); /tmp/uv.toml absent. T74RESUME5 dead debris digest-captured into t74resume6-debris-prestate.sha256 (35191 entries) and removed for fresh roots.
- Card amendment executed exactly: `--supervisor-timeout 2400`; remedy D wrapper restaged byte-identical (sha256 641843ee…) at /root/t74-resume6/uv-bin/uv + container /tmp/arnold-t74-resume6-uvwrap/uv. Grok probe PASSED (baseline 41/63 nlink>1 → wrapped 0/63; defective v1 probe execution — census lines lost to a staging-edit defect, verdict never rendered — recorded honestly; v2 authoritative). `_ENV_PASSTHROUGH` re-read unchanged.
- S9 single authorized attempt (22:22:44Z): **budget wall FIXED** — prepare rc=0 (audit + copy-mode runtime-create + full pre-mutation snapshot all inside budget; no SIGKILL). Generation `05b40a47…` rebuilt fresh (created 22:36:15Z), 7992 files, **nlink>1 = 0** — remedy D mechanics hold end-to-end. Mutate worker then died rc=2 at phase promotion-adjacent-mutation on a NEW deterministic wall never before reached: `advance_generation refused` — dependency-generation proof binds `frozen_spec_sha256` = generation id (`05b40a47…`, recorded at create from the base tree) while the flow's own probe commit `e57c4f3d…` recomputes the spec digest to `440afcff…`. Supervisor anchor-verified restore reconstructed the pre-mutation baseline (ok=true, 16140 entries, extra_entries=[]); zero mutation escaped. Build CLI exited 0 reporting structured exit_2; assert crashed KeyError 'candidate' and its structured outcome was recorded per card amendment (not masking exercised as specified).
- Classification: candidate-flow defect (probe-commit vs generation-binding lifecycle) — fail-closed binding check working as coded against a flow that never rebuilds/selects the generation after its own probe commit. Outside card authority (candidate-code change ⇒ fresh card required), same boundary as every prior halt. Halted honestly per one-attempt/no-retry; S10–S14 not run.
- Fence: oom=2 oom_kill=2 throughout; ambient `max` drifted 9926860→10097926 across the session — counter-based fence restated as invariant. Verdict updated to T74RESUME6 (manifest receipt canary-rollback-t74-resume, status halted_s9_generation_binding_refused, validator exit 0). Evidence: evidence/artifacts/t74-resume6/.

### T74RESUME6 — budget+hardlink walls cleared; third wall = candidate generation-binding defect
- supervisor-timeout 2400 sufficient (prepare rc=0 incl. full pre-mutation snapshot, 16140 entries). Remedy D re-proven (0/7992 nlink>1). Supervisor restore anchor-verified, zero mutation.
- New wall: mutate worker rc=2 `advance_generation refused` — flow's probe commit (e57c4f3d) recomputes spec digest (440afcff) vs proof bound to generation id (05b40a47); no rebuild/select post-commit. Candidate-code defect class, first exposure (prior resumes died in prepare).
- Routing per JUDG-S9 blend -> remedy A class: candidate fix (rebuild/select generation post-commit OR scope spec digest to commit-independent inputs), new SHA, T7.3 re-receipt of BOTH candidates (supersede, not amend), rerun identity-bound T7.4 stages, then S9 resume. Dispatching T74GENFIX.
