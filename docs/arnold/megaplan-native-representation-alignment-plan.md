...
| Parallel critique lenses with fan-in | `run_parallel_critique(...)` hidden in critique handler. | Composition. | Composition M0/M1/M3/M6 | implemented (M3: `parallel_map` general construct + reducer + canonical critique-fanout node; M6: final lens-selection conformance) | `parallel_map`/fanout over selected checks plus reducer/fan-in is visible. | `test_runtime.py::test_parallel_map_reducer_receives_ordered_results_and_item_paths`, `test_graph_projection.py::test_parallel_map_becomes_dynamic_fanout_stage`, `valid_m3_canonical_megaplan_topology.py` (critique-fanout parallel_map node with reducer). M6 owns selected-lens wiring and full conformance. | Native trace lists child calls but source still invokes one handler. | Static topology snapshot includes untaken selected-lens branches. |
...
| Tiebreaker researcher/challenger path | `tiebreaker_run` / `tiebreaker_decide` handlers hide internal split. | Composition. | Composition M1/M3/M4/M6 | implemented (M3: nested workflow invocation with literal `id=` + canonical tiebreaker call-site with `call_site_path` metadata; M4: researcher/challenger split internals; M6: final conformance) | Tiebreaker is a declared subworkflow with researcher, challenger, decision, and parent promotion. | `test_source_compiler_api.py::test_source_compiler_m3_native_nested_workflow_requires_literal_call_site_id`, `test_source_compiler_api.py::test_source_compiler_m3_native_nested_workflow_validates_child_input_schema`, `test_canonical_megaplan_conformance.py::test_tournament_has_two_full_tiebreaker_rounds`, `valid_m3_canonical_megaplan_topology.py` (tiebreaker subworkflow node with `call_site_path: "tiebreaker"`). M4 owns internal split semantics; M6 owns final conformance. | One native node calls old tiebreaker handler. | Structural conformance fails for single handler-backed tiebreaker stage. |
...
| Dependency-aware execute batches | Execute DAG and batching live in `execute/batch.py`. | Composition, with platform worker hardening. | Composition M1/M3/M4/M6, Platform M1/M5/M6 | implemented (M3: `parallel_map` for execute batching + canonical execute-batches node with stable call-site identity; M4: DAG dependency internals; M6: full execute conformance) | Execution is not one opaque handler; dependency-aware runtime task batches are visible as a subworkflow or dynamic DAG primitive. | `valid_m3_canonical_megaplan_topology.py` (execute-batches parallel_map node, tiebreaker-execute-batches node), `test_canonical_megaplan_conformance.py::test_canonical_shape`. M4 owns DAG internals; M6 owns final execute conformance. | Execute remains one node with richer internal logging. | Structural conformance fails if execute is only one handler-backed stage. |
...
| Review parallel checks/fan-in | Review panels and checks hidden in review code. | Composition. | Composition M1/M3/M4/M6 | implemented (M3: `parallel_map` reducer for review fan-in + canonical review-fan-in node; M4/M6: final review panel conformance) | Review fanout and reducer are represented as workflow structure or declared policy. | `test_runtime.py::test_parallel_map_reducer_receives_ordered_results_and_item_paths`, `test_graph_projection.py::test_parallel_map_becomes_dynamic_fanout_stage`, `valid_m3_canonical_megaplan_topology.py` (review-fan-in parallel_map node with reducer). M4/M6 own final review panel conformance. | Parallel checks run but topology has one review node. | Static snapshot includes child review checks and reducer. |
...
| Runtime-list iteration | Current source compilers reject broad dynamic iteration. | Composition. | Composition M0/M1/M3/M6 | implemented | Supported runtime-list iteration or typed dynamic map for critique/review/execute. | `test_runtime.py::test_parallel_map_uses_parameter_precedence_and_preserves_item_order`, `test_compiler.py::test_parallel_map_accepts_workflow_mapper`, `test_compiler.py::test_parallel_map_rejects_dynamic_step_expression`, `test_decorators.py::test_parallel_map_importable`, `valid_m3_parallel_map_loop_policy` (dynamic `items=briefs` with runtime fanout). | Megaplan uses bespoke helper instead of general native construct. | Compiler acceptance/rejection fixtures prevent Megaplan-only escape hatch. |
| Dynamic parallel map | Fixed fanout exists; Megaplan needs runtime-selected lists. | Composition. | Composition M0/M1/M3/M6 | implemented | `parallel_map` or equivalent is a first-class native construct. | `test_decorators.py::test_parallel_map_instruction_importable` (ParallelMapInstruction IR distinct from ParallelInstruction), `test_graph_projection.py::test_parallel_map_becomes_dynamic_fanout_stage`, `test_runtime.py::test_parallel_map_empty_collection_invokes_reducer_with_empty_results`, `valid_m3_canonical_megaplan_topology.py` (critique-fanout, execute-batches, review-fan-in nodes all using `parallel_map`). | Fanout only works for hardcoded examples. | Dynamic-list fixture must vary list at runtime and preserve child paths. |
