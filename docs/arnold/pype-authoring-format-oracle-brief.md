...
The adopted answer allows small private local `@step` definitions in `.pype`;
they are unimportable and their behavior folds into the workflow digest.
Reusable steps live in ordinary `.py`, where their leaf implementations may use
normal Python/third-party imports under the exact dependency/environment/
feature/plugin lock and declared effect boundary. A step cannot contain or
invoke workflow topology.

Policies are immutable typed values in ordinary `.py`, conventionally
`policies.py`, lowered through one canonical envelope with stable kind/schema,
recursively immutable values, scope/attachment, provenance, precedence and
digest. They are not hidden route tables or another authoring language.
...
13. Component executable identity covers the component's own canonical closure
    and direct dependency contract requirements; a separate transitive graph-
    lock digest pins selected concrete descendants, and durable carriers bind
    both.
14. The first machine canonicalization uses the adopted semantic-IR/envelope/
    graph-lock schemas, RFC 8785 canonical JSON, and SHA-256; undefined tagged
    encodings reject.
15. Route selection uses finite named discriminants separate from payloads.
16. `default_pipeline` selects a workflow only; invoking admission separately
    binds exactly one total root-result adapter.
17. Intermediate proof runs before merge and rebinds after merge, while an
    irreversible authority/effect/publication switch separately consumes the
    exact accepted post-merge receipt.
