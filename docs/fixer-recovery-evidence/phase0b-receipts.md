...
| G6 (Phase 0B exit) | **GO** | 2026-08-12T09:02Z (after 11 rounds) | Rounds 1–8: 13 real defects closed by N1–N3, O1–O5, P1–P4, P5 + paperwork by N4; three recurring false positives documented (action_validator.py:331, raw-but-fail-closed class, ops-store test proof). Rounds 9–11: Q1/Q2 (template.py raw reads), Q3 (dependency-manifest-rematerialization census) closed; round 11 oracle GO — "safe to commit Phase 0+0B and proceed to Phase 1, T-0101 publish/stage/bind/resume"; committed receipt [g6-round11-receipt.md](g6-round11-receipt.md) (tree 90775c4403, exit 0, session 019ff51e-cdd4-7421-b34c-94abe653b2d6, output sha256 `d0f01b366fedc430c425a469505229cd0118cab5794d62b87a379f55437be2d1`). |

## 3. G6 gate runs (rounds 1–11, Codex oracle) — findings and closing fixes
...
- **Round 9 — 2 manifest-admission findings:** `template.py:228,233` raw `ENGINE_DIR`/`EXPECTED_REVISION` reads before schema validation → **Q1TemplateSchemaGate** (schema-gated canonical reads; compatibility-only/missing-root manifests rejected → exit 24, no template). `cli.py:3840` raw per-epic read + `:3973` `or default_runtime_src` fallback → **Q2CliNoDefaultRuntimeSrc** (schema-gated `_pinned_manifest_field_read`; fallback removed — the default was `_chain_runtime_worktree_path`, never ≠ canonical runtime_root). 149 + 112 focused tests green.

- **Round 10 — 1 finding:** live repair-loop plan-dir rematerialization deletes census-protected plan dirs with no CLEAR proof — `dependency_manifest_repair.py:156` `shutil.rmtree(target_nested)` and `:169-173` `rmtree(target_plan)`+`copytree(source_plan, target_plan)` → **Q3RepairRematerializationCensus** (reference census before every rmtree/copytree in `_sync_completed_prerequisite` and nested `.megaplan` replacement; REFERENCED/DANGLING/UNKNOWN → `CensusRefusalError`, zero deletion, writer note `dependency_manifest_repair_refused_census`, no false sync completion; CLEAR → proceeds). 6 new refusal/replace tests + 8 total green.

- **Round 11 — GO** (2026-08-12T09:02Z): oracle reviewed the full Phase 0+0B diff + receipts; verdict: all E1–E8 classes fixed-or-assigned, no mutation adapter admits SHADOW_PASS, launches consume the accepted canonical root and reject stale/shared/mismatched roots, raw/noncoherent observations cannot produce green views, no direct-provider fallback, T-0027 census covers plan-lease/managed-run/status/ops stores with REFERENCED+UNKNOWN refusal on every deletion route, residual defects in the six searched classes: none. "Yes — safe to commit Phase 0+0B and proceed to Phase 1, T-0101 publish/stage/bind/resume." Durable receipt: `g6-round11-receipt.md` (reviewed tree `90775c440311dcad83747596abccc48cd1e14ccf`, invocation `codex exec --sandbox read-only -m gpt-5.6-sol -c model_reasoning_effort=high`, exit 0, session `019ff51e-cdd4-7421-b34c-94abe653b2d6`, model gpt-5.6-sol, output sha256 `d0f01b366fedc430c425a469505229cd0118cab5794d62b87a379f55437be2d1`); the original raw output remains at `/tmp/codex_g6_out11.txt`.
