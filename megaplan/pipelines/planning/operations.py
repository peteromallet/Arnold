"""Planning plugin adapter for the neutral runtime operation registry."""
...
class PlanningOperationRegistry(OperationRegistry):
    ...
    def _dispatch_profile_validate(self, payload: Mapping[str, Any]) -> OperationResult:
        profile = _payload_mapping(payload, "profile") or _payload_mapping(
            payload, "resolved_profile"
        )
        if profile is None:
            return _invalid_request("profile_validate requires payload.profile")
