...
def _get_profiles_list() -> list[str]:
    ...
    try:
        result = subprocess.run(
            [sys.executable, "-m", "megaplan", "config", "profiles", "list"],
            capture_output=True,
            text=True,
            timeout=10,
    ...
    try:
        import megaplan.profiles

        profiles_dir = Path(megaplan.profiles.__file__).parent
        return sorted(
...
def _parse_decision_skill_profiles() -> list[str]:
    ...
    try:
        ...
        skill_path = Path(megaplan.data.__file__).parent / "decision_skill.md"
...
def _check_rubric_drift() -> list[tuple[str, str, str]]:
    """Check rubric/binary drift: diff decision_skill.md profiles vs installed profiles."""
    ...
    if missing:
        results.append(_check_status(
            f"Rubric/binary drift: {len(missing)} profile(s) in decision_skill.md not found in binary",
            False,
            severity="WARN",
            remediation=(
...
def _check_skill_sync() -> list[tuple[str, str, str]]:
    ...
    skill_pairs = [
        ("megaplan-decision", "decision_skill.md"),
    ...
    for skill_name, src_filename in skill_pairs:
        try:
            ...
            src_path = Path(megaplan.data.__file__).parent / src_filename
