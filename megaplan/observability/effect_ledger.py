...
unenforced field on STATE_WRITTEN events.
...
* ``effect_taint`` is the Effect-scoped propagation marker (renamed from any
  ...
  :class:`megaplan._pipeline.envelope.RunEnvelope.taint`.  RunEnvelope.taint
...
literal stamp on the STATE_WRITTEN payload (which is a plain string like
``"state_write"``).
"""
...
#: through :func:`megaplan.observability.events.emit_state_wal` without
...
class Effect:
    ...
    effect_taint:
        ...
        :class:`megaplan._pipeline.envelope.RunEnvelope.taint`.  Empty string
        means *unset*.
    """
