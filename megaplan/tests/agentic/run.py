...
Usage::
    ...
    python -m megaplan.tests.agentic.run [--actor hermes|fake] [--verbose]
...
def main(argv: list[str] | None = None) -> int:
    ...
    # __file__ = <repo>/megaplan/tests/agentic/run.py — go up 4 to hit <repo>.
    repo_root = Path(__file__).resolve().parents[3]

    # Lazy import so a missing sisypy is caught early with a clear message.
    from megaplan.tests.agentic.adapter import MegaplanAdapter  # noqa: E402
    ...
    result = run_all(
        adapter=adapter,
        scenarios_dir=repo_root / "megaplan" / "tests" / "agentic" / "scenarios",
        briefs_dir=repo_root / "megaplan" / "tests" / "agentic" / "briefs",
