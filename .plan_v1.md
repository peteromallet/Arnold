...
6. **Define the Verdict prose update rule**: in `_pipeline/` source files, update docstrings, comments, and prose that refer to the dataclass by its class name (e.g., "typed `Verdict`", "Verdict-first edge dispatch", "a `Verdict` on the parent") to use `JudgeVerdict`. Leave generic conceptual prose about "verdicts" as a noun (e.g., "the judge returns a verdict", "gate verdict") unchanged in non-pipeline files and in pipeline files where the word is clearly a domain noun rather than a class reference.
7. Note that the vocabulary map is a living document amendable during implementation if edge cases are discovered; amendments must be reviewer-visible.
...
4. Update active metadata readers to accept `phase` first and legacy `step` second in:
   ...
   - `megaplan/observability/doctor.py` (line ~269): same pattern
   - `megaplan/observability/introspect.py` (lines ~233, ~461): same pattern
...
### Step 8: Rename `_pipeline.types.Verdict` to `JudgeVerdict`
**Complexity: 4**
1. Rename the dataclass in `megaplan/_pipeline/types.py` and update `StepResult.verdict` annotation to `JudgeVerdict | None`; do not rename the `StepResult.verdict` field unless a separate migration is approved.
...
3. Apply the Verdict prose rule from the vocabulary map: in `_pipeline/` source files, update docstrings, comments, and prose that reference the dataclass by its class name (e.g., "typed `Verdict`", "Verdict-first edge dispatch", "a `Verdict` on the parent") to use `JudgeVerdict`. Leave generic conceptual prose about "verdicts" as a domain noun (e.g., "the judge returns a verdict", "gate verdict") unchanged.
4. Update active docs `docs/pipelines.md` and `docs/pipeline-architecture.md` to refer to `JudgeVerdict` for the dataclass; leave conceptual verdict prose.
5. Leave `review_verdict`, `reviewer_verdict`, `sense_checks[].verdict`, verifiability pass/fail text, and schema-required `verdict` keys unchanged; these are called out in `docs/canonical-vocabulary.md` as separate domain meanings.
...
1. Run focused tests for each touched area:
   ...
   - Step 8 (Verdict→JudgeVerdict): `tests/_pipeline/test_*`, `tests/test_pipeline_*.py`, `tests/test_init_plan.py`, `tests/test_finalize.py`
...
3. Run cross-format grep checks over `.py`, `.json`, `.md`, `.yaml`, `.yml` for stale identifiers, excluding the zones defined in the vocabulary map (Step 1.6): `dispatch_execute_auto_loop`, `dispatch_execute_one_batch`, `HumanGateStep`, `STATE_AWAITING_HUMAN` (constant only, not string value), `ActiveStep` (TypedDict only), `active_step` objects writing `"step"` (the TypedDict field, not history entries), and pipeline-dataclass `Verdict` references (class construction/annotation, not domain prose in non-pipeline files). Exclusion zones from vocabulary map apply: `docs/archive/**`, `briefs/**`, `CHANGELOG.md`, `**/__pycache__/**`, `docs/yaml-pipelines-migration.md`, `docs/foundation-audit/**`, generated/vendored agent material under `megaplan/agent/**` except active source.
4. Run the broader relevant suite if focused tests pass.

## Execution Order
1. Land `docs/canonical-vocabulary.md` first (includes exclusion zones and Verdict prose rule).
...
5. Rename `Verdict` to `JudgeVerdict` (Step 8) after the smaller vocabulary changes are stable.
...
- Whether external users require a temporary `Verdict = JudgeVerdict` alias. Default: no alias, because the done criteria require stale identifier greps to go clean for active code.
