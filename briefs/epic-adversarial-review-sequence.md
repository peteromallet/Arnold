# Add an executable epic adversarial review sequence

## Outcome

Add a first-class executable megaplan pipeline sequence that critiques an epic
through three ordered critique/revise rounds:
...
- Add a new registered pipeline, likely discoverable as
  `epic-adversarial-review`, following the current Python pipeline module
  pattern used by `megaplan/pipelines/writing_panel_strict.py`.
- The pipeline should accept an epic draft document as its primary input.
- It should run three critique panels, each with five independent critic lenses.
...
- Add co-located prompts and, if consistent with current pipeline conventions,
  a `SKILL.md` describing when and how to invoke the sequence.
...
- Do not merge the epic review checks into the existing sprint critique check
...
- The user-facing CLI should be something like:

  ```bash
  megaplan run epic-adversarial-review path/to/epic.md
  ```
...
There is an existing untracked `docs/epic-adversarial-review.md` from an
earlier docs-only attempt. Treat it as optional context, not as the deliverable.
The deliverable is the executable sequence.

## Done criteria

- `registered_pipelines()` includes `epic-adversarial-review`.
- `megaplan run --list` includes the new pipeline with a useful description.
- `megaplan run epic-adversarial-review path/to/epic.md --plan-dir <dir>` works
  with a deterministic test worker or test harness.
