...
(`briefs/validation/sequencing/PROGRAM.md` — the M1 entry L84–103; critical path L340; strangler
discipline L361–389; open risks #2 L428–435) and the eleven-organ architecture
(`briefs/pipeline-unification-EPIC.md` §"The architecture" L23–84, §"Sequenced build program — FINAL"
L225–258). Organ specs: `briefs/validation/committed-uu/SYNTHESIS.md` Reshaper #1 (L69, L343–348),
R7 model-identity (L75, L385–388), Effect Ledger (L259–266), design principles #1/#2/#3/#10/#14
(L445–514). Human blockers all resolved to defaults: `briefs/validation/human-blockers/REGISTER.md`
...
milestone `label` + `idea:` brief path (format per `briefs/hardening-epic/chain.yaml:19–127`) maps to a
milestone in the EPIC program (`PROGRAM.md` L69–332) and a real file in `briefs/epic-pipeline-unification/`
— count and ordering match, no orphan brief, no dangling path. NB `briefs/epic-pipeline-unification/
...
- **chain.yaml provenance (blocking W8).** → **This epic authors its own
  `briefs/epic-pipeline-unification/chain.yaml`** (gives the lint a target); W8 fails loud until it exists
...
`megaplan/cloud/supervise.py:54` · `briefs/hardening-epic/chain.yaml` (format ref) · new
`briefs/epic-pipeline-unification/chain.yaml` · new `tests/fixtures/state_json/` · new `pipelines`
