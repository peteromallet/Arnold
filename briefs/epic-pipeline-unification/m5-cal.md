...
1. **CapabilityClaim record + write path into the one Ledger.** New record `CapabilityClaim{task_signature (typed, domain-tagged), predicted_tier:int, routed_model:ModelIdentity, verifier_identity, verifier_tier, outcome:EvaluandRef, cost, counterfactual_tag, taint_class, recorded_at}`. The outcome field is an Evaluand reference (M5-eval / m4 `:30` record shape `{judge-version, rubric-version, input-set-hash, score}`), **never a bare float** — enforced by a type assertion + grep gate. Write on every adjudicated dispatch via the M4 `EventSink.emit` one-log path (m4 §7, R5), keyed by the R7 hash-pinned `model_identity` seeded into `COST_RECORDED` at M1 (`cost.py:100–118`). New module `megaplan/calibration/ledger.py`.
...
6. **TAINT-AWARE AGGREGATION.** Read the taint lattice off the M3 Envelope (per `Depends_on` M3). A claim's `taint_class` decides SHARED-ledger vs tenant-local: tainted/private claims stay tenant-local and never enter the cross-tenant corpus (UU#11). The privacy↔flywheel trade is an explicit tunable `aggregation_policy` knob (default: in-tree single-tenant = shared; out-of-tree = tenant-local), not an emergent shatter.
...
- `megaplan/observability/cost.py:23,71–118` — R7 `model_identity` source (seeded M1); the sensors (prefix-cache-hit-rate, monoculture index) feeding the gated experiments; kill the read-time substring `_classify_vendor` reliance once the Ledger is the source (report-only here, retired with R5 at M6 per m4 §7).
