...
**Concrete fix:** Add a lighter orchestration profile for behavior-preserving move/rename milestones (m4, m5a, m5b, m5d) — keep premium only for the critique pass, drop the per-batch driving/review turns to a mid-tier model. Edit `briefs/hardening-epic/chain.yaml` m5b entry to `depth: medium` and verify the harness routes review/gate to a cheaper model for `directed` at medium depth.
