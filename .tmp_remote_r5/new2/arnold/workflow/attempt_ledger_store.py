...
* **Idempotency-key uniqueness with content-safe dedup** — appending an
  event whose ``(attempt_id, idempotency_key)`` already exists compares
  canonical event content. If byte-equivalent, the store returns the
  existing persisted event with ``is_duplicate=True``. If divergent, it
  raises :class:`IdempotencyConflictError`. Two different events with the
  same idempotency key can never coexist.
...
class IdempotencyConflictError(AttemptLedgerError):
    """Raised when an idempotency key matches but the event content differs.

    When an ``(attempt_id, idempotency_key)`` pair already exists in the
    durable store, the append path compares the canonical serialized
    content of the incoming event against the persisted row.  Exact
    byte-equivalent retries are preserved as no-ops (the existing event is
    returned with ``is_duplicate=True``).  Any divergence — different
    sequence, event_type, identity, provenance, versions, or any other
    field — raises this error so a retry carrying different evidence can
    never silently overwrite or hide the original.

    This check runs *before* the post-terminal and monotonic-sequence
    checks, so a divergent retry is always reported as a conflict even if
    the attempt has since reached a terminal state or the incoming
    sequence would violate monotonicity.  A corrupt stored row that
    cannot be deserialized for comparison is also treated as a conflict
    because equivalence cannot be established.
    """


# ── Canonical content comparison (Step 3: shared idempotency contract) ────


def canonical_event_content(event: LedgerEvent) -> str:
    """Return the canonical string serialization of *event* for exact comparison.

    The same rule is used by every append path (plain attempt append and
    append-with-outbox) so that idempotency-key dedup has exactly one
    definition of "same event".  Two events are considered byte-equivalent
    when this function returns identical strings for both.

    The serialization matches the durable on-disk representation:
    ``json.dumps(event.to_dict(), sort_keys=True, ensure_ascii=False)``.
    Callers should never construct a competing canonicalization.
    """
    return json.dumps(event.to_dict(), sort_keys=True, ensure_ascii=False)


def canonical_event_json(event: LedgerEvent) -> str:
    """Compatibility name for the shared canonical event serializer.

    The ledger/outbox contract historically called this helper
    ``canonical_event_json``.  Keep that public spelling as a thin alias so
    callers cannot accidentally reintroduce a second serialization rule.
    """
    return canonical_event_content(event)


def events_canonical_equal(a: LedgerEvent, b: LedgerEvent) -> bool:
    """Return ``True`` when *a* and *b* have byte-equivalent canonical content.

    This is the single shared comparison routine used across all append
    paths.  It delegates to :func:`canonical_event_content` so there is
    exactly one definition of event equivalence in the store.
    """
    return canonical_event_content(a) == canonical_event_content(b)
...
class SqliteAttemptLedgerStore(AttemptLedgerStore):
    ...
    * The store metadata table captures the pinned contract version
    ...
    def _append_tx(
        ...
        Order of checks (canonical-content idempotency wins over rejection):
        ...
        3. Idempotency-key dedup — if ``(attempt_id, idempotency_key)``
           already exists, compare canonical event content.  If
           byte-equivalent, return the existing event (no raise).  If
           divergent or the stored row is corrupt and cannot be compared,
           raise :class:`IdempotencyConflictError`.
        ...
        try:
            ...
            # (3) Idempotency-key dedup with canonical-content comparison.
            #     Checked BEFORE any rejection so retries of an event that
            #     has since become post-terminal still return the existing
            #     event rather than raising — but only when the content is
            #     byte-equivalent.  Divergent content always raises
            #     IdempotencyConflictError, even post-terminal.
            cur.execute(
            ...
            if dup_row is not None:
                # Deserialize the stored row for comparison.  If the row
                # is corrupt (invalid JSON or schema mismatch) we cannot
                # establish equivalence, so treat it as a conflict.
                try:
                    existing = _deserialize_ledger_event(json.loads(dup_row[0]))
                except Exception as exc:
                    conn.execute("ROLLBACK")
                    raise IdempotencyConflictError(
                        f"Idempotency key {event.idempotency_key!r} for "
                        f"attempt {attempt_id!r} has a corrupt stored row "
                        f"that cannot be compared: {exc}"
                    ) from exc

                if not events_canonical_equal(event, existing):
                    conn.execute("ROLLBACK")
                    raise IdempotencyConflictError(
                        f"Idempotency key {event.idempotency_key!r} for "
                        f"attempt {attempt_id!r} already exists with "
                        f"divergent canonical content."
                    )

                # Byte-equivalent — return existing event as duplicate.
                conn.execute("ROLLBACK")
                return AppendResult(
                    attempt_id=attempt_id,
                    event=existing,
