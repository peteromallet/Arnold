...
class SqliteLedgerOutbox(LedgerOutbox):
    ...
    def append_event_with_outbox(
        ...
        Atomicity:
            1. ``BEGIN IMMEDIATE``
            2. Idempotency-key dedup check with canonical-content
               comparison (idempotent no-op wins over rejection; divergent
               content raises :class:`IdempotencyConflictError`)
        ...
        If *event* is a byte-equivalent duplicate, only steps 1-2 execute
        and the transaction is rolled back before returning the existing
        event with ``is_duplicate=True`` and empty outbox records.  If the
        stored content diverges, :class:`IdempotencyConflictError` is
        raised before any post-terminal or monotonic-sequence check.
        """
        ...
        event_json = canonical_event_content(event)
        conn = self._conn
        now_ns = time.time_ns()
        ...
        try:
            ...
            # (2) Idempotency-key dedup with canonical-content comparison.
            #     Checked BEFORE post-terminal / monotonic so an exact retry
            #     always no-ops (even if the attempt is now terminal), while
            #     divergent content always raises IdempotencyConflictError.
            cur.execute(
